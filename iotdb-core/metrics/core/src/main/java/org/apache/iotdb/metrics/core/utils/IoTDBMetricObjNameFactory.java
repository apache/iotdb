...
public class IoTDBMetricObjNameFactory implements ObjectNameFactory {
  ...
  public ObjectName createName(String type, String domain, String name) {
    try {
      ...
      // Quote before constructing the name; falling back to a name-only MBean loses its type.
      properties.put("name", shouldQuote(name) ? ObjectName.quote(name) : name);
      properties.put("type", shouldQuote(type) ? ObjectName.quote(type) : type);
      return new ObjectName(domain, properties);
    } catch (MalformedObjectNameException e) {
      LOGGER.warn(MetricsCoreMessages.JMX_UNABLE_TO_REGISTER, type, name, e);
