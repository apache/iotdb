...
public class AsyncConfigNodeClientPool {
    ...
    private AsyncConfigNodeClientPool() {
        clientManager =
                new IClientManager.Factory<TEndPoint, AsyncConfigNodeIServiceClient>()
                        .createClientManager(
                                new ClientPoolFactory.AsyncConfigNodeIServiceClientPoolFactory());
    }
     ...
     * @param clientHandler <RequestType, ResponseType> which will also contain the result
     */
    public void sendAsyncRequestToConfigNodeWithRetry(AsyncConfigNodeClientHandler<?, ?> clientHandler) {
