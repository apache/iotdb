...
//            System.out.println("p="+p);
    ...
    private static int[] adjustCase1(
            int[][] ts_block,
            int alpha,
            int j_star,
            double[] theta,
    ...
    private static int[] adjustCase2(
            int[][] ts_block,
            int alpha,
            int j_star,
            double[] theta,
    ...
    private static int[] adjustCase3(
            int[][] ts_block,
            int alpha,
            int j_star,
            double[] theta,
    ...
    private static int[] adjustCase4(
            int[][] ts_block,
            int alpha,
            int j_star,
            double[] theta,
...
//    quickSort(ts_block_delta_value, 1, 0, block_size - 2);
    ...
    private static int REGERBlockEncoderPartition(int[][] data, int i, int block_size, int segment_size, int k, int p, int encode_pos, byte[] cur_byte) {
...
//    ArrayList<ArrayList<Integer>> new_length_list = new ArrayList<>();
        ...
        while (isMoveable.size() != 0) {
            ...
            if (all_length.get(0).get(1) <= raw_length[0]) {
                ...
                ts_block_delta = getEncodeBitsRegression(new_ts_block, block_size, new_length, theta, segment_size, p);
                int[][] bit_width_segments = segmentBitPacking(ts_block_delta, block_size, segment_size);
                new_length[0] = numberOfEncodeSegment2Bytes(ts_block_delta, bit_width_segments, new_length, segment_size, p, theta);
        ...
        ts_block_delta = getEncodeBitsRegressionNoTrain(ts_block, block_size, raw_length, theta, segment_size, p);
...
//    ArrayList<ArrayList<Integer>> bit_width_segments = new ArrayList<>();
        ...
        encode_pos = encodeSegment2Bytes(ts_block_delta, bit_width_segments, raw_length, segment_size, p, theta, encode_pos, cur_byte);

        return encode_pos;
    }

    private static int REGERBlockEncoder(int[][] data, int order, int i, int block_size, int supply_length, int[] third_value, int segment_size, int k, int p, int encode_pos, byte[] cur_byte) {
...
//        System.out.println(Arrays.deepToString(ts_block));
        ...
        if (order == 1) {
            ...
            ts_block_delta_reorder = getEncodeBitsRegression(ts_block, block_size, reorder_length, theta_reorder, segment_size, p);
            ...
            ts_block_delta_time = getEncodeBitsRegression(ts_block, block_size, time_length, theta_time, segment_size, p);
            ...
            ts_block_delta = getEncodeBitsRegression(ts_block_partition, block_size, raw_length, theta, segment_size, p);
...
//        System.out.println("getEncodeBitsRegressionNoTrain before:" + Arrays.deepToString(ts_block_delta));

        ts_block_delta = getEncodeBitsRegressionNoTrain(ts_block, block_size, raw_length, theta, segment_size, p);

//        System.out.println("getEncodeBitsRegressionNoTrain after:" +Arrays.deepToString(ts_block_delta));
...
//        System.out.println("encode_pos="+encode_pos);
    ...
    public static int ReorderingRegressionEncoder(
            int[][] data, int block_size, int[] third_value, int segment_size, int p, int k, byte[] encoded_result) {
        for (int i = 0; i < p; i++)
            block_size++;
...
//            for (int i = 0; i < 2; i++) {
            ...
            for (int i = 0; i < block_num; i++) {
                encode_pos = REGERBlockEncoderPartition(data, i, block_size, segment_size, k, p, encode_pos, encoded_result);
            }
        } else {
...
//                for (int i = 0; i < 2; i++) {
                for (int i = 0; i < block_num; i++) {
                    ...
                    encode_pos = REGERBlockEncoder(data, 1, i, block_size, 0, third_value, segment_size, k, p, encode_pos, encoded_result);
                }
            } else {
                System.out.println("Time");
//                for (int i = 0; i < 2; i++) {
                for (int i = 0; i < block_num; i++) {
                    encode_pos = REGERBlockEncoder(data, 0, i, block_size, 0, third_value, segment_size, k, p, encode_pos, encoded_result);
                }
            }
        }
        ...
        if (remaining_length != 0 && remaining_length != 1) {
            ...
            encode_pos = REGERBlockEncoder(data, 0, block_num, block_size, supple_length + remaining_length, third_value, segment_size, k, p, encode_pos, encoded_result);
...
//                for (int p = 1; p < 10; p++) {
                    ...
                    for (int repeat_i = 0; repeat_i < 1; repeat_i++)
                        length = ReorderingRegressionEncoder(data2_arr, dataset_block_size.get(file_i), dataset_third.get(file_i), 8, p, 1, encoded_result);
