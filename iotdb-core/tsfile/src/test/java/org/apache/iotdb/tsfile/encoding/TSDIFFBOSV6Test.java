...
//        System.out.println(encode_pos);
        ...
        int final_x_5_minus = -1;
        int final_x_5_plus = 0;
...
//            cur_bits += Math.min((cur_k2 + cur_k1) * getBitWith(block_size-1), block_size + cur_k2 + cur_k1);
            ...
            if (cur_k5 != n_center_values3)
                cur_bits += (n_center_values - cur_k5)  * getBitWith(final_x_3_minus - (x_4_plus_value));
...
//            encode_pos += ((block_size + (final_k1+final_k2)+2*(final_k3+final_k4))/8);
            ...
            int final_center_bits =(final_k5 *getBitWith(final_x_4_minus-final_x_2_plus)
                    +(n_center_values3-final_k5)*getBitWith(final_x_3_minus-(final_x_4_plus)));
            encode_pos += (final_center_bits)/8;
            encode_pos += ((block_size +final_k1+final_k2)/8);
            encode_pos += ((n_center_values +final_k3+final_k4)/8);
