...
public class TSDIFFBOSBetaPrune1Test {
...
//        return encode_pos;
    ...
    private static int BOSEncodeBits(int[] ts_block_delta,
        ...
        int end_encode_pos = encode_pos;
//        System.out.println("final___________bits : "+(end_encode_pos-start_encode_pos));
...
//            System.out.println(num_of_alpha_list);
//            System.out.println(Arrays.toString(cur_group_gamma.map.get(12)));
        }
...
//                        && pow_2_alpha + (max_pow_2_lambda + 1) * pow_2_gamma <= max_delta_value)) {
                    if (!(2* pow_2_alpha + pow_2_gamma <= max_delta_value
                            && pow_2_alpha + 2 * pow_2_gamma <= max_delta_value)) {

                        int[] beta_array = cur_group_alpha.map.get(gamma);
//                System.out.println(Arrays.toString(beta_array));
                    for(int beta_i=0;beta_i<beta_array.length;beta_i+=2) {
                        int x_u_i = beta_array[beta_i];
                        int cur_cur_k1 = cur_k1;
                        int cur_cur_k2 = cur_k2 + beta_array[beta_i+1];
                        cur_bits = 0;
                        ...
                        if (cur_cur_k1 + cur_cur_k2 != block_size)
                            cur_bits += (block_size - cur_cur_k1 - cur_cur_k2) *
                                    getBitWith(max_delta_value - pow_2_gamma + x_u_i - pow_2_alpha);
                        cur_bits += cur_cur_k2 * gamma;
                        if (cur_bits < min_bits) {
                            min_bits = cur_bits;
                            final_k_start_value = k1_start;
                            final_k_end_value = k2_end - x_u_i;
//                        System.out.println("cur_gamma:" + groupU[gamma]);
//                        System.out.println("ts_block: "+(Arrays.toString(ts_block_delta)));
//                        System.out.println("cur_bits: "+(cur_bits/8));
                        }
                    }
...
//
            ...
            for(int gamma_j=0;gamma_j<num_of_gamma_list;gamma_j++){
                int gamma = gamma_list[gamma_i];
                if(gamma > gamma_size){
                    break;
                }
                GroupU cur_group_gamma_ii = groupU[gamma];
                int[] beta_array = cur_group_gamma_ii.map.get(alpha);
                for(int beta_i=0;beta_i<beta_array.length;beta_i+=2) {
                    int x_l_i = beta_array[beta_i];
                    int cur_cur_k1 = cur_k1+beta_array[beta_i+1];
                    ...
                    if (cur_cur_k1 + cur_cur_k2 != block_size)
                        cur_bits += (block_size - cur_cur_k1 - cur_cur_k2) *
                                getBitWith(max_delta_value-pow_2_alpha-pow_2_alpha-x_l_i);
                    cur_bits += cur_cur_k2 * gamma;
                    if (cur_bits < min_bits) {
                        min_bits = cur_bits;
                        final_k_start_value = k1_start+x_l_i;
...
//            System.out.println(i);
    ...
    public void PruneBOSTest() throws IOException {
        ...
        String output_parent_dir = parent_dir + "vldb/compression_ratio/test_beta_prune1";
