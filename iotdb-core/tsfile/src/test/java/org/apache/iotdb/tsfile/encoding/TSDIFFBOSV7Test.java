...
//        int test= 0;
        ...
        for (int start_value_i = 1; start_value_i < cur_value_i_unique; start_value_i++) {
            ...
            cur_bits = 0;
            cur_bits += 80;
            cur_bits += Math.min((cur_k3) * getBitWith(block_size-1), block_size + cur_k3);
...
//        System.out.println(encode_pos);
        int n_center_values3 = n_center_values-final_k3-final_k4;
        long[] center_values_3L = new long[n_center_values];
        int final_x_4_minus = final_x_2_plus;
        int final_x_4_plus = final_x_3_minus;
        int final_x_5_minus = final_x_4_minus;
        int final_x_5_plus = final_x_4_plus;
        int cur_value_i_3_unique = 0;
        int final_k5 = 0;
        int final_k6 = 0;

        for (int end_value_i = 0; end_value_i < cur_value_i_unique; end_value_i++) {

            int cur_value = getUniqueValue(center_valuesL[end_value_i], left_shift);

            if(cur_value > final_x_2_minus && cur_value < final_x_3_plus){
                int cur_count =  getCount(center_valuesL[end_value_i],mask);
                center_values_3L[cur_value_i_3_unique] = (((long) (cur_value)) << left_shift) + cur_count - final_k3;
                cur_value_i_3_unique ++;
            }
        }

        int center_bits_5_part = n_center_values3 * getBitWith(final_x_3_minus - final_x_2_plus);
        min_bits_center = center_bits_5_part;
        for (int start_value_i = 1; start_value_i < cur_value_i_3_unique; start_value_i++) {
            int x_4_minus_value = getUniqueValue(center_values_3L[start_value_i-1], left_shift);
            int x_4_plus_value = getUniqueValue(center_values_3L[start_value_i], left_shift);
            int cur_bits = n_center_values3+40;
            int cur_k5 = getCount(center_values_3L[start_value_i-1],mask);
//            cur_bits += Math.min((cur_k2 + cur_k1) * getBitWith(block_size-1), block_size + cur_k2 + cur_k1);
            if (cur_k5 != 0 )
                cur_bits += cur_k5 * getBitWith(x_4_minus_value-final_x_2_plus); // cur_k1 = 0
            if (cur_k5 != n_center_values3)
                cur_bits += (n_center_values3 - cur_k5)  * getBitWith(final_x_3_minus - (x_4_plus_value));


            if (cur_bits < min_bits_center) {
                min_bits_center = cur_bits;
                final_x_4_minus = x_4_minus_value;
                final_x_4_plus = x_4_plus_value;
                final_k5 = cur_k5;
            }
        }
//        int test1= 0;
        ...
        for (int start_value_i = 1; start_value_i < cur_value_i_3_unique; start_value_i++) {
            long k_start_valueL = center_values_3L[start_value_i];
            int x_4_minus_value =  getUniqueValue(k_start_valueL, left_shift) ;
            int x_4_plus_value =  getUniqueValue(center_values_3L[start_value_i+1], left_shift) ;


            int cur_k5 = getCount(k_start_valueL,mask);

            int k_end_value;
            int cur_bits;
            int cur_k6=0;
            k_end_value = final_x_3_minus + 1;

            cur_bits = 0;
            cur_bits += 160;
            cur_bits += (n_center_values3 + cur_k5);
            cur_bits += cur_k5 * getBitWith(x_4_minus_value - final_x_2_plus);
            if (cur_k5 != n_center_values3)
                cur_bits += (n_center_values3 - cur_k5) * getBitWith(final_x_3_minus- x_4_plus_value); //cur_k2 =0
            ...
            for (int end_value_i = start_value_i + 1; end_value_i < cur_value_i_3_unique; end_value_i++) {

                int x_5_minus_value = getUniqueValue(center_values_3L[end_value_i-1], left_shift);
                int x_5_plus_value = getUniqueValue(center_values_3L[end_value_i], left_shift);

                cur_bits = 0;
                cur_bits += 160;
                cur_k6 = n_center_values3 - getCount(center_values_3L[end_value_i-1],mask);

//                cur_bits += (23 * 8);
                cur_bits += ( n_center_values3 + cur_k5 + cur_k6);
                cur_bits += cur_k5 * getBitWith(x_4_minus_value-final_x_2_plus);
                if (cur_k5 + cur_k6 != n_center_values3)
                    cur_bits += (n_center_values3 - cur_k5 - cur_k6) * getBitWith(x_5_minus_value - x_4_plus_value);
                if (cur_k6 != 0)
                    cur_bits += cur_k6 * getBitWith(final_x_3_minus - x_5_plus_value);
                ...
                if (cur_bits < min_bits_center) {
                    ...
                    final_x_4_minus = x_4_minus_value;
                    final_x_4_plus = x_4_plus_value;
                    final_x_5_minus = x_5_minus_value;
                    final_x_5_plus = x_5_plus_value;
                    final_k5 = cur_k5;
                    final_k6 = cur_k6;
                }
        ...
        encode_pos += ((final_k1*getBitWith(final_k_start_value )
                +final_k2*getBitWith(max_delta_value-final_k_end_value)
        +final_k3*getBitWith(final_x_2_minus-final_x_l_plus)
        +final_k4*getBitWith(final_x_u_minus-final_x_3_plus))/8);
        if(min_bits_center >= center_bits_5_part){
            encode_pos += 33;
            encode_pos += (center_bits_5_part/8);
            encode_pos += ((block_size +final_k1+final_k2)/8);
            encode_pos += ((n_center_values +final_k3+final_k4)/8);
//            encode_pos += ((block_size + (final_k1+final_k2)+2*(final_k3+final_k4))/8);

        }else {
            encode_pos += 43;
            int final_center_bits =(final_k5 *getBitWith(final_x_4_minus-final_x_2_plus)
                    +(n_center_values3-final_k5-final_k6)*getBitWith(final_x_5_minus-(final_x_4_plus))
            +final_k6*getBitWith(final_x_3_minus-final_x_5_plus));
            encode_pos += (final_center_bits)/8;
            encode_pos += ((block_size +final_k1+final_k2)/8);
            encode_pos += ((n_center_values +final_k3+final_k4)/8);
            encode_pos += ((n_center_values3 +final_k5+final_k6)/8);

//            encode_pos += ((block_size +(final_k1+final_k2)+2*(final_k3+final_k4)+3*final_k5)/8);
        }
...
//        String parent_dir = "/Users/zihanguo/Downloads/R/outlier/outliier_code/encoding-outlier/";
        String output_parent_dir = parent_dir + "icde0802/supply_experiment/R3O4_vary_part/compression_ratio/bos_7";
