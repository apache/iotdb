...
//            System.out.println(Arrays.toString(time_length));
    ...
    public static int ReorderingRegressionEncoder(long[] data, int block_size, int[] third_value, int segment_size, byte[] encoded_result, long[] cur_memory_cost) {
        block_size++;
...
//            System.out.println("memoryBefore: "+runtime.freeMemory());
...
//            System.out.println("memoryAfter: "+runtime.freeMemory());
            if(memoryAfter>=memoryBefore){
                memory_cost += (memoryAfter - memoryBefore);
                memory_count ++;
            }
        }
...
//            System.out.println("memoryAfter: "+runtime.freeMemory());
        ...
        if(memory_count==0)
            ...
            cur_memory_cost[0] = (memory_cost/memory_count);
...
//                for (int block_size_exp = 12; block_size_exp >= 12; block_size_exp--) {
                for (int block_size_exp = 13; block_size_exp >= 4; block_size_exp--) {
                    ...
                    for (int repeat = 0; repeat < repeatTime2; repeat++){
                        Runtime runtime = Runtime.getRuntime();
//                        long memoryBefore = runtime.totalMemory() - runtime.freeMemory();
                        long[] cur_memory_cost = new long[1];
...
//                        long memoryAfter = runtime.totalMemory() - runtime.freeMemory();
                        if(cur_memory_cost[0] != 0){
                            realRepeatTime ++;
                            memory_cost += cur_memory_cost[0];
                            encodeTime += (e - s);
                        }
...
//                        memory_cost += (memoryAfter - memoryBefore)/(data.size());
                    ...
                    encodeTime /=realRepeatTime;// repeatTime2;
                    memory_cost /= ((long) realRepeatTime *block_size);
