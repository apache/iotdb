...
//        if(k1+k2!=block_size)
    ...
    private static int BOSBlockEncoder(int[] ts_block, int block_i, int block_size, int remaining ,int encode_pos , byte[] cur_byte) {
        ...
        for (int end_value_i = 1; end_value_i < unique_value_count; end_value_i++) {
            ...
            cur_bits += min((cur_k2 + cur_k1) * getBitWith(block_size-1), block_size + cur_k2 + cur_k1);
...
//        }
        ...
        for (int start_value_i = 0; start_value_i < unique_value_count-1; start_value_i++) {
            ...
            int end_minus_start_i = 1;
            int end_of_end_minus_start_i = unique_value_count - start_value_i - 1;
            int previous = start_value_i;
            ...
            for(int beta = 0; beta < beta_max; beta++){
                ...
                end_value_i = min(start_value_i +  end_minus_start_i,unique_value_count-1);

                for (; end_value_i > previous; end_value_i--) {
                    ...
                    if(x_u_minus_value < x_u_plus_pow_beta && k_end_value >= x_u_plus_pow_beta){
                        ...
                        cur_bits += min((cur_k1 + cur_k2) * getBitWith(block_size-1), block_size + cur_k1 + cur_k2);
                ...
                end_minus_start_i = 2*end_minus_start_i;
                previous = end_value_i;
            }
