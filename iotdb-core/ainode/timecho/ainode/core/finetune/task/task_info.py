...
class FinetuneTask:
    ...
    def exp_dir(self) -> str:
        exp_base = AINodeDescriptor().get_config().get_ain_exp_dir()
        return os.path.join(exp_base, self.task_id)

    @property
    def output_model_dir(self) -> str:
        models_base = AINodeDescriptor().get_config().get_ain_models_dir()
        return os.path.join(
            models_base, ModelCategory.FINE_TUNED.value, self.model_args.model_id
        )
