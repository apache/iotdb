...
from iotdb.ainode.core.model.model_constants import TRAINING_STATE
from iotdb.ainode.core.model.model_loader import (
    restore_model_weights,
    restore_training_state,
)
from iotdb.ainode.core.model.model_loader import save_model as _save_model
from iotdb.ainode.core.model.model_loader import save_training_checkpoint as _save_ckpt
from iotdb.ainode.core.model.model_loader import unwrap_model as _unwrap
...
class Trainer:
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.start_epoch, num_epochs):
            ...
            if self.val_dataloader is not None:
                ...
                if 0 < args.early_stopping_patience <= self.patience_counter:
    ...
    def _train_epoch(self, epoch: int) -> float:
        self.model.train()
        accum_steps = self.training_args.gradient_accumulation_steps
        total_loss = 0.0
        num_micro_batches = 0
        ...
        for batch_idx, batch in enumerate(self.train_dataloader):
            ...
            if (batch_idx + 1) % accum_steps == 0 or (batch_idx + 1) == len(
                ...
                if (
                    self.global_step % self.training_args.logging_steps == 0
    ...
    def _validate(self) -> float:
        ...
        with torch.no_grad():
            for batch in self.val_dataloader:
                ...
                total_loss += self._extract_loss(outputs).item()
                num_batches += 1

        avg_loss = total_loss / max(num_batches, 1)

        # In DDP each rank validates a subset.  Aggregate to a single value so
        # all ranks make the *same* early-stopping decision (avoids hang).
        if dist.is_available() and dist.is_initialized():
            loss_tensor = torch.tensor(
                [avg_loss, float(num_batches)], device=self.device
            )
            dist.all_reduce(loss_tensor, op=dist.ReduceOp.SUM)
            total_sum, total_count = loss_tensor.tolist()
            avg_loss = total_sum / max(total_count, 1.0)
    ...
    def _save_checkpoint(self, name: str) -> None:
        ...
        if self._is_main_process and self.SAVE_CKPTS:
