...
class WeaverArguments:
    ...
    def to_weaver_config_dict(self) -> dict:
        ...
        if int(self.input_channel) <= 0:
            raise ValueError(
                "Finetune argument 'input_channel' must be a positive integer."
            )
        return {
            "weaver_type": self.weaver_type,
            "input_channel": int(self.input_channel),
            "output_channel": int(self.output_channel),
            "kernel_size": int(self.weaver_kernel_size),
            "dropout": float(self.weaver_dropout),
            "zero_init": bool(self.weaver_zero_init),
            "test_pred_len": int(self.test_pred_len),
            "test_n_sample": int(self.test_n_sample),
        }
