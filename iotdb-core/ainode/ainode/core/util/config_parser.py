...
def convert_thuTL_config_to_ainode_format(
    ...
    Args:
        thuTL_config: thuTL配置字典

    Returns:
        AINode格式的配置字典
    """
    # 提取基础信息
    model_type = thuTL_config.get("model_type", "unknown")
    input_length = thuTL_config.get("input_token_len", 96)
    output_length = thuTL_config.get("output_token_lens", [96])[0]

    # 转换为AINode格式
    ainode_config = {
        "configs": {
            "input_shape": [input_length, 1],  # thuTL时序模型输入为单维
            "output_shape": [output_length, 1],  # thuTL时序模型输出为单维
            "input_type": ["float32"],
            "output_type": ["float32"],
        },
        "attributes": {
            ...
            "thuTL_model": True,
            "original_config": thuTL_config,
        },
    }

    logger.debug(f"转换thuTL配置: {model_type} -> AINode格式")
