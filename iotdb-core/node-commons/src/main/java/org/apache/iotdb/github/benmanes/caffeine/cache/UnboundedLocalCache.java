...
import static org.apache.iotdb.github.benmanes.caffeine.cache.LocalLoadingCache.newBulkMappingFunction;
import static org.apache.iotdb.github.benmanes.caffeine.cache.LocalLoadingCache.newMappingFunction;

/**
 * An in-memory cache that has no capabilities for bounding the map. This implementation provides a
 * lightweight wrapper on top of {@link ConcurrentHashMap}.
 *
...
final class UnboundedLocalCache<K, V> implements LocalCache<K, V> {
        ...
        data.computeIfPresent(
            ...
            (K k, V value) -> {
              BiFunction<? super K, ? super V, ? extends V> function =
                  statsAware(
                      remappingFunction,
                      /* recordMiss */ false, /* recordLoad */
                      true, /* recordLoadFailure */
                      true);
  ...
  static final class UnboundedLocalAsyncCache<K, V> implements LocalAsyncCache<K, V>, Serializable {
    ...
    UnboundedLocalAsyncCache(Caffeine<K, V> builder) {
      cache =
          new UnboundedLocalCache<>((Caffeine<K, CompletableFuture<V>>) builder, /* async */ true);
    }
  ...
  static final class UnboundedLocalAsyncLoadingCache<K, V> extends LocalAsyncLoadingCache<K, V>
