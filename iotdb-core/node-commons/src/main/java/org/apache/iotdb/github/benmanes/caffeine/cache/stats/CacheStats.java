 ...
 *             missCount}.
 *       </ul>
 *   <li>When an entry is computed through the {@linkplain Cache#asMap asMap} the {@code
 *       loadSuccessCount} or {@code loadFailureCount} is incremented.
 ...
 *   <li>No stats are modified by non-computing operations invoked on the {@linkplain Cache#asMap
 *       asMap} view of the cache.
 * </ul>
 *
 * <p>A lookup is specifically defined as an invocation of one of the methods {@link
 * LoadingCache#get(Object)}, {@link Cache#get(Object, java.util.function.Function)}, or {@link
 * LoadingCache#getAll(Iterable)}.
 *
 * <p>This is a <em>value-based</em> class; use of identity-sensitive operations (including
 * reference equality ({@code ==}), identity hash code, or synchronization) on instances of {@code
 * CacheStats} may have unpredictable results and should be avoided.
 *
...
public final class CacheStats {
   ...
   * Constructs a new {@code CacheStats} instance.
   *
   * <p>Many parameters of the same type in a row is a bad thing, but this class is not constructed
   ...
   * loadSuccessCount + loadFailureCount}. Multiple concurrent misses for the same key will result
   ...
   * #loadFailureCount}). Multiple concurrent misses for the same key will result in a single load
   * operation.
   *
   ...
   * loadFailureCount / (loadSuccessCount + loadFailureCount)}, or {@code 0.0} when {@code
  ...
  /**
   * Returns the sum of weights of evicted entries. This total does not include manual {@linkplain
