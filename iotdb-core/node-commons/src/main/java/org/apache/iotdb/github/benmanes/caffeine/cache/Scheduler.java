...
public interface Scheduler {
   ...
   * {@link #disabledScheduler()} if not present. This scheduler is provided in Java 9 or above by
   * using {@link CompletableFuture} {@code delayedExecutor}.
   *
  ...
  /**
   * Returns a scheduler that suppresses and logs any exception thrown by the delegate {@code
   * scheduler}.
   *
...
enum SystemScheduler implements Scheduler {
  ...
  public Future<?> schedule(Executor executor, Runnable command, long delay, TimeUnit unit) {
    ...
    try {
      Executor scheduler =
          (Executor) delayedExecutor.invoke(CompletableFuture.class, delay, unit, executor);
