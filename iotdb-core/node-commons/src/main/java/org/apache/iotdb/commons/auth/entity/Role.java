...
public class Role {
  ...
  private Map<String, ObjectPrivilege> objectPrivilegeMap;
  ...
  public Map<String, ObjectPrivilege> getObjectPrivilegeMap() {
  ...
  public ObjectPrivilege getObjectPrivilege(String objectName) {
    return this.objectPrivilegeMap.computeIfAbsent(
        objectName, k -> new ObjectPrivilege(objectName));
  }
  ...
  public Set<TDBPrivilege> getRelationalPrivilegeInfo() {
    ...
    for (ObjectPrivilege objectPrivilege : objectPrivilegeMap.values()) {
      TDBPrivilege tdbPrivilege = new TDBPrivilege();
      tdbPrivilege.setDatabasename(objectPrivilege.getDatabaseName());
      tdbPrivilege.setPrivileges(objectPrivilege.getPrivilegeSet());
      tdbPrivilege.setGrantOpt(objectPrivilege.getPrivilegeGrantOptSet());
      if (!objectPrivilege.getTablePrivilegeMap().isEmpty()) {
        for (TablePrivilege tablePrivilege : objectPrivilege.getTablePrivilegeMap().values()) {
  ...
  public void loadRelationalPrivileInfo(Map<String, TDBPrivilege> info) {
    for (TDBPrivilege tdbPrivilege : info.values()) {
      ObjectPrivilege objectPrivilege = new ObjectPrivilege(tdbPrivilege.getDatabasename());
      for (Integer privId : tdbPrivilege.getPrivileges()) {
        objectPrivilege.grantDBObjectPrivilege(PrivilegeType.values()[privId]);
      }
      for (Integer privId : tdbPrivilege.getGrantOpt()) {
        objectPrivilege.grantDBGrantOption(PrivilegeType.values()[privId]);
      }
      if (tdbPrivilege.getTableinfoSize() != 0) {
        for (TTablePrivilege tablePrivilege : tdbPrivilege.getTableinfo().values()) {
          for (Integer privId : tablePrivilege.getPrivileges()) {
            objectPrivilege.grantTableObjectPrivilege(
                tablePrivilege.getTablename(), PrivilegeType.values()[privId]);
          }
          for (Integer privId : tablePrivilege.getGrantOption()) {
            objectPrivilege.grantTableObejctGrantOption(
                tablePrivilege.getTablename(), PrivilegeType.values()[privId]);
          }
        }
      }
      this.objectPrivilegeMap.put(tdbPrivilege.getDatabasename(), objectPrivilege);
    }
  }
  ...
  public void setObjectPrivilegeMap(Map<String, ObjectPrivilege> privilegeMap) {
  ...
  private ObjectPrivilege getObjectPrivilegeInternal(String dbName) {
    return objectPrivilegeMap.computeIfAbsent(dbName, ObjectPrivilege::new);
  }

  public void grantDBPrivilege(String dbName, PrivilegeType priv, boolean grantOption) {
    ObjectPrivilege objectPrivilege = getObjectPrivilege(dbName);
    objectPrivilege.grantDBObjectPrivilege(priv);
    if (grantOption) {
      objectPrivilege.grantDBGrantOption(priv);
    }
  }
  ...
  public void grantTBPrivilege(
    ...
    ObjectPrivilege objectPrivilege = getObjectPrivilege(dbName);
    objectPrivilege.grantTableObjectPrivilege(tbName, priv);
    if (grantOption) {
      objectPrivilege.grantTableObejctGrantOption(tbName, priv);
    }
  }

  public void revokeDBPrivilege(String dbName, PrivilegeType priv) {
    ObjectPrivilege objectPrivilege = getObjectPrivilegeInternal(dbName);
    objectPrivilege.revokeDBObjectPrivilege(priv);
    objectPrivilege.revokeGrantOptionFromDB(priv);
  }

  public void revokeTBPrivilege(String dbName, String tbName, PrivilegeType priv) {
    ObjectPrivilege objectPrivilege = getObjectPrivilegeInternal(dbName);
    objectPrivilege.revokeTableObjectGrantOption(tbName, priv);
    objectPrivilege.revokeTableObjectPrivilege(tbName, priv);
    if (objectPrivilege.getTablePrivilegeMap().isEmpty()) {
  ...
  public boolean checkObjectPrivilegeGrantOpt(String dbname, String table, PrivilegeType priv) {
    ObjectPrivilege privilege = this.objectPrivilegeMap.get(dbname);
    if (privilege == null) {
      return false;
    }
  ...
  public ByteBuffer serialize() {
    ...
    try {
      ...
      for (Map.Entry<String, ObjectPrivilege> item : objectPrivilegeMap.entrySet()) {
  ...
  public void deserialize(ByteBuffer buffer) {
    ...
    for (int i = 0; i < objectPrivilegesSize; i++) {
      ...
      this.objectPrivilegeMap.put(objectName, objectPrivilege);
    }
  }
