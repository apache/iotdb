...
public class MatchState {
    ...
    public Boolean calcOneSectionMatchValue(Section section, double smoothValue, double threshold) {
        ...
        double localWidthRadio = section.getWidthBound() / (patternSectionNow.getWidthBound()*globalWitdhRadio);

        // smooth value is used to avoid the zero division
        double localHeightUp = Math.max(section.getHeightBound(),smoothValue);
        double localHeightDown = Math.max(patternSectionNow.getHeightBound() * globalHeightRadio, smoothValue);
        ...
        double alignHeightDiff = patternSectionNow.getPoints().get(0).y * globalHeightRadio * localHeightRadio - section.getPoints().get(0).y;
        ...
        for(int i=1; i < dataPointNum; i++){
            ...
            shapeError += pointHeight * globalHeightRadio * localHeightRadio - alignHeightDiff - section.getPoints().get(i).y;
        }
    ...
    private double getPointHeight(int i, double numRadio, int patternPointNum) {
        ...
        return patternSectionNow.getPoints().get(leftIndex).y * rightRadio + patternSectionNow.getPoints().get(rightIndex).y * leftRadio;
    }
