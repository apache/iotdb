...
public class PatternSegment {
    ...
    private void closeLastSection(double smoothValue){
        ...
        if(sections.size() >= 2 && sections.get(sections.size() - 2).getSign() == 0){
            if(sections.get(sections.size() - 1).getSign() == 0){
                List<Section> concatResult = sections.get(sections.size() - 2).concat(sections.get(sections.size() - 1),smoothValue);

                sections.remove(sections.size() - 1);
                sections.remove(sections.size() - 1);
                if(concatResult.size() == 3){
                    if(!sections.isEmpty() && concatResult.get(0).getPoints().size() <= lineSectionTolerance){
                    ...
                    if(!sections.isEmpty() && concatResult.get(0).getSign() == 0 && concatResult.get(0).getPoints().size() <= lineSectionTolerance){
                        sections.get(sections.size() - 1).combine(concatResult.get(0));
                        if(sections.get(sections.size() - 1).getSign() == concatResult.get(1).getSign()){
                            sections.get(sections.size() - 1).combine(concatResult.get(1));
                        }
                ...
                if(sections.size() >= 3 && sections.get(sections.size() - 2).getPoints().size() <= lineSectionTolerance){
                    if(sections.get(sections.size() - 3).getSign() == sections.get(sections.size() - 1).getSign()){
    ...
    public void trans2SectionList(String Type,double smoothValue){
        ...
        // the tangent is calc by the point before the current point, so the first point is not included, and need to add it into the section
    ...
    private void concat(PatternSegment patternSegment) {
        ...
        if(lastSection.getSign() != firstSection.getSign()){
            ...
            for(int i = 1; i < section.getPoints().size(); i++){
                section.addPoint(new Point(section.getPoints().get(i).x, section.getPoints().get(i).y + upHeight));
            }

            // connect the last 2 section to sectionNow to first 2 section
            patternSegment.getSections().get(patternSegment.getSections().size() - 2).getNextSectionList().add(0,section);
