...
public interface LoadingCache<K, V> extends Cache<K, V> {
   ...
   *
   * <p>If another call to {@link #get} is currently loading the value for the {@code key}, this
   * thread simply waits for that thread to finish and returns its loaded value. Note that multiple
   ...
   * and enters it into this cache unless {@code null}. The entire method invocation is performed
   ...
   * @throws CompletionException if a checked exception was thrown while loading the value
   * @throws RuntimeException or Error if the {@link CacheLoader} does so, if {@link
   *     CacheLoader#loadAll} returns {@code null}, or returns a map containing null keys or values.
   *     In all cases, the mapping is left unestablished.
   */
