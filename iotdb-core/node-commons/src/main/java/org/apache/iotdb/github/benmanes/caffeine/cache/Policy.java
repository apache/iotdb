 ...
 * characteristics. These operations are optional and dependent on how the cache was constructed and
 * what abilities the implementation exposes.
 *
...
public interface Policy<K, V> {
   ...
   */
  default V getIfPresentQuietly(Object key) {
   ...
   * policy. If the cache was not constructed with a size-based bound or the implementation does not
   ...
   *
   * <p>If the cache was not constructed with access-based expiration or the implementation does not
   * support these operations, an empty {@link Optional} is returned.
   *
   ...
   *
   * <p>If the cache was not constructed with write-based expiration or the implementation does not
   * support these operations, an empty {@link Optional} is returned.
   *
   ...
   *
   * <p>If the cache was not constructed with variable expiration or the implementation does not
   * support these operations, an empty {@link Optional} is returned.
   *
   ...
   *
   * <p>If the cache was not constructed with write-based refresh or the implementation does not
   * support these operations, an empty {@link Optional} is returned.
   *
  ...
  interface Eviction<K, V> {
     ...
     *
     * <p>Note that some implementations may have an internal inherent bound on the maximum total
     * size. If the value specified exceeds that bound, then the value is set to the internal
     * maximum.
     *
     ...
     *
     * <p>Beware that obtaining the mappings is <em>NOT</em> a constant-time operation. Because of
     * the asynchronous nature of the page replacement policy, determining the retention ordering
  ...
  interface Expiration<K, V> { // To be renamed FixedExpiration in version 3.0.0
     ...
     *
     * <p>An expiration policy uses the age to determine if an entry is fresh or stale by comparing
     * it to the freshness lifetime. This is calculated as {@code fresh = freshnessLifetime > age}
     * where {@code freshnessLifetime = expires - currentTime}.
     *
     * <p>This method is scheduled for removal in version 3.0.0.
     *
     * @param key the key for the entry being queried
