...
public interface AsyncLoadingCache<K, V> extends AsyncCache<K, V> {
  ...
  /**
   * Returns the future associated with {@code key} in this cache, obtaining that value from {@link
   * AsyncCacheLoader#asyncLoad} if necessary. If the asynchronous computation fails, the entry will
   ...
   *
   * <p>Caches loaded by a {@link AsyncCacheLoader} supporting bulk loading will issue a single
   * request to {@link AsyncCacheLoader#asyncLoadAll} for all keys which are not already present in
   ...
   * @throws RuntimeException or Error if the {@link AsyncCacheLoader} does so, if {@link
   *     AsyncCacheLoader#asyncLoadAll} returns {@code null}, or fails when constructing the future,
