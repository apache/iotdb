 ...
 * #get(Object, Function)} or {@link #put(Object, Object)}, and are stored in the cache until either
...
public interface Cache<K, V> {
   ...
   *
   * <p><b>Warning:</b> as with {@link CacheLoader#load}, {@code mappingFunction} <b>must not</b>
   ...
   *
   * <p>A single request to the {@code mappingFunction} is performed for all keys which are not
   * already present in the cache. All entries returned by {@code mappingFunction} will be stored in
   * the cache, over-writing any previously cached values. If another call to {@link #get} tries to
   * load the value for a key in {@code keys}, implementations may either have that thread load the
   * entry or simply wait for this thread to finish and return the loaded value. In the case of
  ...
  default Map<K, V> getAll(
      Iterable<? extends K> keys, Function<Iterable<? extends K>, Map<K, V>> mappingFunction) {
   ...
   *
   * <p>Due to the performance penalty of maintaining statistics, some implementations may not
   ...
   *
   * <p>A computation operation, such as {@link ConcurrentMap#compute}, performs the entire method
