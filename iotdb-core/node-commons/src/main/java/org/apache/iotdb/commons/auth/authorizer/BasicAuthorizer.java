...
public abstract class BasicAuthorizer implements IAuthorizer, IService {
  ...
  public void grantPrivilegeToUser(String username, PrivilegeType priv, Boolean grantOpt)
    ...
    userManager.grantPrivilegeToEntry(username, new PrivilegeUnin(priv, grantOpt));
  }
  ...
  public void grantPrivilegeToUser(
      ...
      throws AuthException {
    checkAdmin(userName, "Invalid operation, administrator already has all privileges");
    userManager.grantPrivilegeToEntry(userName, new PrivilegeUnin(path, priv, grantOpt));
  }
  ...
  public void grantPrivilegeToUser(
    ...
    checkAdmin(userName, "Invalid operation, administrator already has all privileges");
    userManager.grantPrivilegeToEntry(userName, new PrivilegeUnin(database, priv, grantOpt));
  }
  ...
  public void grantPrivilegeToUser(
      ...
      throws AuthException {
    checkAdmin(userName, "Invalid operation, administrator already has all privileges");
    userManager.grantPrivilegeToEntry(userName, new PrivilegeUnin(database, table, priv, grantOpt));
  }
  ...
  public void revokePrivilegeFromUser(String username, PrivilegeType priv) throws AuthException {
    ...
    if (!userManager.revokePrivilegeFromEntry(username, new PrivilegeUnin(priv))) {
      throw new AuthException(
  ...
  public void revokePrivilegeFromUser(String username, PartialPath path, PrivilegeType priv)
      throws AuthException {
    checkAdmin(username, "Invalid operation, administrator must have all privileges");
    if (!userManager.revokePrivilegeFromEntry(username, new PrivilegeUnin(path, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
  ...
  public void revokePrivilegeFromUser(String username, String database, PrivilegeType priv)
      throws AuthException {
    checkAdmin(username, "Invalid operation, administrator must have all privileges");
    if (!userManager.revokePrivilegeFromEntry(username, new PrivilegeUnin(database, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
          String.format("User %s does not have %s", username, priv));
    }
  ...
  public void revokePrivilegeFromUser(
    ...
    checkAdmin(username, "Invalid operation, administrator must have all privileges");
    if (!userManager.revokePrivilegeFromEntry(username, new PrivilegeUnin(database, table, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
          String.format("User %s does not have %s", username, priv));
    }
  ...
  public void grantPrivilegeToRole(String rolename, PrivilegeType priv, Boolean grantOpt)
      throws AuthException {
    roleManager.grantPrivilegeToEntry(rolename, new PrivilegeUnin(priv, grantOpt));
  }
  ...
  public void grantPrivilegeToRole(
      ...
      throws AuthException {
    roleManager.grantPrivilegeToEntry(roleName, new PrivilegeUnin(path, priv, grantOpt));
  }

  @Override
  public void grantPrivilegeToRole(
      String roleName, String database, PrivilegeType priv, Boolean grantOpt) throws AuthException {
    roleManager.grantPrivilegeToEntry(roleName, new PrivilegeUnin(database, priv, grantOpt));
  }
  ...
  public void grantPrivilegeToRole(
      ...
      throws AuthException {
    roleManager.grantPrivilegeToEntry(roleName, new PrivilegeUnin(database, table, priv, grantOpt));
  }

  @Override
  public void revokePrivilegeFromRole(String roleName, PrivilegeType priv) throws AuthException {
    if (!roleManager.revokePrivilegeFromEntry(roleName, new PrivilegeUnin(priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
  ...
  public void revokePrivilegeFromRole(String roleName, PartialPath path, PrivilegeType priv)
      throws AuthException {
    if (!roleManager.revokePrivilegeFromEntry(roleName, new PrivilegeUnin(path, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
          String.format("Role %s does not have %s", roleName, priv));
    }
  ...
  public void revokePrivilegeFromRole(String roleName, String database, PrivilegeType priv)
      throws AuthException {
    if (!roleManager.revokePrivilegeFromEntry(roleName, new PrivilegeUnin(database, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
          String.format("Role %s does not have %s", roleName, priv));
    }
  ...
  public void revokePrivilegeFromRole(
    ...
    if (!roleManager.revokePrivilegeFromEntry(roleName, new PrivilegeUnin(database, table, priv))) {
      throw new AuthException(
          TSStatusCode.NOT_HAS_PRIVILEGE,
          String.format("Role %s does not have %s", roleName, priv));
    }
