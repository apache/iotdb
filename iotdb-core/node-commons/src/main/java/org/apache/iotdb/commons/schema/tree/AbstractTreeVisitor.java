...
public abstract class AbstractTreeVisitor<N extends ITreeNode, R> implements SchemaIterator<R> {
  ...
  /**
   * Get the number of child keys that can be iterated directly. Return {@link Integer#MAX_VALUE}
  ...
  /**
   * Get current children iterator generated by {@link AbstractTreeVisitor#getChildrenIterator}
   *
  ...
  private class PreciseMatchChildrenIterator extends AbstractChildrenIterator {
    ...
    private PreciseMatchChildrenIterator(N parent, IStateMatchInfo stateMatchInfo) {
      ...
      // adaptive without relying on unstable wall-clock thresholds.
    ...
    protected void getNext() throws Exception {
      if (iterateChildren) {
        ...
        while (childrenIterator.hasNext()) {
          ...
          IFATransition transition = preciseMatchTransitionMap.get(child.getName());
    ...
    private boolean trySaveResult(final N child, final IFATransition transition) {
