...
import org.apache.iotdb.github.benmanes.caffeine.cache.LocalAsyncCache.AsyncBulkCompleter.NullMapCompletionException;
...
interface LocalAsyncCache<K, V> extends AsyncCache<K, V> {
  ...
  default CompletableFuture<V> get(
      K key, BiFunction<? super K, Executor, CompletableFuture<V>> mappingFunction) {
   ...
   * combined mapping if successful. If any future fails then it is automatically removed from the
   * cache if still present.
   */
  ...
  default void handleCompletion(
      K key, CompletableFuture<V> valueFuture, long startTime, boolean recordMiss) {
    AtomicBoolean completed = new AtomicBoolean();
    valueFuture.whenComplete(
        (value, error) -> {
  ...
  final class AsyncAsMapView<K, V> implements ConcurrentMap<K, CompletableFuture<V>> {
    ...
    public CompletableFuture<V> computeIfAbsent(
        K key, Function<? super K, ? extends CompletableFuture<V>> mappingFunction) {
      requireNonNull(mappingFunction);
        ...
        CompletableFuture<V> computed =
            delegate.compute(
                ...
                (k, valueFuture) -> {
                  ...
                  V newValue =
                      delegate.statsAware(mappingFunction, /* recordLoad */ true).apply(key);
                  if (newValue == null) {
                    return null;
                  }
