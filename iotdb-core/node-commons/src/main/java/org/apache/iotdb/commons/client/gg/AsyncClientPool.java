...
import org.apache.iotdb.commons.client.async.Client;
...
public abstract class AsyncClientPool<Client, RequestType, Location> {

  private static final Logger LOGGER = LoggerFactory.getLogger(AsyncClientPool.class);

  private final IClientManager<TEndPoint, Client> clientManager;

  private static final int MAX_RETRY_NUM = 6;

  private AsyncClientPool() {
    clientManager =
        new IClientManager.Factory<TEndPoint, Client>()
            .createClientManager(getClientPoolFactory());
  }

  protected abstract IClientPoolFactory<TEndPoint, Client> getClientPoolFactory();
  ...
  public void sendAsyncRequestToConfigNodeWithRetry(
      AsyncRequestContext clientHandler) {
    sendAsyncRequest(clientHandler, MAX_RETRY_NUM, null);
  }

  public void sendAsyncRequestToConfigNode(AsyncRequestContext clientHandler) {
    sendAsyncRequest(clientHandler, 1, null);
  }

  private void sendAsyncRequest(
      AsyncRequestContext<?,?,RequestType,?,?> clientHandler, int retryNum, Long timeoutInMs) {
    if (clientHandler.getRequestIndices().isEmpty()) {
      return;
    }

    RequestType requestType = clientHandler.getRequestType();
    for (int retry = 0; retry < retryNum; retry++) {
      ...
      for (int requestId : clientHandler.getRequestIndices()) {
        Location targetConfigNode = clientHandler.getNodeLocation(requestId);
  ...
  public Client getAsyncClient(TConfigNodeLocation targetConfigNode)
