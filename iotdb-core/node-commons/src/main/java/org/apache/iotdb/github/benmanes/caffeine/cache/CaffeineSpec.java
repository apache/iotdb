...
import org.apache.iotdb.github.benmanes.caffeine.cache.Caffeine.Strength;
 ...
 *
 * <p>{@code CaffeineSpec} supports parsing configuration off of a string, which makes it especially
 ...
 * to a {@code Caffeine} builder method.
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Durations are represented as either an ISO-8601 string using {@link
 * Duration#parse(CharSequence)} or by an integer followed by one of "d", "h", "m", or "s",
 * representing days, hours, minutes, or seconds respectively. There is currently no short syntax to
 * request durations in milliseconds, microseconds, or nanoseconds.
 *
 ...
 * </ul>
 *
 * <p>{@code CaffeineSpec} does not support configuring {@code Caffeine} methods with non-value
 * parameters. These must be configured in code.
 *
 * <p>A new {@code Caffeine} builder can be instantiated from a {@code CaffeineSpec} using {@link
 * Caffeine#from(CaffeineSpec)} or {@link Caffeine#from(String)}.
 *
...
public final class CaffeineSpec {
  ...
  void parseOption(String option) {
    ...
    requireArgument(
        keyAndValue.length <= 2, "key-value pair %s with more than one equals sign", option);
  ...
  void initialCapacity(String key, String value) {
    requireArgument(
        initialCapacity == UNSET_INT, "initial capacity was already set to %,d", initialCapacity);
  ...
  void maximumSize(String key, String value) {
    ...
    requireArgument(
        maximumWeight == UNSET_INT, "maximum weight was already set to %,d", maximumWeight);
  ...
  static Duration parseDuration(String key, String value) {
    ...
    if (isIsoFormat) {
      ...
      requireArgument(
          !duration.isNegative(),
          "key %s invalid format; was %s, but the duration cannot be negative",
          key,
          value);
      return duration;
    }
