 ...
 *
 * <p>Most <tt>LinkedDeque</tt> operations run in constant time by assuming that the element
 ...
 *
 * <p>An element can exist in only one instance of a deque implementation, but may exist in multiple
...
interface LinkedDeque<E> extends Deque<E> {
  ...
  interface PeekingIterator<E> extends Iterator<E> {
    ...
    static <E> PeekingIterator<E> comparing(
        PeekingIterator<E> first, PeekingIterator<E> second, Comparator<E> comparator) {
      return new PeekingIterator<E>() {
        @Override
        public boolean hasNext() {
          return first.hasNext() || second.hasNext();
