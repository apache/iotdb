 ...
 *
 * <p>A {@code SingleConsumerQueue} is an appropriate choice when many producer threads will share
 ...
 *
 * <p>This implementation employs combination to transfer elements between threads that are
 * producing concurrently. This approach avoids contention on the queue by combining colliding
 * operations that have identical semantics. When a pair of producers collide, the task of
 * performing the combined set of operations is delegated to one of the threads and the other thread
 * optionally waits for its operation to be completed. This decision of whether to wait for
 * completion is determined by constructing either a <em>linearizable</em> or <em>optimistic</em>
 * queue.
 *
 * <p>Iterators are <i>weakly consistent</i>, returning elements reflecting the state of the queue
 * at some point at or since the creation of the iterator. They do <em>not</em> throw {@link
 ...
 * read access to the queue. This implementation does <em>not</em> include fail-fast behavior to
...
public final class SingleConsumerQueue<E> extends SCQHeader.HeadAndTailRef<E>
  ...
  static final long PROBE =
      org.apache.iotdb.github.benmanes.caffeine.base.UnsafeAccess.objectFieldOffset(
  ...
  /**
   * Creates a queue with an optimistic backoff strategy. A thread completes its operation without
   * waiting after it successfully hands off the additional element(s) to another producing thread
   * for batch insertion. This optimistic behavior may result in additions not appearing in FIFO
   * order due to the backoff strategy trying to compensate for queue contention.
   *
   * @param <E> the type of elements held in this collection
  ...
  /**
   * Creates a queue with a linearizable backoff strategy. A thread waits for a completion signal if
  ...
  static int index() {
    int probe =
        org.apache.iotdb.github.benmanes.caffeine.base.UnsafeAccess.UNSAFE.getInt(
  ...
  static class Node<E> {
    ...
    Node<E> getNextRelaxed() {
      return (Node<E>)
          org.apache.iotdb.github.benmanes.caffeine.base.UnsafeAccess.UNSAFE.getObject(
              this, NEXT_OFFSET);
    }

    void lazySetNext(Node<E> newNext) {
      org.apache.iotdb.github.benmanes.caffeine.base.UnsafeAccess.UNSAFE.putOrderedObject(
...
final class SCQHeader {
  ...
  abstract static class HeadAndTailRef<E> extends PadHeadAndTail<E> {
    ...
    boolean casTail(SingleConsumerQueue.Node<E> expect, SingleConsumerQueue.Node<E> update) {
      return org.apache.iotdb.github.benmanes.caffeine.base.UnsafeAccess.UNSAFE
