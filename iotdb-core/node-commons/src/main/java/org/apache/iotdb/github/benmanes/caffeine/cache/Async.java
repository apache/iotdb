...
import static org.apache.iotdb.github.benmanes.caffeine.cache.BoundedLocalCache.MAXIMUM_EXPIRY;
...
final class Async {
   ...
   * the expiration timeouts reflect the value once present. This can be done safely using {@link
  ...
  static final class AsyncExpiry<K, V> implements Expiry<K, CompletableFuture<V>>, Serializable {
    ...
    public long expireAfterUpdate(
        K key, CompletableFuture<V> future, long currentTime, long currentDuration) {
      if (isReady(future)) {
        long duration =
            (currentDuration > MAXIMUM_EXPIRY)
