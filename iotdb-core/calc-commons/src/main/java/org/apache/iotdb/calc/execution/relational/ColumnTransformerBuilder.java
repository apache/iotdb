...
import com.timecho.iotdb.calc.plan.relational.function.scalar.binary.ReadObject2ColumnTransformer;
import com.timecho.iotdb.calc.plan.relational.function.scalar.ternary.ReadObject3ColumnTransformer;
...
public class ColumnTransformerBuilder
  ...
  private ColumnTransformer getFunctionColumnTransformer(
    ...
    if (TableBuiltinScalarFunction.DIFF.getFunctionName().equalsIgnoreCase(functionName)) {
      ...
      ColumnTransformer first = this.process(children.get(0), context);
      if (children.size() == 1) {
        return new ReadObjectColumnTransformer(BLOB, first, context.memoryReservationManager);
        ...
        if (isLongLiteral(offset)) {
          return new ReadObjectColumnTransformer(
              ...
              ((LongLiteral) children.get(1)).getParsedValue(),
              first,
              context.memoryReservationManager);
        } else {
          return new ReadObject2ColumnTransformer(
              BLOB, first, this.process(offset, context), context.memoryReservationManager);
        }
      } else {
        if (isLongLiteral(children.get(1)) && isLongLiteral(children.get(2))) {
          ...
          return new ReadObjectColumnTransformer(
              BLOB, offset, length, first, context.memoryReservationManager);
        } else {
