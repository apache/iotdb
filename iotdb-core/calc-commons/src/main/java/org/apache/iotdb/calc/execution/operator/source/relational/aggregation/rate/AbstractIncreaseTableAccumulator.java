...
abstract class AbstractIncreaseTableAccumulator implements TableAccumulator {
  ...
  protected AbstractIncreaseTableAccumulator(TSDataType valueDataType) {
  ...
  protected final double calculateIncrease(
      int sampleCount, long firstTime, double firstValue, long lastTime, double correctedIncrease) {
  ...
  protected final SemanticException duplicateTimestamp(long time) {
    return new SemanticException(
        String.format(
            ...
            RateFunctionType.INCREASE.getFunctionName(),
            time));
  }
