...
public final class NaiveIrateAccumulator extends AbstractIrateTableAccumulator {
  ...
  public TableAccumulator copy() {
    return new NaiveIrateAccumulator(valueDataType, memoryReservationManager);
  }
  ...
  public void evaluateFinal(ColumnBuilder output) {
    samples.sortAndValidate(RateFunctionType.IRATE.getFunctionName());
    if (samples.size() < 2) {
      output.appendNull();
      return;
    }
    ...
    output.writeDouble(
        calculateIrate(
            samples.getTime(lastIndex - 1),
            samples.getValue(lastIndex - 1),
