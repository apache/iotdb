...
public class CeilColumnTransformer extends UnaryColumnTransformer {
  public CeilColumnTransformer(Type returnType, ColumnTransformer childColumnTransformer) {
  ...
  private void transform(Column column, ColumnBuilder columnBuilder, int i) {
    if (TSDataType.DOUBLE.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.ceil(column.getDouble(i)));
