...
public class PlainObjectPath implements IObjectPath {
  ...
  public PlainObjectPath(String filePath) {
    ...
    for (int i = 0; i < deviceIdSegments.length; i++) {
      deviceIdSegments[i] = path.getName(i + 1).toString();
      if ("NUL".equals(deviceIdSegments[i])) {
        deviceIdSegments[i] = null;
      } else if ("EPT".equals(deviceIdSegments[i])) {
        deviceIdSegments[i] = "";
      }
    }
  ...
  public PlainObjectPath(int regionId, long time, IDeviceID deviceID, String measurement) {
    final String objectFileName = time + ".bin";
    final Object[] segments = deviceID.getSegments();
    final StringBuilder relativePathString =
  ...
  public static PlainObjectPath deserialize(ByteBuffer byteBuffer) {
    final String filePath = ReadWriteIOUtils.readString(byteBuffer);
