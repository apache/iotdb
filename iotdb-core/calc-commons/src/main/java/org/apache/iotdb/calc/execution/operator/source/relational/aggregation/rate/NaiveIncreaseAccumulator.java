...
public final class NaiveIncreaseAccumulator extends AbstractIncreaseTableAccumulator {

  private static final long INSTANCE_SIZE =
      RamUsageEstimator.shallowSizeOfInstance(NaiveIncreaseAccumulator.class);

  private final TimeValueBuffer samples = new TimeValueBuffer();
  private final MemoryReservationManager memoryReservationManager;
  private long previousSamplesSize;

  public NaiveIncreaseAccumulator(
  ...
  public TableAccumulator copy() {
    return new NaiveIncreaseAccumulator(valueDataType, memoryReservationManager);
  }
  ...
  public void evaluateFinal(ColumnBuilder output) {
    samples.sortAndValidate(RateFunctionType.INCREASE.getFunctionName());
    if (samples.size() < 2) {
      output.appendNull();
      return;
    }
