...
public class LastByAccumulator implements TableAccumulator {
    ...
    switch (xDataType) {
      ...
      default:
        throw new UnSupportedDataTypeException(
            String.format(
                CalcMessages.EXCEPTION_UNSUPPORTED_DATA_TYPE_LAST_AGGREGATION_ARG_8C41FBCC,
                yDataType));
    }
  }
  ...
  public void addIntermediate(Column argument) {
    ...
    for (int i = 0; i < argument.getPositionCount(); i++) {
      ...
      switch (xDataType) {
        case INT32:
        case DATE:
          // if the x value is null, could not serialize the x value
          int xIntVal = isXValueNull ? 0 : BytesUtils.bytesToInt(bytes, offset);
          if (!isOrderTimeNull) {
            updateIntLastValue(isXValueNull, xIntVal, curTime);
          } else {
            updateIntNullTimeValue(isXValueNull, xIntVal);
          }
          break;
        ...
        case TIMESTAMP:
          ...
          if (!isOrderTimeNull) {
            updateLongLastValue(isXValueNull, longVal, curTime);
          } else {
        ...
        case FLOAT:
          ...
          if (!isOrderTimeNull) {
            updateFloatLastValue(isXValueNull, floatVal, curTime);
          } else {
        ...
        case DOUBLE:
          ...
          if (!isOrderTimeNull) {
            updateDoubleLastValue(isXValueNull, doubleVal, curTime);
          } else {
        ...
        case OBJECT:
          ...
          if (!isOrderTimeNull) {
            updateBinaryLastValue(isXValueNull, binaryVal, curTime);
          } else {
        ...
        case BOOLEAN:
          ...
          if (!isOrderTimeNull) {
            updateBooleanLastValue(isXValueNull, boolVal, curTime);
          } else {
  ...
  public void evaluateIntermediate(ColumnBuilder columnBuilder) {
    ...
    if (initResult || initNullTimeValue) {
      ...
      columnBuilder.writeBinary(
          new Binary(
              serializeTimeValueWithNull(xDataType, yLastTime, xIsNull, isOrderTimeNull, xResult)));
      return;
    }
  ...
  // maybe could add last position optimization if last position is null ?
  ...
  protected void updateIntLastValue(boolean isXValueNull, int xValue, long curTime) {
  ...
  protected void addLongInput(
    ...
    for (int i = 0; i < selectPositionCount; i++) {
      ...
      if (!timeColumn.isNull(position)) {
        ...
        // Case A: The order time is  null. Attempt to update the xNullTimeValue.
        updateLongNullTimeValue(xColumn.isNull(position), xColumn.getLong(position));
      }
    }
  }

  protected void updateLongLastValue(boolean isXValueNull, long xValue, long curTime) {
    if (checkAndUpdateLastTime(isXValueNull, curTime)) {
  ...
  protected void updateFloatLastValue(boolean isXValueNull, float xValue, long curTime) {
    if (checkAndUpdateLastTime(isXValueNull, curTime)) {
  ...
  protected void updateDoubleLastValue(boolean isXValueNull, double xValue, long curTime) {
    if (checkAndUpdateLastTime(isXValueNull, curTime)) {
  ...
  protected void updateBinaryLastValue(boolean isXValueNull, Binary xValue, long curTime) {
    if (checkAndUpdateLastTime(isXValueNull, curTime)) {
  ...
  protected void updateBooleanLastValue(boolean isXValueNull, boolean xValue, long curTime) {
    if (checkAndUpdateLastTime(isXValueNull, curTime)) {
