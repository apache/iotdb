...
public class PipeMemoryWeightUtil {
  ...
  private static Pair<Integer, Integer> calculateTabletRowCountAndMemoryBySize(int rowSize) {
    if (rowSize <= 0) {
      return new Pair<>(1, 0);
    }
    ...
    int rowNumber =
        Math.max(1, (PipeConfig.getInstance().getPipeDataStructureTabletSizeInBytes() / rowSize));
