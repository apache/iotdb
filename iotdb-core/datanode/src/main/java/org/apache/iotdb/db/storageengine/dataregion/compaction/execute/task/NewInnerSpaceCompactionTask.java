...
public class NewInnerSpaceCompactionTask extends AbstractCompactionTask {
  private final InnerCompactionTaskFilesView filesView;
  protected File logFile;
  protected boolean[] isHoldingWriteLock;
  protected AbstractInnerSpaceEstimator innerSpaceEstimator;

  public NewInnerSpaceCompactionTask(
      long timePartition,
  ...
  private static class InnerCompactionTaskFilesView {
    private final List<TsFileResource> sortedAllSourceFilesInTask;
    private final List<TsFileResource> sourceFilesInCompactionPerformer;
    private final List<TsFileResource> skippedSourceFiles;
    private final boolean sequence;
    private List<TsFileResource> sourceFilesInLog;
    private List<TsFileResource> targetFilesInLog;
    private List<TsFileResource> targetFilesInPerformer;
    private List<TsFileResource> renamedTargetFiles;

    private long selectedFileSize;
    private int sumOfCompactionCount;
    private long maxFileVersion;
    private int maxCompactionCount;

    private InnerCompactionTaskFilesView(
    ...
    private InnerCompactionTaskFilesView(List<TsFileResource> selectedFiles, boolean sequence)
        throws DiskSpaceInsufficientException, IOException {
      this(selectedFiles, Collections.emptyList(), sequence);
    }

    private void calculateSourceFilesAndTargetFiles()
        throws IOException, DiskSpaceInsufficientException {
      LinkedList<TsFileResource> availablePositionForTargetFiles = new LinkedList<>();
      for (int i = sourceFilesInCompactionPerformer.size() - 1; i >= 0; i--) {
        TsFileResource resource1 = sourceFilesInCompactionPerformer.get(i);
        TsFileResource resource2 =
            sortedAllSourceFilesInTask.get(
                sortedAllSourceFilesInTask.size() - 1 - availablePositionForTargetFiles.size());
      ...
      boolean needToAdjustSourceFilesPosition = availableSizeForTargetFiles < selectedFileSize;
      this.targetFilesInLog = new ArrayList<>();

      if (needToAdjustSourceFilesPosition) {
        this.sourceFilesInLog = sortedAllSourceFilesInTask;
        // To avoid increase the inner compaction cnt for skipped files, we increase the cross
        // compaction cnt
        calculateRenamedTargetFiles();
      }

      this.targetFilesInPerformer =
          TsFileNameGenerator.getNewInnerCompactionTargetFileResources(
              availablePositionForTargetFiles, sequence);
      this.targetFilesInLog =
          new ArrayList<>(targetFilesInPerformer.size() + renamedTargetFiles.size());
      targetFilesInLog.addAll(renamedTargetFiles);
      targetFilesInLog.addAll(targetFilesInPerformer);
    }

    private void calculateRenamedTargetFiles() throws IOException {
      this.renamedTargetFiles = new ArrayList<>();
      for (TsFileResource resource : skippedSourceFiles) {
        ...
        renamedTargetFiles.add(renamedTargetFile);
      }
    }
  ...
  protected void rollback() throws IOException {
    ...
    if (recoverMemoryStatus) {
      replaceTsFileInMemory(filesView.targetFilesInLog, filesView.sourceFilesInLog);
    }
  ...
  public boolean equalsOtherTask(AbstractCompactionTask otherTask) {
    if (!(otherTask instanceof NewInnerSpaceCompactionTask)) {
      return false;
    }
    NewInnerSpaceCompactionTask task = (NewInnerSpaceCompactionTask) otherTask;
  ...
  public boolean equals(Object other) {
    if (!(other instanceof NewInnerSpaceCompactionTask)) {
      return false;
    }
    return equalsOtherTask((NewInnerSpaceCompactionTask) other);
  }
