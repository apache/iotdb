...
public class TsFileDeduplicationBlockingPendingQueue extends SubscriptionBlockingPendingQueue {
  ...
  private synchronized Event filter(final Event event) { // make it synchronized
    ...
    if (!(event instanceof EnrichedEvent)) {
      LOGGER.info("Subscription: Ignore {}", event);
      return null;
    }

    if (event instanceof PipeTerminateEvent) {
      LOGGER.info("Subscription: Detect PipeTerminateEvent {}, commit it directly", event);
      // commit directly
      ((PipeTerminateEvent) event)
          .decreaseReferenceCount(TsFileDeduplicationBlockingPendingQueue.class.getName(), true);
      return null;
    }
