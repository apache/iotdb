...
public class PruneWindowColumns extends ProjectOffPushDownRule<WindowNode> {
  ...
  protected Optional<PlanNode> pushDownProjectOff(Context context, WindowNode windowNode, Set<Symbol> referencedOutputs) {
    ...
    ImmutableSet.Builder<Symbol> referencedInputs = ImmutableSet.<Symbol>builder()
        .addAll(windowNode.getChild().getOutputSymbols().stream()
            .filter(referencedOutputs::contains)
            .iterator())
        .addAll(windowNode.getSpecification().getPartitionBy());

    windowNode.getSpecification().getOrderingScheme().ifPresent(
    ...
    PlanNode prunedWindowNode = new WindowNode(
        ...
        restrictOutputs(context.getIdAllocator(), windowNode.getChild(), referencedInputs.build())
