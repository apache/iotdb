...
public class FloorColumnTransformer extends UnaryColumnTransformer {
  public FloorColumnTransformer(Type returnType, ColumnTransformer childColumnTransformer) {
  ...
  private void transform(Column column, ColumnBuilder columnBuilder, int i) {
    if (TSDataType.DOUBLE.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.floor(column.getDouble(i)));
    } else if (TSDataType.FLOAT.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.floor(column.getFloat(i)));
