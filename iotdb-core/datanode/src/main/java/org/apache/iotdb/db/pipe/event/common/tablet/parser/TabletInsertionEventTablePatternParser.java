...
/** TODO: simplify the code by reusing the code in {@link TabletInsertionEventTreePatternParser}. */
public class TabletInsertionEventTablePatternParser implements TabletInsertionEventParser {
  ...
  private Tablet.ColumnType[] valueColumnTypes;
  private TSDataType[] valueColumnDataTypes;
  ...
  public TabletInsertionEventTablePatternParser(
    ...
    if (insertNode instanceof RelationalInsertRowNode) {
      parse((RelationalInsertRowNode) insertNode, pattern);
    } else if (insertNode instanceof RelationalInsertTabletNode) {
      parse((RelationalInsertTabletNode) insertNode, pattern);
    } else {
  ...
  private void parse(final RelationalInsertRowNode insertRowNode, final TablePattern pattern) {
    ...
    this.tableName = insertRowNode.getTargetPath().getFullPath();
  ...
  private void parse(
      final RelationalInsertTabletNode insertTabletNode, final TablePattern pattern) {
    final int originColumnSize = insertTabletNode.getMeasurements().length;
    final Integer[] originColumnIndex2FilteredColumnIndexMapperList = new Integer[originColumnSize];

    // The full path is always cached when device path is deserialized
    this.tableName = insertTabletNode.getTargetPath().getFullPath();
  ...
  private void parse(final Tablet tablet, final boolean isAligned, final TablePattern pattern) {
    final int originColumnSize = tablet.getSchemas().size();
    final Integer[] originColumnIndex2FilteredColumnIndexMapperList = new Integer[originColumnSize];

    // Only support tree-model tablet
    this.tableName = tablet.getDeviceId();
  ...
  private String getTableName() {
    return Objects.nonNull(tableName) ? tableName : deviceId.getTableName();
  }
}
