...
public final class ColumnStatistics {
  private static final ColumnStatistics EMPTY =
      new ColumnStatistics(
          Estimate.unknown(), Estimate.unknown(), Estimate.unknown(), Optional.empty());
  ...
  public ColumnStatistics(
    ...
    if (!nullsFraction.isUnknown()) {
      if (nullsFraction.getValue() < 0 || nullsFraction.getValue() > 1) {
        throw new IllegalArgumentException(
            format("nullsFraction must be between 0 and 1: %s", nullsFraction.getValue()));
      }
    }
    ...
    if (!dataSize.isUnknown() && dataSize.getValue() < 0) {
      throw new IllegalArgumentException(
          format("dataSize must be greater than or equal to 0: %s", dataSize.getValue()));
    }
  ...
  /**
   * If one of the estimates below is unspecified (i.e. left as "unknown"), the optimizer may not be
   * able to derive the statistics needed for performing cost-based query plan optimizations.
   */
