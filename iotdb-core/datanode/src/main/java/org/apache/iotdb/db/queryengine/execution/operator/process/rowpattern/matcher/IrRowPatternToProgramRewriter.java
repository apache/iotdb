...
import org.apache.iotdb.db.node_commons.plan.relational.planner.rowpattern.rowpattern.IrRowPatternVisitor;
...
public class IrRowPatternToProgramRewriter {
  ...
  private static class Rewriter extends IrRowPatternVisitor<Void, Void> {
    ...
    protected Void visitIrPermutation(IrPermutation node, Void context) {
      ...
      List<List<IrRowPattern>> permutations =
          orderedPermutations(indexes).stream()
              .map(
                  permutation ->
                      permutation.stream().map(node.getPatterns()::get).collect(toImmutableList()))
    ...
    private void rangeQuantified(IrRowPattern pattern, boolean greedy, int min, int max) {
      ...
      for (int i = 0; i < splitPositions.size(); i++) {
        if (greedy) {
          instructions.set(
              splitPositions.get(i), new Split(splitTargets.get(i), instructions.size()));
        } else {
          instructions.set(
              splitPositions.get(i), new Split(instructions.size(), splitTargets.get(i)));
        }
      }
    }
  }
