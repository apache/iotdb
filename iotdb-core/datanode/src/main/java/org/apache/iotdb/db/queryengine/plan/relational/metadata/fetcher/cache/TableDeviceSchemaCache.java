...
public class TableDeviceSchemaCache {
   ...
   * if it is known to be {@code null} at a concrete last-row time, preserve that time and use
   * {@link TableDeviceLastCache#EMPTY_PRIMITIVE_TYPE} as the value. For time column, the input
   ...
   *     TableDeviceLastCache#EMPTY_PRIMITIVE_TYPE} iff cache hit and the measurement is known to be
   *     {@code null} when last by the source measurement's time, {@link
   *     TableDeviceLastCache#STALE_PRIMITIVE_TYPE} iff cache hit but the target measurement is
