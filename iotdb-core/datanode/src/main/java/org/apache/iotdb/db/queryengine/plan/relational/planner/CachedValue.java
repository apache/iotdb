...
public class CachedValue {
  ...
  private static class PlanNodeCloner extends PlanVisitor<PlanNode, ClonerContext> {
    ...
    @Override
    public PlanNode visitExplainAnalyze(ExplainAnalyzeNode node, ClonerContext context) {
      PlanNode newChild = node.getChild().accept(this, context);
  ...
  public long estimateMemoryUsage() {
    ...
    if (planNode != null) {
      size += RamUsageEstimator.sizeOfObject(planNode);
    }
      ...
      for (List<Expression> list : metadataExpressionLists) {
        if (list != null) {
          for (Expression e : list) {
            if (e != null) {
              size += RamUsageEstimator.sizeOfObject(e);
            }
          }
        }
      }
  ...
  public static class ClonerContext {
    ...
    private final long localQueryId;
    private final List<Literal> newLiterals;

    public ClonerContext(QueryId queryId, long localQueryId, List<Literal> newLiterals) {
