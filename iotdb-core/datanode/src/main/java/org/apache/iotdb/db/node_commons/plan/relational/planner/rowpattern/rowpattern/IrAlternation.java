...
public class IrAlternation extends IrRowPattern {
  ...
  public IrAlternation(List<IrRowPattern> patterns) {
    ...
    checkArgument(
        patterns.size() >= 2,
        "pattern alternation must have at least 2 elements (actual: %s)",
        patterns.size());
  }
  ...
  public static void serialize(IrAlternation pattern, ByteBuffer byteBuffer) {
  ...
  public static void serialize(IrAlternation pattern, DataOutputStream stream) throws IOException {
  ...
  public static IrAlternation deserialize(ByteBuffer byteBuffer) {
