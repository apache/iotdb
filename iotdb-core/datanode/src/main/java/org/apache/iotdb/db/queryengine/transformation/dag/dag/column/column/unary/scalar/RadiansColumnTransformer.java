...
public class RadiansColumnTransformer extends UnaryColumnTransformer {
  public RadiansColumnTransformer(Type returnType, ColumnTransformer childColumnTransformer) {
  ...
  private void transform(Column column, ColumnBuilder columnBuilder, int i) {
    if (TSDataType.DOUBLE.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.toRadians(column.getDouble(i)));
