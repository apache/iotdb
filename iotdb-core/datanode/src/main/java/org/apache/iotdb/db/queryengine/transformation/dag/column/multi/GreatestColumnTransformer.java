...
public class GreatestColumnTransformer extends AbstractGreatestLeastColumnTransformer {
  ...
  protected void transformLong(ColumnBuilder builder, List<Long> values) {
    returnType.writeLong(builder, values.stream().max(Long::compareTo).orElse(0L));
  }

  @Override
  protected void transformFloat(ColumnBuilder builder, List<Float> values) {
    returnType.writeFloat(builder, values.stream().max(Float::compareTo).orElse(0f));
  }

  @Override
  protected void transformDouble(ColumnBuilder builder, List<Double> values) {
    returnType.writeDouble(builder, values.stream().max(Double::compareTo).orElse(0.0));
  }

  @Override
  protected void transformBinary(ColumnBuilder builder, List<Binary> values) {
    returnType.writeBinary(
        builder, values.stream().max(Binary::compareTo).orElse(BytesUtils.valueOf("")));
  }
}
