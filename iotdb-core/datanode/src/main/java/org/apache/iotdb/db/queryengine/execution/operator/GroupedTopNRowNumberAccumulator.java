...
public class GroupedTopNRowNumberAccumulator {
  ...
  void verifyIntegrity() {
    ...
    for (int groupId = 0; groupId < groupIdToHeapBuffer.getTotalGroups(); groupId++) {
      ...
      verify(
          rootNodeIndex == UNKNOWN_INDEX || calculateRootRowNumber(groupId) <= topN,
  ...
  private IntegrityStats verifyHeapIntegrity(long heapNodeIndex) {
    ...
    if (leftChildHeapIndex != UNKNOWN_INDEX) {
      verify(
          strategy.compare(rowId, heapNodeBuffer.getRowId(leftChildHeapIndex)) >= 0,
    ...
    if (rightChildHeapIndex != UNKNOWN_INDEX) {
      ...
      verify(
          strategy.compare(rowId, heapNodeBuffer.getRowId(rightChildHeapIndex)) >= 0,
          "Max heap invariant violated");
    }
    ...
    verify(
        abs(leftIntegrityStats.getMaxDepth() - rightIntegrityStats.getMaxDepth()) <= 1,
        "Heap not balanced");
