...
public class SqrtColumnTransformer extends UnaryColumnTransformer {
  public SqrtColumnTransformer(Type returnType, ColumnTransformer childColumnTransformer) {
  ...
  private void transform(Column column, ColumnBuilder columnBuilder, int i) {
    if (TSDataType.DOUBLE.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.sqrt(column.getDouble(i)));
