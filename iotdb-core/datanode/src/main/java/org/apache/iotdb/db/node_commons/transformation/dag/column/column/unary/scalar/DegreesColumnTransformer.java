...
package org.apache.iotdb.db.node_commons.transformation.dag.column.column.unary.scalar;
...
public class DegreesColumnTransformer extends UnaryColumnTransformer {
  public DegreesColumnTransformer(Type returnType, ColumnTransformer childColumnTransformer) {
  ...
  private void transform(Column column, ColumnBuilder columnBuilder, int i) {
    if (TSDataType.DOUBLE.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.toDegrees(column.getDouble(i)));
    } else if (TSDataType.FLOAT.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.toDegrees(column.getFloat(i)));
    } else if (TSDataType.INT32.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.toDegrees(column.getInt(i)));
    } else if (TSDataType.INT64.equals(column.getDataType())) {
      columnBuilder.writeDouble(Math.toDegrees((double) column.getLong(i)));
    }
  }
}
