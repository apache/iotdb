...
public class TypeUtil {
  ...
  public static boolean rowNotDistinctFromRow(
      Type[] types, int leftPosition, Column[] leftBlock, int rightPosition, Column[] rightBlock) {
    for (int i = 0; i < types.length; i++) {
      ...
      switch (types[i].getTypeEnum()) {
        case BOOLEAN:
          if (leftBlock[i].getBoolean(leftPosition) != rightBlock[i].getBoolean(rightPosition)) {
            return false;
          }
          break;
        case INT32:
        case DATE:
          if (leftBlock[i].getInt(leftPosition) != rightBlock[i].getInt(rightPosition)) {
            return false;
          }
          break;
        case INT64:
        case TIMESTAMP:
          if (leftBlock[i].getLong(leftPosition) != rightBlock[i].getLong(rightPosition)) {
            return false;
          }
          break;
        case FLOAT:
          if (leftBlock[i].getFloat(leftPosition) != rightBlock[i].getFloat(rightPosition)) {
            return false;
          }
          break;
        case DOUBLE:
          if (leftBlock[i].getDouble(leftPosition) != rightBlock[i].getDouble(rightPosition)) {
            return false;
          }
          break;
        case TEXT:
  ...
  public static void appendTo(Type type, Column input, int position, ColumnBuilder columnBuilder) {
    ...
    switch (type.getTypeEnum()) {
      case BOOLEAN:
        columnBuilder.writeBoolean(input.getBoolean(position));
        break;
      case INT32:
      case DATE:
        columnBuilder.writeInt(input.getInt(position));
        break;
      case INT64:
      case TIMESTAMP:
        columnBuilder.writeLong(input.getLong(position));
        break;
      case FLOAT:
        columnBuilder.writeFloat(input.getFloat(position));
        break;
      case DOUBLE:
        columnBuilder.writeDouble(input.getDouble(position));
        break;
      case TEXT:
      case STRING:
      case BLOB:
        columnBuilder.writeBinary(input.getBinary(position));
        break;
      default:
        throw new UnsupportedOperationException();
    }
