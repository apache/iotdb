...
package org.apache.iotdb.db.pipe.sink.protocol.legacy;
...
public class IoTDBLegacyPipeSink implements PipeConnector {
  ...
  private void openClientSession() throws TException {
    ...
    if (openSessionResp.getStatus().getCode() != TSStatusCode.SUCCESS_STATUS.getStatusCode()) {
      final String errorMsg =
          String.format(
              "Failed to login to receiver %s:%s for legacy pipe transfer because code: %s, message: %s",
              ipAddress,
              port,
  ...
  private void doTransferWrapper(
    ...
    if (!pipeInsertNodeInsertionEvent.increaseReferenceCount(IoTDBLegacyPipeSink.class.getName())) {
      return;
    }
    try {
      ...
      pipeInsertNodeInsertionEvent.decreaseReferenceCount(
          IoTDBLegacyPipeSink.class.getName(), false);
    }
  }
  ...
  private void doTransferWrapper(final PipeTsFileInsertionEvent pipeTsFileInsertionEvent)
    ...
    // We increase the reference count for this event to determine if the event may be released.
    if (!pipeTsFileInsertionEvent.increaseReferenceCount(IoTDBLegacyPipeSink.class.getName())) {
      return;
    }
    try {
