...
public class LongBigArrayFIFOQueue {
  private static final long INSTANCE_SIZE = shallowSizeOfInstance(LongBigArrayFIFOQueue.class);
  ...
  /**
   * The end position in {@link #array}. It is always strictly smaller than {@link #length}. Might
   * be actually smaller than {@link #start} because {@link #array} is used cyclically.
   */
  protected long end;
  ...
  public LongBigArrayFIFOQueue(final long capacity) {
    ...
    length =
        Math.max(INITIAL_CAPACITY, capacity); // Never build a queue smaller than INITIAL_CAPACITY
