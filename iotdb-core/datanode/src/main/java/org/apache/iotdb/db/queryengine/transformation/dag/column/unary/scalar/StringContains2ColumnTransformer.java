...
public class StringContains2ColumnTransformer extends BinaryColumnTransformer{
    public StringContains2ColumnTransformer(Type returnType, ColumnTransformer leftTransformer, ColumnTransformer rightTransformer) {
    ...
    protected void doTransform(Column leftColumn, Column rightColumn, ColumnBuilder columnBuilder, int positionCount) {
        for (int i = 0; i < positionCount; i++) {
            if (!leftColumn.isNull(i) && !rightColumn.isNull(i)) {
                ...
                columnBuilder.writeBoolean(leftValue.contains(rightValue));
            } else {
