...
public class PipeTransferTabletBatchReqPerformanceTest {
  ...
  private static final String ENABLED_PROPERTY = "iotdb.pipe.tablet.batch.deserialize.perf.enabled";
  private static final String COLUMNS_PROPERTY = "iotdb.pipe.tablet.batch.deserialize.perf.columns";
  ...
  public void singleWideRawTabletDeserializationBenchmark() throws IOException {
    ...
    final PipeTransferTabletBatchReq request = createSingleWideRawTabletBatch(columnCount);
    ...
    System.out.printf(
        Locale.ROOT,
        "Batch V1 single raw tablet deserialization benchmark: columns=%d, warmups=%d, iterations=%d, median=%.3f ms/op%n",
        columnCount,
        warmupIterations,
        iterations,
  ...
  private static PipeTransferTabletBatchReq createSingleWideRawTabletBatch(final int columnCount)
      throws IOException {
    ...
    return PipeTransferTabletBatchReq.toTPipeTransferReq(
        Collections.emptyList(), Collections.singletonList(serializeTablet(tablet)));
  }
  ...
  private static PipeTransferTabletBatchReq deserialize(final PipeTransferTabletBatchReq request) {
    request.body.position(0);
    benchmarkBlackhole = PipeTransferTabletBatchReq.fromTPipeTransferReq(request);
