...
public class TsFileEpochManagerTest {
  ...
  public void testMeasurementsAreDeduplicatedPerDevice() {
    final String device = "root.test.d1";
    final String uniqueDevice = "root.test.d2";
    ...
    final PipeInsertNodeTabletInsertionEvent insertionEvent =
        mock(PipeInsertNodeTabletInsertionEvent.class);
    final PipeRealtimeEvent event =
        new TsFileEpochManager()
            .bindPipeInsertNodeTabletInsertionEvent(insertionEvent, rows, resource);
  ...
  private static InsertRowNode mockRow(final String device, final String... measurements) {
    final InsertRowNode row = mock(InsertRowNode.class);
    when(row.getDevicePath()).thenReturn(new PartialPath(device, false));
