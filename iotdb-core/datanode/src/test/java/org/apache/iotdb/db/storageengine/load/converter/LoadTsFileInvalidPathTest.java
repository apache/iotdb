...
public class LoadTsFileInvalidPathTest {
  ...
  public void testAnalysisRejectsNullDeviceNode() throws Exception {
    final IDeviceID device = new StringArrayDeviceID(new String[] {"root.sg", null, "d1"});
  ...
  public void testConversionReportsInvalidPathAndReleasesMemory() throws Exception {
    assertConversionRejectsInvalidPath(new StringArrayDeviceID(new String[] {"root", ""}));
  }
  ...
  private void assertConversionRejectsInvalidPath(final IDeviceID device) throws Exception {
    ...
    Assert.assertEquals(
        new IllegalPathException(new PartialPath(device).getFullPath()).getMessage(),
  ...
  private void assertAnalysisRejectsInvalidPath(
    ...
    final MPPQueryContext context = new MPPQueryContext(new QueryId("load_invalid_path_test"));
    context.setSession(new SessionInfo(0, "root", ZoneId.systemDefault()));

    try (final LoadTsFileAnalyzer analyzer = new LoadTsFileAnalyzer(statement, false, context)) {
  ...
  private File writeTsFile(final IDeviceID device) throws Exception {
    ...
    // Use the low-level writer to preserve legacy empty/null device nodes in the file.
