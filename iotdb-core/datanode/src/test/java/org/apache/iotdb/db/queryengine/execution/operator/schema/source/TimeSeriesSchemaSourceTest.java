...
public class TimeSeriesSchemaSourceTest {
  ...
  public void testCountStatisticIncludesView() throws Exception {
    ...
    Mockito.when(schemaRegionStatistics.getSeriesNumber(true, false)).thenReturn(5L);

    assertEquals(5L, countSource.getSchemaStatistic(schemaRegion));
    Mockito.verify(schemaRegionStatistics).getSeriesNumber(true, false);
    Mockito.verify(schemaRegionStatistics, Mockito.never()).getSeriesNumber(false, false);
  }
