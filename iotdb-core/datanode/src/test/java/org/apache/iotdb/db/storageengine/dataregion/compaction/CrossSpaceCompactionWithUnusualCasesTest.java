...
/**
 * If an unsequence file is generated by a load operation, some devices in it may not overlap with
 * any files in sequence space, these devices' startTime may larger than the latest endTime of it in
 * sequence space. When performing a cross-space compaction, the selected sequence file may not
 * contain these devices, causing these devices to be allocated to the first target file when
