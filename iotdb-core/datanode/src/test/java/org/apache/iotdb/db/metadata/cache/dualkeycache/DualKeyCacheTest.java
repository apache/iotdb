...
public class DualKeyCacheTest {
  ...
  public void testInvalidDatabase() throws IllegalPathException {
      ...
      int expectSize = PartialPath.estimateSize(new PartialPath("root.db2.d1"))
              + computeStringSize("s1")*2+
      ...
      expectSize = PartialPath.estimateSize(new PartialPath("root.db2.d1"))
  ...
  public void testInvalidPathPattern1() throws IllegalPathException {
      ...
      int expectSize = PartialPath.estimateSize(new PartialPath("root.db1.d1"))
              +PartialPath.estimateSize(new PartialPath("root.db1"))
              + computeStringSize("s1")+computeStringSize("s11")+
              SchemaCacheEntry.estimateSize(new SchemaCacheEntry(
                      "root.db1",
                      new MeasurementSchema("s1", TSDataType.INT32),
                      Collections.emptyMap(),
                      false))+SchemaCacheEntry.estimateSize(new SchemaCacheEntry(
                      "root.db1",
  ...
  public void testInvalidPathPattern2() throws IllegalPathException {
    ...
    private IDualKeyCache<PartialPath, String, SchemaCacheEntry> generateCache() throws IllegalPathException {
        ...
        Assert.assertNotNull(dualKeyCache.get(new PartialPath("root.db2.d1"), "s1"));
        Assert.assertNotNull(dualKeyCache.get(new PartialPath("root.db2.d1"), "s2"));
        int expectSize = PartialPath.estimateSize(new PartialPath("root.db1.d1")) * 2
                + PartialPath.estimateSize(new PartialPath("root.db1"))
                + computeStringSize("s1")*4+computeStringSize("s11")+
                SchemaCacheEntry.estimateSize(new SchemaCacheEntry(
                        "root.db1",
                        new MeasurementSchema("s1", TSDataType.INT32),
                        Collections.emptyMap(),
