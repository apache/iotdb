...
public class SchemaNodeListeningQueueTest {
  ...
  public void testSnapshot() throws TException, IOException, AuthException, IllegalPathException {
    ...
    Assert.assertEquals(node1, ((PipeWriteSchemaPlanEvent) event1).getPlanNode());

    Event event2 = itr.next(0);
    Assert.assertEquals(
        node2.getWriteSchemaNode(), ((PipeWriteSchemaPlanEvent) event2).getPlanNode());
    Assert.assertTrue(((PipeWriteSchemaPlanEvent) event2).isGeneratedByPipe());
