...
public class LastQueryCacheTest {
  ...
  private final TreeDeviceSchemaCacheManager cache = TreeDeviceSchemaCacheManager.getInstance();
  ...
  public void setUp() throws Exception {
    ...
    // Seed database options locally so cache declarations do not contact a ConfigNode.
    Field field = ClusterPartitionFetcher.class.getDeclaredField("partitionCache");
    field.setAccessible(true);
    partitionCache = (PartitionCache) field.get(ClusterPartitionFetcher.getInstance());
    partitionCache.updateDatabaseCache(
        new HashSet<>(Arrays.asList(DATABASE, VirtualDataRegion.getInstance().getDatabaseName())));
  }
  ...
  public void testRealRegionRetainsEmptyResultCachePolicy() throws Exception {
    ...
    try (Operator operator = createOperator(context)) {
      ...
      consumeEmptyResult(operator, context);
      if (updateNullEntry) {
        assertEquals(TableDeviceLastCache.PLACEHOLDER_EMPTY_COLUMN, cache.getLastCache(path));
      } else {
        assertNull(cache.getLastCache(path));
      }
    }
    ...
    FragmentInstanceContext instanceContext =
        spy(
            createFragmentInstanceContext(
                ...
                new SessionInfo(
                    1, new UserEntity(666, "test", "127.0.0.1"), ZoneId.systemDefault()),
                dataRegion,
                (TimePredicate) null,
                queryContexts,
                Long.MAX_VALUE,
