...
public class LocalFileRoleManagerTest {
  ...
  public void test() throws AuthException, IllegalPathException {
    ...
    manager.grantPrivilegeToEntry(role.getName(), new PrivilegeUnin(path, PrivilegeType.READ_DATA));
    manager.grantPrivilegeToEntry(
        role.getName(), new PrivilegeUnin(path, PrivilegeType.WRITE_DATA));
    ...
    manager.grantPrivilegeToEntry(role.getName(), new PrivilegeUnin(PrivilegeType.MAINTAIN));
    manager.grantPrivilegeToEntry(
        role.getName(), new PrivilegeUnin(PrivilegeType.MANAGE_ROLE, true));
    boolean caught = false;
    try {
      manager.grantPrivilegeToEntry("not a role", new PrivilegeUnin(PrivilegeType.MANAGE_ROLE));
    ...
    assertTrue(
        manager.revokePrivilegeFromEntry(
            role.getName(), new PrivilegeUnin(PrivilegeType.MAINTAIN)));
    assertFalse(
        manager.revokePrivilegeFromEntry(
            role.getName(), new PrivilegeUnin(PrivilegeType.MANAGE_USER)));
    assertFalse(
        manager.revokePrivilegeFromEntry(
            role.getName(),
            new PrivilegeUnin(new PartialPath("root.test"), PrivilegeType.WRITE_SCHEMA)));
    assertEquals(1, manager.getEntry(role.getName()).getSysPriGrantOpt().size());
    caught = false;
    try {
      manager.revokePrivilegeFromEntry("not a role", new PrivilegeUnin(PrivilegeType.MAINTAIN));
    } catch (AuthException e) {
      caught = true;
    }
    assertTrue(caught);
