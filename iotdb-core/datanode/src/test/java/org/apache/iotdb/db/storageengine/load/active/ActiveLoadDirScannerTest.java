...
public class ActiveLoadDirScannerTest {
  ...
  public void testScanDeduplicatesTsFileAndCompanionFiles() throws Exception {
    // The recursive scanner sees all three paths, but every companion maps back to the same TsFile.
    ...
    verify(loader, times(1))
        .tryTriggerTsFileLoad(
            eq(tsFile.getAbsolutePath()), eq(pendingDir.getAbsolutePath()), eq(false));
  }
