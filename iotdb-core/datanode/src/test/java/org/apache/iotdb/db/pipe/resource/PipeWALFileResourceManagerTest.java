...
public class PipeWALFileResourceManagerTest {
  ...
  private PipeWALFileResourceManager pipeWALFileResourceManager;
  ...
  public void testIncreaseTsfile() throws IOException {
    ...
    Assert.assertEquals(0, pipeWALFileResourceManager.getFileReferenceCount(originWALFile));

    File pipeWALFile = pipeWALFileResourceManager.increaseFileReference(originWALFile);
    Assert.assertEquals(1, pipeWALFileResourceManager.getFileReferenceCount(pipeWALFile));
    ...
    pipeWALFileResourceManager.increaseFileReference(pipeWALFile);
    Assert.assertEquals(2, pipeWALFileResourceManager.getFileReferenceCount(pipeWALFile));
    Assert.assertTrue(Files.exists(originWALFile.toPath()));
    Assert.assertTrue(Files.exists(pipeWALFile.toPath()));
  }
  ...
  public void testDecreaseTsfile() throws IOException {
    ...
    pipeWALFileResourceManager.decreaseFileReference(originFile);
    Assert.assertEquals(0, pipeWALFileResourceManager.getFileReferenceCount(originFile));

    File pipeWALFile = pipeWALFileResourceManager.increaseFileReference(originFile);
    Assert.assertEquals(1, pipeWALFileResourceManager.getFileReferenceCount(pipeWALFile));
    Assert.assertTrue(Files.exists(pipeWALFile.toPath()));
    Assert.assertTrue(Files.exists(pipeWALFile.toPath()));
    ...
    pipeWALFileResourceManager.decreaseFileReference(pipeWALFile);
    Assert.assertEquals(0, pipeWALFileResourceManager.getFileReferenceCount(pipeWALFile));
    Assert.assertFalse(Files.exists(originFile.toPath()));
