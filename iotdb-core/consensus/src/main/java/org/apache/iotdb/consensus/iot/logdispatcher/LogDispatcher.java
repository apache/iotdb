  ...
  public class LogDispatcherThread implements Runnable {
    ...
    private boolean constructBatchFromWAL(long currentIndex, long maxIndex, Batch logBatches) {
      ...
      while (targetIndex < maxIndex && logBatches.canAccumulate()) {
        ...
        // construct request from wal. Large shardable requests are streamed out shard by shard;
        // when the batch fills up mid-group, the leftover shards are kept for the next getBatch().
        if (!appendSerializedRequests(data, true, logBatches)) {
          break;
        }
      }
