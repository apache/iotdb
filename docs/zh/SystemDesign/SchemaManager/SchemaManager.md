...
为了加快 IoTDB 重启速度，我们为 MTree 设置了检查点，即每隔一段时间（可配置），后台线程检查 MTree 的最后修改时间，如果用户超过1小时没修改 MTree （`mlog.txt` 文件超过1小时没有修改），并且 `mlog.txt` 中积累了用户配置的日志条数，就创建一次 MTree snapshot。这样避免了在重启时按行读取并复现 `mlog.txt` 中的信息。
