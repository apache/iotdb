...
| Metric | Tag                                                                           | level     | 说明                                                            | 示例                              |
| ------ |-------------------------------------------------------------------------------| --------- |---------------------------------------------------------------| --------------------------------- |
| mem    | name="chunkMetaData/storageGroup/schema memory usage/schema memory remaining" | important | chunkMetaData 占用/storageGroup 占用/schema 占用/schema 剩余的内存（byte） | mem{name="chunkMetaData",} 2050.0 |
...
| Metric   | Tag                                                                                       | level     | 说明                                                 | 示例                                          |
|----------|-------------------------------------------------------------------------------------------| --------- |----------------------------------------------------| --------------------------------------------- |
| quantity | name="timeSeries/storageGroup/device/device using template", type="total/normal/template/template" | important | 当前时间timeSeries/storageGroup/device/激活了模板的device的数量 | quantity{name="timeSeries",type="normal"} 1.0 |
