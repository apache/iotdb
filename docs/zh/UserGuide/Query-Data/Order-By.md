...
## 时间对齐模式下的排序
IoTDB的查询结果集默认按照时间对齐，可以使用`ORDER BY TIME`的子句指定时间戳的排序顺序。示例代码如下：
```sql
...
在设备对齐模式下支持4种排序模式的子句,其中包括两种排序键，`DEVICE`和`TIME`，靠前的排序键为主排序键，每种排序键都支持`ASC`和`DESC`两种排序顺序。
1. ``ORDER BY DEVICE``: 按照设备名的字典序进行排序，排序方式为字典序排序，在这种情况下，相同名的设备会以聚类的形式进行展示。

2. ``ORDER BY TIME``: 按照时间戳进行排序，此时不同的设备会按照时间戳的优先级被打乱排序。

3. ``ORDER BY DEVICE,TIME``: 按照设备名的字典序进行排序，设备名相同的序列会通过时间戳进行排序。

4. ``ORDER BY TIME,DEVICE``: 按照时间戳进行排序，时间戳相同的序列会通过设备名的字典序进行排序。
...
Total line number = 6
```
主排序键为`Time`时，结果集会先按照时间戳进行排列，在时间戳相等时按照设备名排序。
示例代码如下：
```sql
