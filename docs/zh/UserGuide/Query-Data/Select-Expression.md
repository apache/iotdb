...
- `method`：采样方法，取值为`random`或`aggregation`或`M4`。在缺省情况下，采用`random`的随机采样法。
- `proportion`：降采样比例，取值区间为`(0, 1]`。
#### 等分桶随机采样
当`method`参数为`random`或缺省时，采用随机采样法对输入序列进行采样。

| 函数名      | 可接收的输入序列类型                     | 必要的属性参数                               | 输出序列类型     | 功能类型                                             |
|----------|--------------------------------|---------------------------------------|------------|--------------------------------------------------|
| EQUAL_BUCKET_SAMPLE   | INT32 / INT64 / FLOAT / DOUBLE | `method`:`random`</br>`proportion`取值范围为`(0, 1]`，默认为`0.1`  | INT32 / INT64 / FLOAT / DOUBLE | 返回符合采样比例的等分桶随机采样                |
...
当`method`参数为`aggregation`时，采用聚合采样法对输入序列进行采样。当选择`aggregation`作为采样方法，用户需要另外提供一个聚合函数参数即
- `type`：聚合类型，取值为`avg`或`max`或`min`。在缺省情况下，采用`avg`。

| 函数名      | 可接收的输入序列类型                     | 必要的属性参数                               | 输出序列类型     | 功能类型                                             |
|----------|--------------------------------|---------------------------------------|------------|--------------------------------------------------|
| EQUAL_BUCKET_SAMPLE   | INT32 / INT64 / FLOAT / DOUBLE | `method`:`aggregation`</br>`proportion`取值范围为`(0, 1]`，默认为`0.1`</br>`type`:取值类型有`avg`, `max`, `min`, 默认为`avg`  | INT32 / INT64 / FLOAT / DOUBLE | 返回符合采样比例的等分桶聚合采样                |
...
当`method`参数为`M4`时，采用M4采样法对输入序列进行采样。即对于每个桶采样首、尾、最小和最大值。

| 函数名      | 可接收的输入序列类型                     | 必要的属性参数                               | 输出序列类型     | 功能类型                                             |
|----------|--------------------------------|---------------------------------------|------------|--------------------------------------------------|
| EQUAL_BUCKET_SAMPLE   | INT32 / INT64 / FLOAT / DOUBLE | `method`:`M4`</br>`proportion`取值范围为`(0, 1]`，默认为`0.1`| INT32 / INT64 / FLOAT / DOUBLE | 返回符合采样比例的等分桶M4采样                |
