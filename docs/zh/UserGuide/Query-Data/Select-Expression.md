...
| `*`            | 在路径中表示一层。                                  |
| `**`           | 在路径中表示是（`*`）+，即为一层或多层`*`。                  |
| `*(start.end)` | 在路径中表示最少start层（包含），最多end层（不包含）。            |
| `*(start,)`    | 在路径中表示最少start层（包含），最多层数不限制。`*(1,)`等价于`**`。 |
| `*(,end)`      | 在路径中表示最少0层（包含），最多end层（不包含）。                |
...
#### 已知存储组`root.ln`； 一个路径节点`wf05`；需要查询已知路径节点下一个节点的status指标。
...
#### 已知存储组`root.ln`； 一个路径节点`wf05`；需要查询已知路径节点下一个节点之后的所有的status指标。
...
#### 已知存储组`root.ln`； 两个路径节点`wf01`，`wf05`；两个路径节点的相对顺序`wf01`在`wf05`的左边。查询符合条件下的所有status指标
...
#### 已知存储组`root.ln`； 一个路径节点`wf05`；需要查询已知路径节点下两个节点之后所有的status指标。
...
#### 已知存储组`root.ln`； 一个路径节点`wf05`；需要查询已知路径节点下两个节点之内的所有的status指标。
...
#### 已知存储组`root.ln`； 一个路径节点`wf05`；需要查询已知路径节点下两个节点之后，四个节点之内的的所有的status指标。
