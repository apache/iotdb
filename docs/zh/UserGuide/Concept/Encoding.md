...
定频数据编码，仅适用于整形（INT32）和长整型（INT64）的定频数据，且允许数据中有一些点缺失，或者偶尔有错误数据，编码时会纠正错误数据。使用此方法编码定频数据优于二阶差分编码（TS_2DIFF）。
例如：
插入数据：（1，1000），（2,1100），（3，1800），（4，1400），（5， 1500）
纠正为：（1，1000），（2,1100），（3，1300），（4，1400），（5， 1500）
定频数据编码无法用于非定频数据，如果插入非定频数据将导致数据丢失。建议使用二阶差分编码（TS_2DIFF）进行处理。
