...
$$MAE = \frac{1}{n}\sum^n_{i=1}|\hat{y_i}-y_i|$$，其中，$\hat{y_i}$为预测值，$y_i$为真实值。

**函数名：** MAE

**输入序列：** 仅支持两个输入序列，类型均为 INT32 / INT64 / FLOAT / DOUBLE。

**输出序列：** 输出单个序列，类型为 DOUBLE。序列仅包含一个时间戳为 0、值为平均绝对误差的数据点。
...
```sql
select mae(s1,s2) from root.test.d2
```
...
```
+-----------------------------+-------------------------------------+
|                         Time|mae(root.test.d2.s1, root.test.d2.s2)|
+-----------------------------+-------------------------------------+
|1970-01-01T08:00:00.000+08:00|                    8.846153846153847|
+-----------------------------+-------------------------------------+
```
...
+-----------------------------+--------------------------------------+
|1970-01-01T08:00:00.000+08:00|                    11.357816691600547|
+-----------------------------+--------------------------------------+
```
