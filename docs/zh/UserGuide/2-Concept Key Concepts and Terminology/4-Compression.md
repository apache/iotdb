...
IoTDB允许在创建一个时间序列的时候指定该列的压缩方式。现阶段IoTDB现在支持的压缩方式有两种：UNCOMPRESSOR（不压缩）和SNAPPY压缩。压缩方式的指定语法详见本文[5.1节](../5-IoTDB%20SQL%20Documentation/1-IoTDB%20Query%20Statement.html)。
