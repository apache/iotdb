...
|334|CREATE_PIPE_SINK_FAILED|创建 PIPE Sink 失败|
...
|340|CREATE_TEMPLATE_FAILED|创建物理量模板失败|
|341|SYNC_FILE_REDIRECTION_FAILED|同步文件时重定向异常|
|342|SYNC_FILE_FAILED|同步文件异常|
...
|710|MIGRATE_REGION_FAILED|Region 迁移失败|
|711|CREATE_REGION_FAILED|创建 region 失败|
|712|DELETE_REGION_FAILED|删除 region 失败|
|713|PARTITION_CACHE_UPDATE_FAIL|更新分区缓存失败|
|714|DESERIALIZE_PIECE_OF_TSFILE_FAILED|反序列化 TsFile 片段异常|
...
|904|NOT_ENOUGH_DATANODE|没有足够的 DataNode|
|905|ERROR_GLOBAL_CONFIG|全局配置参数异常|
|906|ADD_CONFIGNODE_FAILED|新增 ConfigNode 失败|
|907|REMOVE_CONFIGNODE_FAILED|移除 ConfigNode 失败|
|912|DATANODE_NOT_EXIST|此 DataNode 不存在|
|915|REGION_MIGRATE_FAILED|此 DataNode 不存在|
|916|DATANODE_NO_LARGER_THAN_REPLICATION|DataNode 不大于副本数，无法移除节点|
|917|DATANODE_STOP_FAILED|DataNode 关闭失败|
|918|REGION_LEADER_CHANGE_FAILED|Region leader 迁移失败|
|919|REMOVE_DATANODE_FAILED|移除 datanode 失败|
|920|OVERLAP_WITH_EXISTING_TASK|与正在执行的其他操作冲突|
|921|NOT_AVAILABLE_REGION_GROUP|无法找到可用的 Region 副本组|
|922|CREATE_TRIGGER_FAILED|ConfigNode 创建 Trigger 失败|
|923|DROP_TRIGGER_FAILED|ConfigNode 删除 Trigger 失败|
...
|933|CQ_UPDATE_LAST_EXEC_TIME_FAILED|CQ 更新上一次执行时间失败|
