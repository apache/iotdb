...
| client_manager_num_active            | pool_name="{pool_name}"                | AutoGauge | 当前从此池中借用但尚未归还的实例总数                     |
| client_manager_num_idle              | pool_name="{pool_name}"                | AutoGauge | 该池中当前空闲的实例总数                                 |
| client_manager_borrowed_count        | pool_name="{pool_name}"                | AutoGauge | 在池的生命周期内成功从此池借用的对象总数                 |
| client_manager_created_count         | pool_name="{pool_name}"                | AutoGauge | 在池的生命周期内为此池创建的对象总数                     |
| client_manager_destroyed_count       | pool_name="{pool_name}"                | AutoGauge | 在池的生命周期内被该池销毁的对象总数                     |
| client_manager_mean_active_time      | pool_name="{pool_name}"                | AutoGauge | 最后缓存大小个返回到池对象的处于活动状态的平均时间       |
| client_manager_mean_borrow_wait_time | pool_name="{pool_name}"                | AutoGauge | 最后缓存大小个从池中借用对象的线程等待借用对象的平均时间 |
| client_manager_mean_idle_time        | pool_name="{pool_name}"                | AutoGauge | 最后缓存大小个从池中借用对象的对象空闲的平均时间         |
