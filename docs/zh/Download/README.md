...
- 如何从 v0.11.x 或 v0.10.x 升级到 v0.12.x? 
  ...
  * 停止旧版本 IoTDB 的实例，启动 v0.12 的实例。IoTDB 将后台自动升级数据文件格式。在升级过程中数据可以进行查询和写入。
    ...
    * __注意 3: 在文件升级完成前，最好不要进行 delete 操作。如果删除某个存储组内的数据且该存储组内存在待升级文件，删除会失败__
    * __注意 4: 升级途中因某些原因停止了 IoTDB ，先确认`data/system/schema/system.properties`文件中的 `iotdb_version` 为原版本，否则 IoTDB 再次启动时会报错且无法再次触发升级操作__
