...
According to which we can set up the corresponding storage group. The SQL statements for creating storage groups are as follows:
...
### Delete Timeseries
To delete the timeseries we created before, we are able to use the `DELETE TimeSeries <PrefixPath>` statement.
