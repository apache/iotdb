...
* First, construct a `TsFileWriter` instance.
    ...
    * With pre-defined schema
    ...
    ```
    In this example, TSFile files will be stored in HDFS instead of local file system. You can also config the ip and port of your HDFS by `config.setHdfsIp(...)` and `config.setHdfsPort(...)`. The default ip is `localhost` and default port is `9000`.
