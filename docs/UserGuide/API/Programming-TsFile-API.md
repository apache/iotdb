...
* **device**: A device is an entity that is takes several measurements (producing multiple timeseries), e.g., 
...
2. add measurements
    ...
    * compression: The data compression. Not only UNCOMPRESSED and SNAPPY.

    * props: Properties for special data types. `max_point_number` for `FLOAT` and `DOUBLE`, `TEXT` can set `max_string_length`. 
