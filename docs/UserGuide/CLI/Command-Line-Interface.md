...
Here, users need to replace {my-access-token}（note, including{}) with users' token.
...
The-e parameter can be used when users want to do batch operations on IOTDB via the CLI/Sheel via scripts. By using the -e parameter, you can operate IoTDB without entering the cli's input mode.
