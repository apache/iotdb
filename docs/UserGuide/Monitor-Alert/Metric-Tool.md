...
| client_manager_num_active            | pool_name="{pool_name}"                | AutoGauge | The total number of instances currently borrowed from this pool but not yet returned |
| client_manager_num_idle              | pool_name="{pool_name}"                | AutoGauge | The total number of instances currently idle in this pool    |
| client_manager_borrowed_count        | pool_name="{pool_name}"                | AutoGauge | The total number of objects successfully borrowed from this pool over the lifetime of the pool |
| client_manager_created_count         | pool_name="{pool_name}"                | AutoGauge | The total number of objects created for this pool over the lifetime of the pool |
| client_manager_destroyed_count       | pool_name="{pool_name}"                | AutoGauge | The total number of objects destroyed by this pool over the lifetime of the pool |
| client_manager_mean_active_time      | pool_name="{pool_name}"                | AutoGauge | The mean time objects are active for based on the last MEAN_TIMING_STATS_CACHE_SIZE objects returned to the pool |
| client_manager_mean_borrow_wait_time | pool_name="{pool_name}"                | AutoGauge | The mean time threads wait to borrow an object based on the last MEAN_TIMING_STATS_CACHE_SIZE objects borrowed from the pool |
| client_manager_mean_idle_time        | pool_name="{pool_name}"                | AutoGauge | The mean time objects are idle for based on the last MEAN_TIMING_STATS_CACHE_SIZE objects borrowed from the pool |
