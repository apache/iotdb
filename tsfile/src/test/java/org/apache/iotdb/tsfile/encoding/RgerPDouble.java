...
//        }
    ...
    for (int i = 0; i <= p; i++) {
      coefficient.add((double) param[i]);
      coefficient.add((double) param2[i]);
    }
  }
  // --------------------------------------  base function -----------------------------------------------------
...
//    int length = (max_bit_width_interval + max_bit_width_value) * (block_size - 1);
  ...
  private static ArrayList<ArrayList<Integer>> getEncodeBitsRegressionPNoTrain(
    ...
    // regression residual
    for (int j = 1; j < p; j++) {
      double epsilon_r = ts_block.get(j).get(0) - coefficient.get(0);
      double epsilon_v = ts_block.get(j).get(1) - coefficient.get(1);
      for (int pi = 1; pi <= j; pi++) {
        epsilon_r -= coefficient.get(2 * pi) * (double) ts_block.get(j - pi).get(0);
        epsilon_v -= coefficient.get(2 * pi + 1) * (double) ts_block.get(j - pi).get(1);
      }
...
//            System.out.println(bit_width_value);
  ...
  public static void main(@org.jetbrains.annotations.NotNull String[] args) throws IOException {
    String parent_dir = "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\compression_ratio\\p_double";
        ...
        for (File f : tempList) {
          ...
          for (int i = 0; i < repeatTime; i++) {
            ...
            for (int repeat_i = 0; repeat_i < 1; repeat_i++)
              buffer =
                      ReorderingRegressionEncoder(data, dataset_block_size.get(file_i), dataset_third.get(file_i), 8,p,3);
