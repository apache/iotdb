...
public class Simple8bTest {
    ...
    @Test
    public void testSimple8bReadInt() throws Exception {
        for (int i = 1; i < 10; i++) {
            testInt(intList2, false, i);
        }
    }

    private void testInt(List<Integer> list, boolean isDebug, int repeatCount) throws Exception {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        Encoder encoder = new Simple8bEncoder();
        ...
        Simple8bDecoder decoder = new Simple8bDecoder();
        List<Integer> values_ = new ArrayList<>();

        for (int i = 0; i < repeatCount; i++) {
            values_ = decoder.readIntList(bais);
            for (int j = 0; j < list.size(); j++) {
                if (isDebug) {
                    logger.debug("{} // {}", values_.get(j), list.get(j));
                }
                assertEquals(list.get(j), values_.get(j));
            }
//            for (int value : list) {
