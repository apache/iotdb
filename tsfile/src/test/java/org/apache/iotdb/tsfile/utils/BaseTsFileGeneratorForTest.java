...
@Ignore
public abstract class BaseTsFileGeneratorForTest {
    private static FSFactory fsFactory = FSFactoryProducer.getFSFactory();
    public static final long START_TIMESTAMP = 1480562618000L;
    protected static String inputDataFile;
    public static String outputDataFile;
    protected String errorOutputDataFile;
    protected int chunkGroupSize;
    protected int pageSize;

    public abstract void initParameter();

    public void generateFile(int minRowCount, int maxRowCount, int rowCount) throws IOException {
        initParameter();
        generateSampleInputDataFile(minRowCount, maxRowCount, rowCount);
        write();
    }

    protected void closeAndDelete() {
    ...
    private void generateSampleInputDataFile(int minRowCount, int maxRowCount, int rowCount) throws IOException {
        File file = fsFactory.getFile(inputDataFile);
        if (file.exists()) {
            Assert.assertTrue(file.delete());
        }
    ...
    public void write() throws IOException {
        ...
        // write
        try (TsFileWriter innerWriter = new TsFileWriter(file, schema, TSFileDescriptor.getInstance().getConfig());
    ...
    private Schema generateTestSchema() {
        ...
        schema.registerTimeseries(
                new Path("d1", "s7"), new MeasurementSchema("s7", TSDataType.DOUBLE, TSEncoding.GORILLA_V1));
     ...
     */
    public void writeFileWithOneIncompleteChunkHeader(File file) throws IOException {
