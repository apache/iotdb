...
public class KLLSketchLazyExactPriorigamma extends KLLSketchForQuantilegamma {
  ...
  public KLLSketchLazyExactPriorigamma(int maxMemoryByte, double gamma) {
    super(gamma);
    N = 0;
  ...
  protected void calcLevelMaxSize(int setLevel) { // set cntLevel. make sure cntLevel won't decrease
    ...
    // int newK = 0;
    // for (int addK = 1 << 28; addK > 0; addK >>>= 1) { // find a new K to fit the
    // memory limit. compact when size>cap. sum of cap<maxMemNum
    // int need = 0;
    // for (int i = 0; i < cntLevel; i++)
    // need += Math.max(3, (int) Math.round(((newK + addK) * Math.pow(2.0 / 3,
    // cntLevel - i - 1))));
    // if (need <= maxMemoryNum) newK += addK;
    // }
    // for (int i = 0; i < cntLevel; i++)
    // levelMaxSize[i] = Math.max(3, (int) Math.round((newK * Math.pow(2.0 / 3,
    // cntLevel - i - 1))));
  ...
  private long getLowerBound(long queryRank, double Pr) {
    ...
    while (L < R) {
      ...
      // System.out.println("\t\t\t\t\t"+longToResult(mid)+"\t\trank:"+approxRankMid+"\t\terr:"+rankErrBound+"\t\t\tL,R,mid:"+L+"
      // "+R+" "+mid);
    ...
    // lowerBound.\t\tPr:"+Pr+"\trank:"+queryRank+"\t\t\t\tlowerBoundV:"+L+"(longToResult:"+longToResult(L)+")"+
    // "\t\tL_rank:"+getApproxRank(L)+"\t\tPrErr:"+queryRankErrBound(L,Pr)+"\t1.0Err:"+queryRankErrBound(L,1.0));
    return L;
  }
