 ...
 * {@linkplain TSEncoding TSEncoding}, which stores several configuration
 * related to responding encoding type to generate {@linkplain Encoder Encoder}
 * instance.<br>
 * Each TSEncoding has a responding TSEncodingBuilder. The design referring to
 * visit pattern provides same outer interface for different TSEncodings and
...
public abstract class TSEncodingBuilder {
   ...
   * according to its measurement id and data type.
   *
   ...
   * {@code InitFromJsonObject} gets values from JSON object which will be used
   * latter.<br>
    ...
    public void initFromProps(Map<String, String> props) {
      ...
      if (props == null || !props.containsKey(Encoder.MAX_STRING_LENGTH)) {
        ...
        if (maxStringLength < 0) {
          maxStringLength = TSFileDescriptor.getInstance().getConfig().getMaxStringLength();
          logger.warn("cannot set max string length to negative value, replaced with default value:{}",
              maxStringLength);
        }
      }
    }
  ...
  public static class RLE extends TSEncodingBuilder {
     ...
     * the maximum decimal digits for float or double data.
     */
    @Override
    public void initFromProps(Map<String, String> props) {
      // set max error from initialized map or default value if not set
      if (props == null || !props.containsKey(Encoder.MAX_POINT_NUMBER)) {
        ...
        if (maxPointNumber < 0) {
          maxPointNumber = TSFileDescriptor.getInstance().getConfig().getFloatPrecision();
          logger.warn("cannot set max point number to negative value, replaced with default value:{}", maxPointNumber);
        }
      }
    }
