...
public class KLLDupliPairFT3way extends KLLSketchForQuantile { // TODO: too large count
  ...
  public KLLDupliPairFT3way(int maxMemoryByte) {
    N = 0;
  ...
  public KLLDupliPairFT3way(int maxMemoryByte, boolean enableFreqHashMap) {
    this(maxMemoryByte);
    this.ENABLE_FREQ_HASHMAP = enableFreqHashMap;
  }

  public KLLDupliPairFT3way(
      int maxMemoryByte, boolean enableFreqHashMap, boolean compactActualSize) {
  ...
  private boolean processLV0() {
    ...
    sort_time -= System.nanoTime();
    Long[] tempArray = new Long[levelPos[0] - bufferPosForLV0];
    for (int i = bufferPosForLV0; i < levelPos[0]; i++) tempArray[i - bufferPosForLV0] = num[i];
    Quick3way.sort(tempArray);
    for (int i = bufferPosForLV0; i < levelPos[0]; i++) num[i] = tempArray[i - bufferPosForLV0];
    sort_time += System.nanoTime();
