  ...
  public float readFloat(ByteBuffer buffer) {
    if (!flag) {
      ...
      try {
        ...
        LOGGER.error("SinglePrecisionDecoder cannot read first float number because: {}", e);
      }
    } else {
      try {
        float tmp = Float.intBitsToFloat(preValue);
        ...
        LOGGER.error("SinglePrecisionDecoder cannot read following float number because: {}", e);
      }
    }
    return Float.NaN;
  }
