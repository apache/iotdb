...
public class KLLDupliPairFTgamma extends KLLSketchForQuantilegamma { // TODO: too large count
  boolean DEBUG = false;
  ...
  public KLLDupliPairFTgamma(int maxMemoryByte, double gamma) {
    super(gamma);
    N = 0;
  ...
  public void continuousCompact(int targetNumLen) {
    ...
    while (getTotalNumLen() > targetNumLen) {
      ...
      for (int i = cntLevel - 1; i >= 0; i--) {
        totW += cntW;
        cntW /= 1.0 / this.GAMMA; // exponential actual capacity
      }
  ...
  private LongArrayList proNumF = new LongArrayList();
  private IntArrayList proPairF = new IntArrayList(); // System.out.println("");
  private LongArrayList proNumT = new LongArrayList();
  private IntArrayList proPairT = new IntArrayList(); // System.out.println("");
