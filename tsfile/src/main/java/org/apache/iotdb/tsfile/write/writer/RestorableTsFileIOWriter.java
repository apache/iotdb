...
public class RestorableTsFileIOWriter extends TsFileIOWriter {
  ...
  */
  ...
  /**
   * get all the chunkGroups' metadata which are appended after the last calling
  ...
  */
   ...
   * complete TsFile, the function erases all FileMetadata and supports writing
   * new data; For a incomplete TsFile, the function supports writing new data
   * directly. However, it is more efficient using the construction function of
   * RestorableTsFileIOWriter, if the tsfile is incomplete.
   *
   * @param file a TsFile
  ...
  public static RestorableTsFileIOWriter getWriterForAppendingDataOnCompletedTsFile(File file) throws IOException {
    ...
    try (TsFileSequenceReader reader = new TsFileSequenceReader(file.getAbsolutePath(), false)) {
      ...
      if (reader.isComplete()) {
        ...
        for (long tsOffset : metaData.getTsOffsets()) {
          if (position > tsOffset) {
            position = tsOffset;
          }
        }
      }
