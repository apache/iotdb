...
import static org.apache.iotdb.tsfile.file.metadata.enums.MetadataIndexNodeType.INTERNAL_DEVICE;
...
public class TsFileSequenceReader implements AutoCloseable {
  ...
  public TimeseriesMetadata readTimeseriesMetadata(Path path, boolean ignoreNotExists)
    ...
    if (!metadataIndexNode.getNodeType().equals(MetadataIndexNodeType.LEAF_MEASUREMENT)) {
      try {
        ...
        logger.error(
            "Something error happened while deserializing MetadataIndexNode of file {}", file, e);
        throw e;
      }
      metadataIndexPair =
    ...
    while (buffer.hasRemaining()) {
      try {
        ...
        logger.error(
            "Something error happened while deserializing TimeseriesMetadata of file {}", file, e);
        throw e;
      }
    }
  ...
  public List<TimeseriesMetadata> readTimeseriesMetadata(String device, Set<String> measurements)
    ...
    MetadataIndexNode deviceMetadataIndexNode = tsFileMetaData.getMetadataIndex();
    Pair<MetadataIndexEntry, Long> metadataIndexPair =
        getMetadataAndEndOffset(deviceMetadataIndexNode, device, INTERNAL_DEVICE, false);
    if (metadataIndexPair == null) {
      return Collections.emptyList();
    }
    ...
    for (int i = 0; i < measurementList.size(); i++) {
      ...
      while (buffer.hasRemaining()) {
        try {
          ...
          logger.error(
              "Something error happened while deserializing TimeseriesMetadata of file {}",
              file,
              e);
          throw e;
        }
  ...
  private List<TimeseriesMetadata> getDeviceTimeseriesMetadata(String device) throws IOException {
    MetadataIndexNode metadataIndexNode = tsFileMetaData.getMetadataIndex();
    Pair<MetadataIndexEntry, Long> metadataIndexPair =
        getMetadataAndEndOffset(metadataIndexNode, device, INTERNAL_DEVICE, true);
    if (metadataIndexPair == null) {
      return Collections.emptyList();
    }
    ByteBuffer buffer = readData(metadataIndexPair.left.getOffset(), metadataIndexPair.right);
  ...
  private Pair<MetadataIndexEntry, Long> getMetadataAndEndOffset(
    ...
    try {
      ...
      logger.error(
          "Something error happened while deserializing MetadataIndex of file {}", file, e);
      throw e;
    }
  }
