...
public abstract class KLLSketchForQuantilegamma {
  ...
  double GAMMA = 2.0 / 3;
  // long XORSHIFT=new
  // Random(/*System.nanoTime()*/).nextInt();//0x2333333319260817L;
  public XoRoShiRo128PlusRandom random = new XoRoShiRo128PlusRandom(/* 11111 */ );
  // static public XoRoShiRo128PlusRandom random = new
  // XoRoShiRo128PlusRandom(11111);
  // Random test_random = new Random();

  public KLLSketchForQuantilegamma(double gamma) {
    this.GAMMA = gamma;
  }
  ...
  public int[] calcLevelMaxSizeByLevel(int maxMemoryNum, int setLevel) {
    ...
    for (int addK = 1 << 28; addK > 0; addK >>>= 1) { // find a new K to fit the memory limit.
      ...
      for (int i = 0; i < setLevel; i++)
        need +=
            Math.max(
                MinimumLevelMaxSize,
                (int) Math.round(((newK + addK) * Math.pow(this.GAMMA, setLevel - i - 1))));
    ...
    for (int i = 0; i < setLevel; i++)
      levelMaxSize[i] =
          Math.max(
              MinimumLevelMaxSize,
              (int) Math.round((newK * Math.pow(this.GAMMA, setLevel - i - 1))));
    return levelMaxSize;
  }

  public int[] calcLevelMaxSize(int maxMemoryNum, int maxN) {
  ...
  public void update(long x) { // signed long
    ...
    // boolean flag=false;
    // for(int
    // i=0;i<cntLevel;i++)if(levelPos[i+1]-levelPos[i]>levelMaxSize[i])flag=true;
    // if(flag)compact();
