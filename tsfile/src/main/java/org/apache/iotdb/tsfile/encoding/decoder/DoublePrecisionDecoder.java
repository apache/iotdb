  ...
  public double readDouble(ByteBuffer buffer) {
    if (!flag) {
      ...
      try {
        ...
        LOGGER.error("DoublePrecisionDecoder cannot read first double number because: {}", e);
      }
    } else {
      try {
        double tmp = Double.longBitsToDouble(preValue);
        ...
        LOGGER.error("DoublePrecisionDecoder cannot read following double number because: {}", e);
      }
    }
    return Double.NaN;
  }
