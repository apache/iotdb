...
public class TsFileRead {
  private static void queryAndPrint(ArrayList<Path> paths, ReadOnlyTsFile readTsFile, IExpression statement)
    ...
    System.out.println("count v2: " + count);
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    //paths.add(new Path("device_44.sensor_199"));
    //paths.add(new Path("device_2.sensor_1"));
    
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 320; j++) {
        paths.add(new Path("device_"+i+".sensor_"+j));
      }
    }
  ...
  */
    ...
    System.out.println("run time v2: " + (end - start));
  }
}
