...
public class ThriftConfigNodeSerializeDeserializeUtilsTest {
  ...
  public void readWriteTStorageGroupSchemaTest() {
    ...
    for (int i = 0; i < 3; i++) {
      storageGroupSchema0.getSchemaRegionGroupIds().add(new TConsensusGroupId(TConsensusGroupType.SchemaRegion, i * 2));
      storageGroupSchema0.getDataRegionGroupIds().add(new TConsensusGroupId(TConsensusGroupType.DataRegion, i * 2 + 1));
    }

    ThriftConfigNodeSerializeDeserializeUtils.writeTStorageGroupSchema(storageGroupSchema0, buffer);
    TStorageGroupSchema storageGroupSchema1 = ThriftConfigNodeSerializeDeserializeUtils.readTStorageGroupSchema(buffer);
