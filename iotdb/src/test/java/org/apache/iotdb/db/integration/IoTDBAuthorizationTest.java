...
public class IoTDBAuthorizationTest {
    ...
    public void allPrivilegesTest() throws ClassNotFoundException, SQLException {
        ...
        Connection userCon = DriverManager.getConnection(Config.IOTDB_URL_PREFIX + "127.0.0.1:6667/", "tempuser",
                "temppw");
    ...
    public void authPerformanceTest() throws ClassNotFoundException, SQLException {
        ...
        for (int i = 0; i < insertCnt;) {
            for (int j = 0; j < batchSize; j++)
                userStmt.addBatch(
                        "INSERT INTO root.a(timestamp, b" + (privilegeCnt - 1) + ") VALUES (" + (i++ + 1) + ", 100)");
            userStmt.executeBatch();
            userStmt.clearBatch();
        }
        System.out.println("User inserted " + insertCnt + " data points used " + (System.currentTimeMillis() - time)
        ...
        System.out.println("admin inserted " + insertCnt + " data points used " + (System.currentTimeMillis() - time)
                + " ms with " + privilegeCnt + " privileges");
    ...
    public void testListRoleUsers() throws SQLException, ClassNotFoundException {
        ...
        String[] members = { "HighFly", "SunComparison", "Persistence", "GoodWoods", "HealthHonor", "GoldLuck",
                "DoubleLight", "Eastwards", "ScentEffusion", "Smart", "East", "DailySecurity", "Moon", "RayBud",
                "RiverSky" };
