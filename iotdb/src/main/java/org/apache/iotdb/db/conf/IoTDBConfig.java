...
public class IoTDBConfig {
    ...
    /**
     * When a certain amount of write ahead log is reached, it will be flushed to disk. It is possible to lose at most
     * flush_wal_threshold operations
     */
    public int flushWalThreshold = 10000;

    /**
     * The cycle when write ahead log is periodically refreshed to disk(in milliseconds) It is possible to lose at most
     * flush_wal_period_in_ms ms operations
     */
    ...
    /**
     * The maximum concurrent thread number for merging overflow. When the value<=0 or > CPU core number, use the CPU
     * core number.
     */
    ...
    /**
     * When set true, start timing flush and merge service. False, stop timing flush and merge service. Default is true.
     */
    ...
    /**
     * MemMonitorThread will check every such interval. If memThresholdWarning is reached, MemMonitorThread will inform
     * FileNodeManager to flush.
     */
    public long memMonitorInterval = 1000; // in ms

    /**
     * Decide how to control memory used by inserting data. 0 is RecordMemController, which count the size of every
     * record (tuple). 1 is JVMMemController, which use JVM heap memory as threshold.
     */
    ...
    /**
     * The statMonitor writes statistics info into IoTDB every backLoopPeriodSec secs. Default value is 5s.
     */
    ...
    /**
     * Choose a postBack strategy of merging historical data: 1. It's more likely to update historical data, choose
     * "true". 2. It's more likely not to update historical data or you don't know exactly, choose "false".
     */
     ...
     * Then, if dataDir is absolute, leave dataDir as it is. If dataDir is relative, dataDir will be converted to the
     * complete version using non-empty %IOTDB_HOME%. e.g. for windows platform, | IOTDB_HOME | dataDir before | dataDir
     * after | |-----------------|--------------------|---------------------------| | D:\\iotdb\iotdb | null |
     * D:\\iotdb\iotdb\data\data | | D:\\iotdb\iotdb | dataDir | D:\\iotdb\iotdb\dataDir | | D:\\iotdb\iotdb |
     ...
     * Then, if sysDir is absolute, leave sysDir as it is. If sysDir is relative, sysDir will be converted to the
     * complete version using non-empty %IOTDB_HOME%. e.g. for windows platform, | IOTDB_HOME | sysDir before | sysDir
     * after | |-----------------|--------------------|-----------------------------| | D:\\iotdb\iotdb | null |
     * D:\\iotdb\iotdb\data\system | | D:\\iotdb\iotdb | sysDir | D:\\iotdb\iotdb\sysDir | | D:\\iotdb\iotdb |
     * C:\\sysDir | C:\\sysDir | | D:\\iotdb\iotdb | "" | D:\\iotdb\iotdb\ |
     * 
     * First, if walDir is null, walDir will be assigned the default value(i.e.,"data"+File.separatorChar+"data". Then,
     * if walDir is absolute, leave walDir as it is. If walDir is relative, walDir will be converted to the complete
     * version using non-empty %IOTDB_HOME%. e.g. for windows platform, | IOTDB_HOME | walDir before | walDir after |
     * |-----------------|--------------------|-----------------------------| | D:\\iotdb\iotdb | null |
     * D:\\iotdb\iotdb\data\wal | | D:\\iotdb\iotdb | walDir | D:\\iotdb\iotdb\walDir | | D:\\iotdb\iotdb | C:\\walDir |
