...
/**
 * A tsfile io writer that has the ability to restore an incomplete tsfile. <br/>
 * An incomplete tsfile represents the file which does not have tsfile metadata in the end. Besides, the last Chunk
 * group data may be broken. This class can slice off the broken Chunk group data, accept writing new data, and finally
 * write the tsfile metadata. <br/>
 * There are two cases: (1) though the tsfile loses the tsfile metadata in the end, a corresponding .restore file
 * exists. (2) no .restore file, and then the class has to traverse all the data for fixing the file.
 */
public class RestorableTsFileIOWriter extends TsFileIOWriter {
     ...
     * 
     * @return a pair, whose left Long value is the tail position of the last complete Chunk Group in the unsealed
     *         file's position, and the right List value is the ChunkGroupMetadata of all complete Chunk Group in the
     *         same file.
     * @throws IOException
     *             if errors when reading restoreFile.
     */
        ...
        if (metadatas.containsKey(deviceId)) {
            if (metadatas.get(deviceId).containsKey(measurementId)) {
                for (ChunkMetaData chunkMetaData : metadatas.get(deviceId).get(measurementId)) {
                    ...
                    // Then someone deletes the timeseries and recreate it with Int type. We have to ignore all the
                    // stale data.
    ...
    /**
     * add all appendChunkGroupMetadatas into memory. After calling this method, other classes can read these metadata.
     */
    void appendMetadata() {
        if (!append.isEmpty()) {
            for (ChunkGroupMetaData rowGroupMetaData : append) {
                for (ChunkMetaData chunkMetaData : rowGroupMetaData.getChunkMetaDataList()) {
                    addInsertMetadata(rowGroupMetaData.getDeviceID(), chunkMetaData.getMeasurementUID(), chunkMetaData);
                }
            }
            append.clear();
        }
    ...
    /**
     * get all the chunkGroups' metadata which are appended after the last calling of this method, or after the class
     * instance is initialized if this is the first time to call the method.
     * 
