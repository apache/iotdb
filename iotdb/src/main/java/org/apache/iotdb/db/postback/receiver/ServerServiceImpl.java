...
public class ServerServiceImpl implements ServerService.Iface {
    ...
    public void mergeOldData(String filePath) throws TException {
        ...
        try {
            ...
            Iterator<String> it = deviceIdMap.keySet().iterator();
            while (it.hasNext()) {
                String deviceId = it.next().toString(); // deviceId represent devices
                ...
                while (queryDataSet.hasNext()) {
                    ...
                    String sql_front = "insert into " + deviceId + "(timestamp";
                    String sql_rear = ") values(" + record.getTimestamp();
                    for (int i = 0; i < fields.size(); i++) {
                        Field field = fields.get(i);
                        if (field.toString() != "null") {
                            sql_front = sql_front + "," + paths.get(i).getMeasurement();
            ...
            while (it.hasNext()) {
                ...
                for (String timesery : timeseries) { // compare data with one timesery in a round to get valid data
                    ...
                    while (queryDataSet.hasNext()) {
                        ...
                        List<Field> fields = record.getFields();
                        String sql;
                        for (int i = 0; i < fields.size(); i++) { // get all data with the timesery in the postback file
                            Field field = fields.get(i);
                            if (!field.isNull()) {
                                sql = String.format(sqlFormat, key, paths.get(i).getMeasurement(), record.getTimestamp(), "%s");
                    ...
                    if (originDataPoint.isEmpty()) { // If there has no overlap data with the timesery, inserting all data in the postback file
