...
public class FileNodeManager implements IStatistic, IService {
     ...
     *            input Data
     * @param isMonitor:
     *            if true, the insertion is done by StatMonitor and thestatistic Info will not be recorded. if false,
     *            the statParamsHashMap will be updated.
            ...
            try {
                if (IoTDBDescriptor.getInstance().getConfig().enableWal) {
                    overflowProcessor.getLogNode()
                            .write(new UpdatePlan(startTime, endTime, v, new Path(deviceId + "." + measurementId)));
                }
            } catch (IOException e) {
                throw new FileNodeManagerException(e);
            }
    ...
    public QueryDataSource query(SingleSeriesExpression seriesExpression) throws FileNodeManagerException {
        ...
        try {
            ...
            try {
                ...
                LOGGER.error("Query error: the deviceId {}, the measurementId {}", deviceId, measurementId, e);
                throw new FileNodeManagerException(e);
            }
    ...
    /**
     * Append one specified tsfile to the storage group. <b>This method is only provided for transmission module</b>
     * 
     * @param fileNodeName
