...
import com.corp.delta.sql.src.ql.lib.Node;
    ...
    */
    ...
    private void getRootNodeWithValidASTStr() {
        ...
        if (rootNode != null && rootNode.parent == null &&
                rootNode.hasValidMemorizedString()) {
            return;
        }
    ...
    private boolean hasValidMemorizedString() {
    ...
    private int getMemorizedStringLen() {
        return astStr == null ? 0 : astStr.length();
    }

    private String getMemorizedSubString(int start, int end) {
        return (astStr == null || start < 0 || end > astStr.length() || start >= end) ? null :
                astStr.subSequence(start, end).toString();
    }

    private void addToMemorizedString(String string) {
        if (astStr == null) {
    ...
    public String toStringTree() {
        ...
        if (startIndx >= 0 && endIndx <= rootNode.getMemorizedStringLen()) {
            return rootNode.getMemorizedSubString(startIndx, endIndx);
        }
        ...
        while (!stack.isEmpty()) {
            ...
            if (!next.visited) {
                if (next.parent != null && next.parent.getChildCount() > 1 &&
                    ...
                    rootNode.addToMemorizedString(" ");
                }

                next.rootNode = rootNode;
                next.startIndx = rootNode.getMemorizedStringLen();

                // Leaf
                if (next.children == null || next.children.size() == 0) {
                    String str = next.toString();
                    rootNode.addToMemorizedString(next.getType() != TSParser.StringLiteral ? str.toLowerCase() : str);
                    next.endIndx = rootNode.getMemorizedStringLen();
                    stack.pop();
                    continue;
                }
                ...
                if (!next.isNil()) {
                    rootNode.addToMemorizedString("(");
                    String str = next.toString();
                    rootNode.addToMemorizedString((next.getType() == TSParser.StringLiteral || null == str) ? str : str.toLowerCase());
                    rootNode.addToMemorizedString(" ");
                }
