...
public class TsfileDBConfig {
	...
	/**
	 * When the total number of write ahead log in the file and memory 
	 * reaches the specified size,  all the logs are compressed and 
	 * the unused logs are removed
	 * Increase this value, it will lead to short write pause. 
	 * Decrease this value, it will increase IO and CPU consumption
	 */
	public int walCleanupThreshold = 500000;
	
	/**
	 * When a certain amount ofwrite ahead log is reached, 
	 * it will be flushed to disk. It is possible to lose at most 
	 * flush_wal_threshold operations
	 */
	public int flushWalThreshold = 10000;
	
	/**
	 * The cycle when write ahead log is periodically refreshed 
	...
	/**
	 * The amount of data that is read every time in batches.
	 * In a session, user can set by himself, and it will only 
	 * take effect in current session.
	 */
