...
public class SessionExample2 {
  ...
  private static final String ROOT_SG1_D3 = "root.sg1.d3";
  private static final String ROOT_SG1 = "root.sg1";
  private static final String LOCAL_HOST = "127.0.0.1";
  public static final String SELECT_D3 = "select * from root.sg1.d3";
  ...
  private static void createTimeseries()
    ...
    //          null);
    //    }
    List<TSDataType> d3DataType =
        Arrays.asList(TSDataType.DATE, TSDataType.TIMESTAMP, TSDataType.BLOB, TSDataType.STRING);
    for (int i = 0; i < d3DataType.size(); i++) {
      String tsPath = ROOT_SG1_D3 + ".s" + (i + 1);
      if (!session.checkTimeseriesExists(tsPath)) {
        session.createTimeseries(
            tsPath, d3DataType.get(i), TSEncoding.PLAIN, CompressionType.SNAPPY);
      }
    }
  }
  ...
  private static void insertRecord() throws IoTDBConnectionException, StatementExecutionException {
    ...
    types.add(TSDataType.TIMESTAMP);
    types.add(TSDataType.BLOB);
    ...
    for (long time = 10; time < 20; time++) {
      ...
      values.add(time);
      values.add(new Binary("hahahehe", TSFileConfig.STRING_CHARSET));
      values.add("" + time);
  ...
  private static void insertTablet() throws IoTDBConnectionException, StatementExecutionException {
    ...
    Tablet tablet = new Tablet(ROOT_SG1_D3, schemaList, 100);

    // Method 1 to add tablet data
    long timestamp = System.currentTimeMillis();

    for (long row = 10; row < 30; row++) {
      int rowIndex = tablet.rowSize++;
      tablet.addTimestamp(rowIndex, timestamp);
      tablet.addValue(schemaList.get(0).getMeasurementId(), rowIndex, "2024-03-" + row);
      tablet.addValue(schemaList.get(1).getMeasurementId(), rowIndex, timestamp);
      tablet.addValue(schemaList.get(2).getMeasurementId(), rowIndex, "OX'Cafebabe'");
      tablet.addValue(schemaList.get(3).getMeasurementId(), rowIndex, "tdengine");
      timestamp++;
    }
  ...
  private static void query() throws IoTDBConnectionException, StatementExecutionException {
    try (SessionDataSet dataSet = session.executeQueryStatement(SELECT_D3)) {
      System.out.println(dataSet.getColumnNames());
