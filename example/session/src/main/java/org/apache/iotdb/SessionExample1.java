...
public class SessionExample1 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    List<Integer> base = Arrays.asList(100,95,50,70,80);
    List<Integer> range = Arrays.asList(20,20,30,25,30);
    List<String> devices = Arrays.asList(
            "root.hebei.tangshan.lunan._03380912",
            "root.hebei.tangshan.lunan._93741819",
            "root.hebei.tangshan.lunan._24646171",
            "root.shanxi.datong.nanjiao._07328049",
            "root.shanxi.datong.nanjiao._51829241"
    );
    List<String> measurements = new ArrayList<>();
    List<TSDataType> types = new ArrayList<>();
    measurements.add("speed");
    measurements.add("fuel_consumption");
    types.add(TSDataType.FLOAT);
    types.add(TSDataType.FLOAT);
    Random random = new Random();

    for (int i = 0; i < 500; i++) {
      long time = timestamp + i * 3000;
      for (int j = 0; j < devices.size(); j++) {
        List<Object> values = new ArrayList<>();
        values.add((float) (random.nextGaussian()*range.get(j)+base.get(j)));
        values.add((float) (random.nextDouble()*7+3));
        session.insertRecord(devices.get(j), time, measurements, types, values);
      }
