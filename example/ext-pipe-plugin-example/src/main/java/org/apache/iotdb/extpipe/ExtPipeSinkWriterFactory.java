...
//
//== Command Example:
//   CREATE PIPESINK mySink1 AS mySink (address='http://129.168.1.1/abc', user='admin', passwd='admin123', project='project1', table='table1', thread_num='5', batch_size='1000');
...
//  batch_size : the number of operations to get from pipe source each time for 1 thread, default value 100_000.
...
public class ExtPipeSinkWriterFactory implements IExternalPipeSinkWriterFactory {
  ...
  /**
   * Return the provider info of current exe-pipe plugin.
   * In current IoTDB, this information is not important.
   * @return
   */
  @Override
  ...
  /**
   * This method is used to validate the parameters in client CMD. For example: When customer
  ...
  public void validateSinkParams(Map<String, String> sinkParams) throws Exception {
    ...
    //== Here, you may add other checking. Such as, checking whether remote address can be accessed etc.
    //...
  }

  /**
   * After IoTDB create IExternalPipeSinkWriterFactory instance,
   * IoTDB will call this method to let IExternalPipeSinkWriterFactory finish some init work.
   *
   * @param sinkParams Contains the parameters in CMD "CREATE PIPESINK ..."
   */
  @Override
