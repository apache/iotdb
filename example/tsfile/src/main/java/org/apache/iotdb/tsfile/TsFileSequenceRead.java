...
public class TsFileSequenceRead {
  ...
  public static void main(String[] args) throws IOException {
    ...
    try (TsFileSequenceReader reader = new TsFileSequenceReader(filename)) {
      ...
      System.out.println("file magic head: " + reader.readHeadMagicInTsFile());
      System.out.println("file magic tail: " + reader.readTailMagicInIndexFile());
      ...
      byte marker;
      while ((marker = reader.readMarkerInIndexFile()) != MetaMarker.SEPARATOR) {
        switch (marker) {
      ...
      for (int j = 0; j < 100; j++) {
        ...
        for (IChunkMetadata chunkMetadata : timeseriesMetadata.getChunkMetadataList()) {
          ...
          System.out.println("\tchunk type: " + chunkMetadata.getChunkType());
          System.out.println("\tposition: " + reader.position());
          if (chunkMetadata.getDataSize() == 0) {
            // empty value chunk
            System.out.println("\t-- Empty Chunk ");
            break;
          }
          System.out.println(
              "\tChunk Size: " + (chunkMetadata.getDataSize() + chunkMetadata.getSerializedSize()));
          ...
          Decoder valueDecoder =
              Decoder.getDecoderByType(
                  chunkMetadata.getEncodingType(), chunkMetadata.getDataType());
          ...
          while (dataSize > 0) {
            ...
            PageHeader pageHeader =
                reader.readPageHeader(
                    ...
                    (chunkMetadata.getChunkType() & 0x3F) == MetaMarker.CHUNK_HEADER);
            System.out.println("\t\tPage data position: " + reader.position());
            ByteBuffer pageData = reader.readPage(pageHeader, chunkMetadata.getCompressionType());
            System.out.println(
                "\t\tUncompressed page data size: " + pageHeader.getUncompressedSize());
            System.out.println("\t\tCompressed page data size: " + pageHeader.getCompressedSize());
            if ((chunkMetadata.getChunkType() & TsFileConstant.TIME_COLUMN_MASK)
            ...
            } else if ((chunkMetadata.getChunkType() & TsFileConstant.VALUE_COLUMN_MASK)
                == TsFileConstant.VALUE_COLUMN_MASK) { // Value Chunk
              ValuePageReader valuePageReader =
                  new ValuePageReader(
                      pageHeader, pageData, chunkMetadata.getDataType(), valueDecoder);
              ...
              if (chunkMetadata.getChunkType() == MetaMarker.CHUNK_HEADER) {
      ...
      //                ValuePageReader valuePageReader =
      //                    new ValuePageReader(pageHeader, pageData, header.getDataType(),
      // valueDecoder);
      //                TsPrimitiveType[] valueBatch =
      //                    valuePageReader.nextValueBatch(timeBatch.get(pageIndex));
      //                if (valueBatch.length == 0) {
