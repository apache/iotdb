...
public class DeletePartitionHandler extends AbstractRetryHandler
    implements AsyncMethodCallback<TSStatus> {

  private static final Logger LOGGER = LoggerFactory.getLogger(DeletePartitionHandler.class);

  private final List<TSStatus> dataNodeResponseStatus;

  public DeletePartitionHandler(
  ...
  public void onComplete(TSStatus response) {
    if (response.getCode() == TSStatusCode.SUCCESS_STATUS.getStatusCode()) {
      ...
      LOGGER.info("Successfully Delete Partition on DataNode: {}", targetDataNode);
    } else {
      LOGGER.error(
          "Failed to Delete Partition on DataNode {}, {}",
    ...
    dataNodeResponseStatus.add(
        new TSStatus(
            RpcUtils.getStatus(
                ...
                "Delete Partition error on DataNode: {id="
