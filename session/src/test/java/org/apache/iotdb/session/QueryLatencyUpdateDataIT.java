...
public class QueryLatencyUpdateDataIT {
  ...
  public void run() throws IoTDBConnectionException, StatementExecutionException, IOException {
    ...
    long ST;
    String[] bitsPerKeyList =
        //        new String[]{"20","24"};
        //      new String[]{"0","12","16"};
        new String[] {"0", "12", "16", "20", "24"};
    String[] updateList = new String[] {"update049"};
    //        new String[]{"update000","update010","update020","update049"};
    String[] muSigList = new String[] {"21"};
    //        new String[]{"20","21","22","23"};
    String[] aggrList =
        new String[] {"count", "exact_median_kll_stat_single", "exact_median_kll_stat"};
    for (String update : updateList) {
      for (String MiuSig : muSigList) {
        double miu = 2, sig = Double.parseDouble(MiuSig.substring(1));
        System.out.println("\n\nUPDATE:\t" + update + "\tMiu:" + miu + "\tSig:" + sig);

        System.out.print("\t\t");
        for (String aggr : aggrList) System.out.print("\t" + aggr);
        System.out.println();
        for (String bits : bitsPerKeyList) {
          System.out.print("\t" + bits + "\t");
          String device = "root." + update + "_" + bits + "_" + MiuSig + ".d0";
          for (String aggr : aggrList) {
            ...
            for (int i = 0; i < TEST_CASE; i++) {
              ...
              dataSet =
                  session.executeQueryStatement(
                      "select " + aggr + "(s0) from " + device + " where time<" + REVERSE_TIME);
              times.add(new Date().getTime() - ST);
            }
            times.sortThis();
            long totT = 0, cntT = 0;
            for (int i = TEST_CASE / 3; i < TEST_CASE * 2 / 3; i++) {
              totT += times.get(i);
              cntT++;
            }
            //      System.out.println("[DEBUG]" + dataSet.next().getFields().toString());
            System.out.print("\t" + totT / cntT);
          }
