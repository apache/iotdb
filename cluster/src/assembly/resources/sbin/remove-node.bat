...
FOR /F "eol=; tokens=2,2 delims==" %%i IN ('findstr /i "cluster_rpc_ip" %IOTDB_CONF%\iotdb-cluster.properties') DO set ip=%%i
FOR /F "eol=; tokens=2,2 delims==" %%i IN ('findstr /i "internal_meta_port" %IOTDB_CONF%\iotdb-cluster.properties') DO set meta_port=%%i

"%JAVA_HOME%\bin\java" %JAVA_OPTS% %IOTDB_HEAP_OPTS% -cp %CLASSPATH% %IOTDB_JMX_OPTS% %MAIN_CLASS% -r %ip% %meta_port%
goto finally
