...
launch_service()
{
  ...
  metaport=`sed '/^internal_meta_port=/!d;s/.*=//' $IOTDB_CONF/iotdb-cluster.properties`
	exec "$JAVA" $iotdb_parms $IOTDB_JMX_OPTS $iotdb_parms -cp "$CLASSPATH"  "$class" -r $ip $metaport
	return $?
}
