...
public class ClusterNodeConstructorTest {
  ...
  private ClusterNodeConstructor nodeConstructor = new ClusterNodeConstructor(metaGroupMember);
  ...
  public void test() throws StorageEngineException, IOException {
    ...
    Node node = nodeConstructor.construct(expression, context);
    for (int i = 3; i < 8; i++) {
      assertTrue(node.hasNext());
      assertEquals(i, node.next());
    }
  }
}