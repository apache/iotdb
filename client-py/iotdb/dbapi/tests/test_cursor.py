...
def test_cursor():
    with IoTDBContainer("iotdb:dev") as db:
        ...
        cursor.execute("create timeseries root.cursor.temperature with datatype=FLOAT,encoding=RLE")
        cursor.execute("insert into root.cursor(timestamp,temperature) values(1,%(temperature)s)", {"temperature": 0.3})
        cursor.execute("insert into root.cursor(timestamp,temperature) values(2,%(temperature)s)", {"temperature": 0.4})
        ...
        cursor.executemany("insert into root.cursor(timestamp,temperature) values(%(timestamp)s,%(temperature)s)", args)
        cursor.execute("select * from root.cursor")
        count = 7
        actual_count = 0
        for row in cursor.fetchall():
