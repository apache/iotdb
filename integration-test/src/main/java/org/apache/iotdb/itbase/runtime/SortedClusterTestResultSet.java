...
public class SortedClusterTestResultSet implements ResultSet {
  ...
  public SortedClusterTestResultSet(
    ...
    for (Statement st : statements) {
      ...
      cachedResult.add(new ArrayList<>());
    }
  ...
  public boolean next() throws SQLException {
    ...
    boolean result = delegate.requestAllAndCompare();
    if (result) {
      for (List<StringBuilder> list : cachedResult) {
  ...
  public void close() throws SQLException {
    ...
    for (StringBuilder item : cachedResult.get(0)) {
      result.add(item.toString());
      Collections.sort(result);
    }

    for (int i = 1; i < cachedResult.size(); i++) {
      List<String> anotherResult = new ArrayList<>();

      for (StringBuilder item : cachedResult.get(i)) {
        anotherResult.add(item.toString());
      }

      Collections.sort(anotherResult);

      if (!result.equals(anotherResult)) {
        throw new InconsistentDataException(cachedResult, endpoints);
      }
    }
  }
  ...
  public String getString(int columnIndex) throws SQLException {
    ...
    return cachedResultAndReturnOne(delegate.requestAll());
  }
