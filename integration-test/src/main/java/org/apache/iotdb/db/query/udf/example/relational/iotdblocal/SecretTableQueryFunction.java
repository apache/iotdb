...
/** Probes IoTDBLocal query against secret_table for permission integration tests. */
public class SecretTableQueryFunction implements ScalarFunction {
  ...
  public void beforeStart(FunctionArguments arguments, IoTDBLocal local) throws UDFException {
    try (UDFResultSet rs = local.query("SELECT secret FROM secret_table")) {
      while (rs.hasNext()) {
        rs.next();
      }
    }
  ...
  public Object evaluate(Record input) {
    return new Binary(input.getString(0), TSFileConfig.STRING_CHARSET);
  }
}
