...
/**
 * UDAF that calls {@link IoTDBLocal#query(String)} only in {@link #addInput(State, Record,
 * IoTDBLocal)}.
 */
public class LocalQueryUdafInAddInputAggregateFunction implements AggregateFunction {
  ...
  public void addInput(State state, Record input, IoTDBLocal local) {
    ...
    if (!countState.queried) {
      try (UDFResultSet rs = local.query("SELECT COUNT(*) FROM device_limits")) {
        if (rs.hasNext()) {
          countState.extraCount = rs.next().getLong(0);
        }
      } catch (UDFException e) {
        throw new IllegalStateException(e);
      }
      countState.queried = true;
    }
  ...
  public void combineState(State state, State rhs) {
    ...
    left.queried = left.queried || right.queried;
  }
