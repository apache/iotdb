...
package org.apache.iotdb.subscription.it.triple.pullconsumer.time;
...
public class TimeRangePullConsumerDataSet extends AbstractSubscriptionRegressionIT {
  ...
  private String topicName = "topic_TimeRangePullConsumerDataSet";
  ...
  public void setUp() throws Exception {
    super.setUp();
    createDB(database);
    createTopic_s(topicName, pattern, "2024-01-01", "2024-03-31", false);
  ...
  public void do_test()
    ...
    consume_data(consumer, session_dest);
    check_count(
        2, "select count(s_0) from " + device, "Write some real-time data later:" + pattern);
    ...
    check_count(8, "select count(s_0) from " + device, "End time limit data:" + pattern);

    insert_data(1711900798000L); // 2024-03-31 23:59:58+08:00
    consume_data(consumer, session_dest);
    check_count(8, "select count(s_0) from " + device, "End time boundary data:" + pattern);
  }
}
