 ...
 * result: fail
 */
public class LooseAllTsDatasetPushConsumerSnapshot extends AbstractSubscriptionRegressionIT {
  ...
  private static final String topicName = "topic_LooseAllTsDatasetPushConsumerSnapshot";
  ...
  public void do_test()
    ...
    AWAIT.untilAsserted(
        () -> {
          ...
          check_count(9, "select count(s_1) from " + device, "Consumption data: s_1 " + device);
    ...
    AWAIT.untilAsserted(
        () -> {
          check_count(14, "select count(s_0) from " + device, "consume data again: s_0 " + device);
