...
public class IoTDBSinhFunctionTableIT {
    ...
    public void testNormalTransformer() {
        ...
        tableResultSetEqualTest(
                "select time,s2,sinh(s2),s3,sinh(s3),s4,sinh(s4),s5,sinh(s5),s8,sinh(s8) from table1",
                expectedHeader,
                expectedAns,
                DATABASE_NAME);
    }
    ...
    public void testFailTransformer() {
        ...
        tableAssertTestFail(
                "select s2,sinh(s2,1) from table1",
        ...
        tableAssertTestFail(
                "select s10,sinh(s10) from table1",
                TSStatusCode.SEMANTIC_ERROR.getStatusCode()
                        + ": Scalar function sinh only accepts one argument and it must be TimeStamp, Double, Float, Int32 or Int64 data type.",
                DATABASE_NAME);
    }
