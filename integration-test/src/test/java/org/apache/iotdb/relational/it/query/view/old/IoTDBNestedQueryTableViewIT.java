...
public class IoTDBNestedQueryTableViewIT {
  ...
  private static void createTimeSeries() {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      statement.execute("CREATE TIMESERIES root.vehicle2.d1.s1 with datatype=FLOAT,encoding=PLAIN");
      statement.execute(
          "CREATE TIMESERIES root.vehicle2.d1.s2 with datatype=DOUBLE,encoding=PLAIN");
      statement.execute(
          "CREATE TIMESERIES root.vehicle2.d1.empty with datatype=DOUBLE,encoding=PLAIN");

      statement.execute("CREATE TIMESERIES root.likeTest.d1.s1 with datatype=TEXT,encoding=PLAIN");
      statement.execute(
          "CREATE TIMESERIES root.likeTest.d1.s2 with datatype=STRING,encoding=PLAIN");
  ...
  private static void generateData() {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      for (int i = 1; i <= ITERATION_TIMES; ++i) {
        ...
        statement.execute(
            (String.format(
                "insert into root.vehicle2.d1(timestamp,s1,s2) values(%d,%d,%d)", i, i, i)));
      }
      ...
      statement.execute(
          "insert into root.likeTest.d1(timestamp,s1,s2) values(1,'abcdef', '123456')");
      statement.execute(
          "insert into root.likeTest.d1(timestamp,s1,s2) values(2,'_abcdef', '123\\456')");
      statement.execute(
          "insert into root.likeTest.d1(timestamp,s1,s2) values(3,'abcdef%', '123#456')");
    } catch (SQLException throwable) {
      fail(throwable.getMessage());
    }
  }
