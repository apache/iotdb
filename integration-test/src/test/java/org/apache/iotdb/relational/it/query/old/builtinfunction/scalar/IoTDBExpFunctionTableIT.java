...
public class IoTDBExpFunctionTableIT {
  ...
  public void testNormalTransformer() {
    ...
    tableResultSetEqualTest(
        "select time,s2,exp(s2),s3,exp(s3),s4,exp(s4),s5,exp(s5),s8,exp(s8) from table1",
        expectedHeader,
        expectedAns,
        DATABASE_NAME);
  }
  ...
  public void testFailTransformer() {
    ...
    tableAssertTestFail(
        "select s2,exp(s2,1) from table1",
    ...
    tableAssertTestFail(
        "select s10,exp(s10) from table1",
        TSStatusCode.SEMANTIC_ERROR.getStatusCode()
            + ": Scalar function exp only accepts one argument and it must be TimeStamp, Double, Float, Int32 or Int64 data type.",
        DATABASE_NAME);
  }
