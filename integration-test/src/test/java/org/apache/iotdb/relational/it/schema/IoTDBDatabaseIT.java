...
public class IoTDBDatabaseIT {
  ...
  public void testManageDatabase() {
    try (final Connection connection =
      ...
      statement.execute(
          "create database test_prop with (ttl=300, time_partition_origin=2000, time_partition_interval=100000)");
  ...
  public void testShowCreateDatabase() throws SQLException {
    try (final Connection connection =
      ...
      try (final ResultSet resultSet =
        ...
        assertTrue(createDatabaseSQL, createDatabaseSQL.contains("need_last_cache=true"));
        assertFalse(resultSet.next());
      }
    }
  }
      ...
      try (final ResultSet resultSet = statement.executeQuery("select * from databases")) {
        ...
        while (resultSet.next()) {
          if ("information_schema".equals(resultSet.getString(1))) {
            ...
            assertEquals(0, resultSet.getLong(5));
            assertEquals(604800000, resultSet.getLong(6));
            assertEquals(0, resultSet.getInt(7));
            assertTrue(resultSet.getInt(8) >= 1);
            assertEquals(0, resultSet.getInt(9));
            assertTrue(resultSet.getInt(10) >= 2);
            assertTrue(resultSet.getBoolean(11));
          }
          cnt++;
        }
  ...
  public void testDBAuth() throws SQLException {
    ...
    try (final Connection userCon =
      ...
      TestUtils.assertResultSetEqual(
          ...
          "database,ttl(ms),schema_replication_factor,data_replication_factor,time_partition_origin,time_partition_interval,schema_region_group_num,max_schema_region_group_num,data_region_group_num,max_data_region_group_num,need_last_cache,",
          Collections.singleton(
              "information_schema,INF,null,null,null,null,null,null,null,null,false,"));
    }
