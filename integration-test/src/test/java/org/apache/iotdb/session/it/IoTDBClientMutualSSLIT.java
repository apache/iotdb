...
public class IoTDBClientMutualSSLIT {
  ...
  public void tearDown() {
    ...
    try (ITableSession session = newMutualSSLTableSession()) {
      session.executeNonQueryStatement("DROP DATABASE IF EXISTS client_mtls_table");
    } catch (Exception ignored) {
      // ignored
    }
  }
  ...
  public void treeSessionCanConnectWithMutualSSL() throws Exception {
    try (ISession session = newMutualSSLSession()) {
      session.executeNonQueryStatement("CREATE DATABASE root.client_mtls_tree");
      session.executeNonQueryStatement(
          "CREATE TIMESERIES root.client_mtls_tree.d1.s1 WITH DATATYPE=INT32, ENCODING=PLAIN");
      session.executeNonQueryStatement(
          "INSERT INTO root.client_mtls_tree.d1(time, s1) VALUES (1, 11)");

      try (SessionDataSet dataSet =
          session.executeQueryStatement("SELECT s1 FROM root.client_mtls_tree.d1")) {
  ...
  public void sessionPoolCanConnectWithMutualSSL() throws Exception {
    ...
    try {
      ...
      pool.executeNonQueryStatement(
          "CREATE TIMESERIES root.client_mtls_pool.d1.s1 WITH DATATYPE=INT32, ENCODING=PLAIN");
      pool.executeNonQueryStatement(
          "INSERT INTO root.client_mtls_pool.d1(time, s1) VALUES (1, 22)");

      try (SessionDataSetWrapper dataSet =
          pool.executeQueryStatement("SELECT s1 FROM root.client_mtls_pool.d1")) {
        assertTrue(dataSet.hasNext());
        final RowRecord record = dataSet.next();
        assertEquals(1L, record.getTimestamp());
  ...
  public void tableSessionCanConnectWithMutualSSL() throws Exception {
    try (ITableSession session = newMutualSSLTableSession()) {
      session.executeNonQueryStatement("CREATE DATABASE IF NOT EXISTS client_mtls_table");
      session.executeNonQueryStatement("USE client_mtls_table");
      session.executeNonQueryStatement(
          "CREATE TABLE IF NOT EXISTS mtls_table (tag1 STRING TAG, value INT32 FIELD)");
      session.executeNonQueryStatement(
          "INSERT INTO mtls_table(time, tag1, value) VALUES (1, 'tag1', 33)");

      try (SessionDataSet dataSet =
          session.executeQueryStatement("SELECT time, value FROM mtls_table WHERE tag1 = 'tag1'")) {
        assertTrue(dataSet.hasNext());
        final RowRecord record = dataSet.next();
  ...
  public void jdbcCanConnectWithMutualSSL() throws Exception {
    ...
    try (Connection connection =
            DriverManager.getConnection(
                Config.IOTDB_URL_PREFIX + dataNode.getIpAndPortString(), mutualSSLProperties());
        Statement statement = connection.createStatement()) {
      statement.execute("CREATE DATABASE root.client_mtls_jdbc");
      statement.execute(
          "CREATE TIMESERIES root.client_mtls_jdbc.d1.s1 WITH DATATYPE=INT32, ENCODING=PLAIN");
      statement.execute("INSERT INTO root.client_mtls_jdbc.d1(time, s1) VALUES (1, 44)");
  ...
  private static ISession newMutualSSLSession() throws IoTDBConnectionException {
    final DataNodeWrapper dataNode = EnvFactory.getEnv().getDataNodeWrapper(0);
    final Session session =
        new Session.Builder()
            .host(dataNode.getIp())
  ...
  private static ITableSession newMutualSSLTableSession() throws IoTDBConnectionException {
    final DataNodeWrapper dataNode = EnvFactory.getEnv().getDataNodeWrapper(0);
