...
public class IoTDBPipeClusterIT extends AbstractPipeDualTreeModelAutoIT {
  ...
  private void testMachineDowntime(String sink) {
    ...
    try (final SyncConfigNodeIServiceClient client =
      ...
      if (!TestUtils.tryExecuteNonQueriesWithRetry(
          senderEnv,
          Arrays.asList("insert into root.db.d1(time, s1) values (now(), 3)", "flush"), null)) {
        return;
      }
    ...
    try (final SyncConfigNodeIServiceClient client =
      ...
      try (Connection connection = senderEnv.getConnection()) {
        for (int i = 0; i < 100; ++i) {
          if (TestUtils.tryExecuteNonQueryWithRetry(
              senderEnv,
              String.format("insert into root.db.d1(time, s1) values (%s, 1)", i * 1000), connection)) {
            succeedNum++;
          }
        }
      }
  ...
  public void testSenderRestartWhenTransferring() throws Exception {
    ...
    try (Connection connection = senderEnv.getConnection()) {
      for (int i = 0; i < 100; ++i) {
        if (TestUtils.tryExecuteNonQueryWithRetry(
            senderEnv, String.format("insert into root.db.d1(time, s1) values (%s, 1)", i * 1000), connection)) {
          succeedNum++;
        }
      }
    }
