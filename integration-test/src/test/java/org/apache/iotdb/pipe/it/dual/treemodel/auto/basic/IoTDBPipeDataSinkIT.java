...
public class IoTDBPipeDataSinkIT extends AbstractPipeDualTreeModelAutoIT {
  ...
  public void testThriftConnectorWithRealtimeFirstDisabled() throws Exception {
    ...
    try (final SyncConfigNodeIServiceClient client =
      ...
      if (!TestUtils.tryExecuteNonQueriesWithRetry(
          senderEnv,
          Arrays.asList("insert into root.vehicle.d0(time, s1) values (0, 1)", "flush"), null)) {
        return;
      }
      ...
      if (!TestUtils.tryExecuteNonQueriesWithRetry(
          senderEnv,
          Arrays.asList("insert into root.vehicle.d0(time, s1) values (1, 1)", "flush"), null)) {
        return;
      }
  ...
  private void testSinkFormat(final String format) throws Exception {
    ...
    try (final SyncConfigNodeIServiceClient client =
      ...
      if (!TestUtils.tryExecuteNonQueriesWithRetry(
          senderEnv,
          Arrays.asList("insert into root.vehicle.d0(time, s1) values (2, 1)", "flush"), null)) {
        return;
      }
