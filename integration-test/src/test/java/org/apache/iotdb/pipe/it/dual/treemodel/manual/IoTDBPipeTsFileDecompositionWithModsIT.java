...
public class IoTDBPipeTsFileDecompositionWithModsIT extends AbstractPipeDualTreeModelManualIT {
   ...
   *
   * <p>Test scenario: 1. Create database root.db1 with 4 devices: d1 (aligned timeseries), d2
  ...
  public void testTsFileDecompositionWithMods() throws Exception {
    TestUtils.executeNonQueryWithRetry(senderEnv, "CREATE DATABASE root.db1");
    TestUtils.executeNonQueryWithRetry(receiverEnv, "CREATE DATABASE root.db1");

    TestUtils.executeNonQueryWithRetry(
        senderEnv, "CREATE ALIGNED TIMESERIES root.db1.d1(s1 FLOAT, s2 FLOAT, s3 FLOAT)");
    TestUtils.executeNonQueryWithRetry(
        senderEnv, "CREATE TIMESERIES root.db1.d2.s1 WITH DATATYPE=FLOAT");
    TestUtils.executeNonQueryWithRetry(
        senderEnv, "CREATE TIMESERIES root.db1.d2.s2 WITH DATATYPE=FLOAT");
    TestUtils.executeNonQueryWithRetry(
        senderEnv, "CREATE ALIGNED TIMESERIES root.db1.d3(s1 FLOAT, s2 FLOAT, s3 FLOAT)");

    TestUtils.executeNonQueryWithRetry(
        senderEnv,
        "INSERT INTO root.db1.d1(time, s1, s2, s3) ALIGNED VALUES (1, 1.0, 2.0, 3.0), (2, 1.1, 2.1, 3.1), (3, 1.2, 2.2, 3.2), (4, 1.3, 2.3, 3.3), (5, 1.4, 2.4, 3.4)");

    TestUtils.executeNonQueryWithRetry(
        senderEnv,
        "INSERT INTO root.db1.d2(time, s1, s2) VALUES (1, 10.0, 20.0), (2, 10.1, 20.1), (3, 10.2, 20.2), (4, 10.3, 20.3), (5, 10.4, 20.4)");

    TestUtils.executeNonQueryWithRetry(
        senderEnv,
        "INSERT INTO root.db1.d3(time, s1, s2, s3) ALIGNED VALUES (1, 100.0, 200.0, 300.0), (2, 100.1, 200.1, 300.1), (3, 100.2, 200.2, 300.2)");

    TestUtils.executeNonQueryWithRetry(senderEnv, "FLUSH");

    TestUtils.executeNonQueryWithRetry(
        senderEnv, "DELETE FROM root.db1.d1.s1 WHERE time >= 2 AND time <= 4");
    ...
    TestUtils.executeNonQueryWithRetry(
        senderEnv, "CREATE ALIGNED TIMESERIES root.db1.d4(s1 FLOAT, s2 FLOAT, s3 FLOAT)");
    String s = "INSERT INTO root.db1.d4(time, s1, s2, s3) ALIGNED VALUES ";
    ...
    TestUtils.executeNonQueryWithRetry(senderEnv, "DELETE FROM root.db1.d4.s1 WHERE time <= 10000");
    TestUtils.executeNonQueryWithRetry(senderEnv, "DELETE FROM root.db1.d4.s2 WHERE time > 1000");
    TestUtils.executeNonQueryWithRetry(senderEnv, "DELETE FROM root.db1.d4.s3 WHERE time <= 8000");

    TestUtils.executeNonQueryWithRetry(senderEnv, "DELETE FROM root.db1.d2.*");

    TestUtils.executeNonQueryWithRetry(senderEnv, "DELETE FROM root.db1.d3.*");
    ...
    TestUtils.assertDataEventuallyOnEnv(
        senderEnv,
        "SELECT * FROM root.db1.d1 ORDER BY time",
        "Time,root.db1.d1.s3,root.db1.d1.s1,root.db1.d1.s2,",
        results);
    ...
    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv, "SELECT * FROM root.db1.d2 ORDER BY time", "Time,", Collections.emptySet());

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv, "SELECT * FROM root.db1.d3 ORDER BY time", "Time,", Collections.emptySet());

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT s1 FROM root.db1.d1 WHERE time >= 2 AND time <= 4",
        "Time,root.db1.d1.s1,",
        Collections.emptySet());

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(**) FROM root.db1.d4",
        "COUNT(root.db1.d4.s3),COUNT(root.db1.d4.s1),COUNT(root.db1.d4.s2),",
   ...
   * root.db1.d1.** path - test_pipe2: handles data for root.db1.d2.** path - test_pipe3: handles
   * data for root.db1.d3.** path - test_pipe4: handles data for root.db1.d4.** path 10. Verify
  ...
  public void testTsFileDecompositionWithMods2() throws Exception {
    ...
    executeNonQueryWithRetry(
        senderEnv,
        String.format(
            "CREATE PIPE test_pipe1 WITH SOURCE ('mods.enable'='true','path'='root.db1.d1.**') WITH CONNECTOR('ip'='%s', 'port'='%s', 'format'='tablet')",
            receiverEnv.getDataNodeWrapperList().get(0).getIp(),
            receiverEnv.getDataNodeWrapperList().get(0).getPort()));
    ...
    executeNonQueryWithRetry(
        senderEnv,
        String.format(
            "CREATE PIPE test_pipe2 WITH SOURCE ('mods.enable'='true','path'='root.db1.d2.**') WITH CONNECTOR('ip'='%s', 'port'='%s', 'format'='tablet')",
            receiverEnv.getDataNodeWrapperList().get(0).getIp(),
            receiverEnv.getDataNodeWrapperList().get(0).getPort()));
    ...
    executeNonQueryWithRetry(
        senderEnv,
        String.format(
            "CREATE PIPE test_pipe3 WITH SOURCE ('mods.enable'='true','path'='root.db1.d3.**') WITH CONNECTOR('ip'='%s', 'port'='%s', 'format'='tablet')",
            receiverEnv.getDataNodeWrapperList().get(0).getIp(),
            receiverEnv.getDataNodeWrapperList().get(0).getPort()));
    ...
    executeNonQueryWithRetry(
        senderEnv,
        String.format(
            "CREATE PIPE test_pipe4 WITH SOURCE ('mods.enable'='true','path'='root.db1.d4.**') WITH CONNECTOR('ip'='%s', 'port'='%s', 'format'='tablet')",
            receiverEnv.getDataNodeWrapperList().get(0).getIp(),
            receiverEnv.getDataNodeWrapperList().get(0).getPort()));
   ...
   *
   * <p>Test scenario: 1. Create database root.db1 with 1 device: d1 (aligned timeseries with 5
  ...
  public void testTsFileDecompositionWithModsLargeScaleSinglePointDeletion() throws Exception {
    ...
    String s1 = "INSERT INTO root.db1.d1(time, s1) ALIGNED VALUES ";
    ...
    String s2 = "INSERT INTO root.db1.d1(time, s2) ALIGNED VALUES ";
    ...
    String s3 = "INSERT INTO root.db1.d1(time, s3) ALIGNED VALUES ";
    ...
    String s4 = "INSERT INTO root.db1.d1(time, s4) ALIGNED VALUES ";
    ...
    String s5 = "INSERT INTO root.db1.d1(time, s5) ALIGNED VALUES ";
    ...
    TestUtils.assertDataEventuallyOnEnv(
        ...
        "COUNT(root.db1.d1.s3),COUNT(root.db1.d1.s4),COUNT(root.db1.d1.s5),COUNT(root.db1.d1.s1),COUNT(root.db1.d1.s2),",
    ...
    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s1) FROM root.db1.d1",
        "COUNT(root.db1.d1.s1),",
        Collections.singleton("19600,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s2) FROM root.db1.d1",
        "COUNT(root.db1.d1.s2),",
        Collections.singleton("19600,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s3) FROM root.db1.d1",
        "COUNT(root.db1.d1.s3),",
        Collections.singleton("19600,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s4) FROM root.db1.d1",
        "COUNT(root.db1.d1.s4),",
        Collections.singleton("19600,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s5) FROM root.db1.d1",
    ...
    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s1) FROM root.db1.d1 WHERE time >= 10 AND time <= 4000 AND time % 10 = 0",
        "COUNT(root.db1.d1.s1),",
        Collections.singleton("0,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s2) FROM root.db1.d1 WHERE time >= 10010 AND time <= 14000 AND time % 10 = 0",
        "COUNT(root.db1.d1.s2),",
        Collections.singleton("0,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s3) FROM root.db1.d1 WHERE time >= 20010 AND time <= 24000 AND time % 10 = 0",
        "COUNT(root.db1.d1.s3),",
        Collections.singleton("0,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s4) FROM root.db1.d1 WHERE time >= 30010 AND time <= 34000 AND time % 10 = 0",
        "COUNT(root.db1.d1.s4),",
        Collections.singleton("0,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s5) FROM root.db1.d1 WHERE time >= 40010 AND time <= 44000 AND time % 10 = 0",
        "COUNT(root.db1.d1.s5),",
        Collections.singleton("0,"));
    ...
    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s1) FROM root.db1.d1 WHERE time >= 1 AND time < 10",
        "COUNT(root.db1.d1.s1),",
        Collections.singleton("9,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s2) FROM root.db1.d1 WHERE time >= 10001 AND time < 10010",
        "COUNT(root.db1.d1.s2),",
        Collections.singleton("9,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s3) FROM root.db1.d1 WHERE time >= 20001 AND time < 20010",
        "COUNT(root.db1.d1.s3),",
        Collections.singleton("9,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s4) FROM root.db1.d1 WHERE time >= 30001 AND time < 30010",
        "COUNT(root.db1.d1.s4),",
        Collections.singleton("9,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s5) FROM root.db1.d1 WHERE time >= 40001 AND time < 40010",
        "COUNT(root.db1.d1.s5),",
        Collections.singleton("9,"));
    ...
    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s1) FROM root.db1.d1 WHERE time > 4000 AND time <= 20000",
        "COUNT(root.db1.d1.s1),",
        Collections.singleton("16000,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s2) FROM root.db1.d1 WHERE time > 14000 AND time <= 30000",
        "COUNT(root.db1.d1.s2),",
        Collections.singleton("16000,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s3) FROM root.db1.d1 WHERE time > 24000 AND time <= 40000",
        "COUNT(root.db1.d1.s3),",
        Collections.singleton("16000,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s4) FROM root.db1.d1 WHERE time > 34000 AND time <= 50000",
        "COUNT(root.db1.d1.s4),",
        Collections.singleton("16000,"));

    TestUtils.assertDataEventuallyOnEnv(
        receiverEnv,
        "SELECT COUNT(s5) FROM root.db1.d1 WHERE time > 44000 AND time <= 60000",
        "COUNT(root.db1.d1.s5),",
        Collections.singleton("16000,"));
