...
public class IoTDBPipePermissionIT extends AbstractPipeTableModelDualManualIT {
  ...
  public void testSinkPermission() {
    ...
    try (final Connection connection = senderEnv.getConnection(BaseEnv.TABLE_SQL_DIALECT);
        final Statement statement = connection.createStatement()) {
      statement.execute(
          "create pipe a2b with source ('database'='test1') with processor('processor'='rename-database-processor', 'processor.new-db-name'='test') with sink ('sink'='write-back-sink')");
    } catch (final SQLException e) {
      e.printStackTrace();
      fail("Create pipe without user shall succeed if use the current session");
    }
