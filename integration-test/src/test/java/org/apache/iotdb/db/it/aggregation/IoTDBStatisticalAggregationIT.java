...
public class IoTDBStatisticalAggregationIT {

  private static final String MULTI_INPUT_TYPE_ERROR =
  ...
  private static final String MOMENT_TYPE_ERROR =
      "Aggregate functions [SKEWNESS, KURTOSIS] only support "
          + "numeric data types [INT32, INT64, FLOAT, DOUBLE, TIMESTAMP]";

  private static final String ARG_ERROR = "Error size of input expressions";
  ...
  public void testCorrWithUnsupportedTypesAndWrongArity() {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      try {
        ...
        Assert.assertTrue(e.getMessage(), e.getMessage().contains(MULTI_INPUT_TYPE_ERROR));
      }
      ...
      try {
        ...
        Assert.assertTrue(e.getMessage(), e.getMessage().contains(ARG_ERROR));
      }
    } catch (Exception e) {
      e.printStackTrace();
      fail(e.getMessage());
  ...
  public void testMomentsWithUnsupportedTypesAndWrongArity() {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      try {
        ...
        Assert.assertTrue(e.getMessage(), e.getMessage().contains(MOMENT_TYPE_ERROR));
      }
