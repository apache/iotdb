...
public class IoTDBGroupByFilterIT {
  ...
  @Test
  public void groupByFilterTest1() {
    String[][] res =
        new String[][] {
          {"1", "2", "1.0", "2", "16.0"},
          {"5", "7", "2.0", "3", "36.0"},
    ...
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=false)";
    normalTestWithEndTime(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=false)";
    normalTest(res, sql2);
  }

  @Test
  public void groupByFilterTest2() {
    String[][] res =
        new String[][] {
          {"1", "2", "1.0", "2", "16.0"},
    ...
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=true)";
    normalTestWithEndTime(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=true)";
    normalTest(res, sql2);
  }

  @Test
  public void groupByFilterTest3() {
    String[][] res =
        new String[][] {
          {"5", "2500000000", "2499999995.0", "9", "100.0"},
        };
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=3,ignoreNull=true)";
    normalTestWithEndTime(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=3,ignoreNull=true)";
    normalTest(res, sql2);
  }

  @Test
  public void groupByFilterTest4() {
    String[][] res =
        new String[][] {
          {"5", "7", "2.0", "3", "36.0"},
        };
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=3,ignoreNull=false)";
    normalTestWithEndTime(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=3,ignoreNull=false)";
    normalTest(res, sql2);
  }
  ...
  @Test
  public void groupByFilterTestAlignByDevice() {
    String[][] res =
        new String[][] {
          {"1", "2", "1.0", "2", "16.0"},
          {"5", "2500000000", "2499999995.0", "9", "100.0"}
    ...
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.** group by filter(charging_status=1,KEEP>=2,ignoreNull=true) align by device";
    normalTestWithEndTimeAlignByDevice(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.** group by filter(charging_status=1,KEEP>=2,ignoreNull=true) align by device";
    normalTestAlignByDevice(res, sql2);
  }

  @Test
  public void groupByFilterTestWithHaving() {
    String[][] res =
        new String[][] {
          {"9", "10", "1.0", "2", "60.0"},
          {"1900000000", "2000000000", "100000000.0", "2", "70.0"},
    ...
    String sql =
        "select __endTime,max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=false) having last_value(soc)>50";
    normalTestWithEndTime(res, sql);
    String sql2 =
        "select max_time(charging_status) - min_time(charging_status),count(vehicle_status),last_value(soc) from root.sg.beijing.car01 group by filter(charging_status=1,KEEP>=2,ignoreNull=false) having last_value(soc)>50";
    normalTest(res, sql2);
  }

  @Test
  public void groupByFilterFirstValueTest() {
    String[][] res = new String[][] {{"5", "7", "18.0"}};
    String sql =
        "select __endTime,first_value(soc) from root.sg.beijing.car01 group by filter(charging_status!=0,KEEP>2,ignoreNull=false)";
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
