...
public class IoTDBTemplatePermissionIT {
  ...
  private static void cleanupAfterTest() {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      executeQuietly(statement, "DELETE DATABASE root.sg2");
      executeQuietly(statement, "DROP DEVICE TEMPLATE t1");
      executeQuietly(statement, "REVOKE ALL ON root.** FROM USER test");
    } catch (SQLException ignored) {
      // ignore
    }
  }
