...
public class IoTDBMaxByIT {
  ...
  public void maxMinByTimeTest() {
    String[] expectedHeader =
        new String[] {"max_by(Time, root.db.d1.s1 + 1)", "min_by(Time, root.db.d1.s1 + 1)"};
    String[] retArray = new String[] {"1,1,"};
    resultSetEqualTest(
        "select max_by(time, s1+1), min_by(time, s1+1)  from root.db.*", expectedHeader, retArray);

    expectedHeader = new String[] {"max_by(Time, root.db.d1.s1)", "min_by(Time, root.db.d1.s1)"};
    resultSetEqualTest(
        "select max_by(time, s1), min_by(time, s1) from root.db.* where s1>0",
        expectedHeader,
        retArray);
  }
