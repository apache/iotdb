...
public class IoTDBMetadataFetchIT extends AbstractSchemaIT {
  ...
  //            "CREATE TIMESERIES root.ln.wf01.wt01.status WITH DATATYPE = BOOLEAN, ENCODING =
  // PLAIN",
  //            "CREATE TIMESERIES root.ln.wf01.wt01.temperature WITH DATATYPE = FLOAT, ENCODING =
  // RLE, "
  //                + "compressor = SNAPPY, 'MAX_POINT_NUMBER' = '3' ",
  //            "CREATE ALIGNED TIMESERIES root.ln.wf01.wt02(s1 INT32, s2 DOUBLE)",
  //            "CREATE TIMESERIES root.ln1.wf01.wt01.status WITH DATATYPE = BOOLEAN, ENCODING =
  // PLAIN",
  //            "CREATE TIMESERIES root.ln1.wf01.wt01.temperature WITH DATATYPE = FLOAT, ENCODING =
  // RLE, "
  //                + "compressor = SNAPPY, 'MAX_POINT_NUMBER' = '3'",
  //            "CREATE TIMESERIES root.ln2.wf01.wt01.status WITH DATATYPE = BOOLEAN, ENCODING =
  // PLAIN",
  //            "CREATE TIMESERIES root.ln2.wf01.wt01.temperature WITH DATATYPE = FLOAT, ENCODING =
  // RLE, "
  //                + "compressor = SNAPPY, 'MAX_POINT_NUMBER' = '3'"
  //          };
  //
  ...
  //        Statement statement = connection.createStatement()) {
  //      String[] sqls =
  //          new String[] {"show devices root.ln.**", "show devices
  // root.ln.wf01.wt01.temperature"};
  //      Set<String>[] standards =
  //          new Set[] {
  ...
  //  @Ignore(
  //      value =
  //          "Old IoTDB service not support 'count timeseries by tag',Waiting for the old IoTDB to
  // be removed.")
  //  public void showCountTimeSeriesWithTag() throws SQLException {
  //    try (Connection connection = EnvFactory.getEnv().getConnection();
  //        Statement statement = connection.createStatement()) {
  //      statement.execute(
  //          "create timeseries root.sg.d.s1 with datatype=FLOAT, encoding=RLE, compression=SNAPPY
  // tags('tag1'='v1', 'tag2'='v2')");
  //      statement.execute(
  //          "create timeseries root.sg1.d.s1 with datatype=FLOAT, encoding=RLE, compression=SNAPPY
  // tags('tag1'='v1')");
  //      String[] sqls =
  //          new String[] {
  ...
  //            new HashSet<>(Arrays.asList("root.ln.wf01,1,", "root.ln1.wf01,1,",
  // "root.ln2.wf01,1,")),
  //          };
  //      for (int n = 0; n < sqls.length; n++) {
  //        String sql = sqls[n];
  ...
  //        Statement statement = connection.createStatement()) {
  //      statement.execute(
  //          "create timeseries root.sg.d.status with datatype=FLOAT, encoding=RLE,
  // compression=SNAPPY tags('tag1'='v1', 'tag2'='v2')");
  //      statement.execute(
  //          "create timeseries root.sg1.d.status with datatype=FLOAT, encoding=RLE,
  // compression=SNAPPY tags('tag1'='v1')");
  //      String[] sqls =
  //          new String[] {
  ...
  //        Statement statement = connection.createStatement()) {
  //      statement.execute(
  //          "create aligned timeseries root.sg.d(s1(alias1) int32 tags('tag1'='v1', 'tag2'='v2'),
  // s2 double attributes('attr3'='v3'))");
  //      String[] expected =
  //          new String[] {
  //
  ...
  //            "CREATE TIMESERIES root.sg1.d0.s2 WITH DATATYPE=INT32,ENCODING=PLAIN,LOSS=SDT,
  // COMPDEV=0.01, COMPMINTIME=2, COMPMAXTIME=15"
  //          };
  //      for (String sql : sqls) {
  //        statement.execute(sql);
  //      }
