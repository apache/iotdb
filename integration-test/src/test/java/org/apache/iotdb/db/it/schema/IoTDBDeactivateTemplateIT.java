...
public class IoTDBDeactivateTemplateIT extends AbstractSchemaIT {
  ...
  private void prepareTemplate() throws SQLException {
    ...
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      statement.execute("SET DEVICE TEMPLATE t2 TO root.db3");
      statement.execute("SET DEVICE TEMPLATE t2 TO root.db4");

      for (int j = 1; j <= 4; j++) {
        statement.execute(String.format("create timeseries of device template on root.db%d.d1", j));
      }
    }
  }
  ...
  public void deactivateTemplateAndReactivateTest() throws Exception {
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      statement.execute("DEACTIVATE DEVICE TEMPLATE t1 FROM root.db1.d1");
  ...
  public void deactivateTemplateCrossSchemaRegionTest() throws Exception {
    String insertSql = "insert into root.db1.d%d(time, s1, s2) values(%d, %d, %d)";
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      for (int i = 1; i <= 4; i++) {
        for (int j = 1; j <= 4; j++) {
      ...
      statement.execute("DEACTIVATE DEVICE TEMPLATE FROM root.db1.*");
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db1.**")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
        Assert.assertEquals(1, resultSetMetaData.getColumnCount());
        Assert.assertFalse(resultSet.next());
  ...
  public void deactivateTemplateCrossDatabaseTest() throws Exception {
    String insertSql = "insert into root.db%d.d2(time, s1, s2) values(%d, %d, %d)";
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      for (int i = 1; i <= 4; i++) {
        for (int j = 1; j <= 2; j++) {
      ...
      int cnt = 0;
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db1.**,root.db2.**")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
      ...
      statement.execute("DEACTIVATE DEVICE TEMPLATE FROM root.**, root.db1.*");
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db1.**,root.db2.**")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
        Assert.assertEquals(1, resultSetMetaData.getColumnCount());
        Assert.assertFalse(resultSet.next());
  ...
  public void deactivateTemplateWithMultiPatternTest() throws Exception {
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      statement.execute("DEACTIVATE DEVICE TEMPLATE t1 FROM root.db1.d1, root.db2.*");
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db1.*, root.db2.*")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
        Assert.assertEquals(1, resultSetMetaData.getColumnCount());
        Assert.assertFalse(resultSet.next());
  ...
  public void deactivateNoneUsageTemplateTest() throws Exception {
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      try {
        statement.execute("DEACTIVATE DEVICE TEMPLATE t1 FROM root.db5.d1");
        Assert.fail();
      } catch (SQLException e) {
        Assert.assertEquals(
            TSStatusCode.TEMPLATE_NOT_SET.getStatusCode()
                + ": Device Template t1 is not set on any prefix path of [root.db5.d1]",
            e.getMessage());
      }

      statement.execute("CREATE DATABASE root.db5");
      statement.execute("SET DEVICE TEMPLATE t1 TO root.db5 ");
      try {
        statement.execute("DEACTIVATE DEVICE TEMPLATE t1 FROM root.db5.d1");
        Assert.fail();
      } catch (SQLException e) {
  ...
  public void multiSyntaxTest() throws Exception {
    try (Connection connection = EnvFactory.getEnv().getConnection();
        Statement statement = connection.createStatement()) {
      statement.execute("DELETE TIMESERIES OF DEVICE TEMPLATE t1 FROM root.db1.d1");
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db1.*")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
        Assert.assertEquals(1, resultSetMetaData.getColumnCount());
        Assert.assertFalse(resultSet.next());
      ...
      statement.execute("DEACTIVATE DEVICE TEMPLATE t2 FROM root.db3.d1");
      try (ResultSet resultSet = statement.executeQuery("SELECT * FROM root.db3.*")) {
        ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
        Assert.assertEquals(1, resultSetMetaData.getColumnCount());
        Assert.assertFalse(resultSet.next());
      }
