...
public class IoTDBClusterRestartIT {
  ...
  @Test
  public void changeLeaderWhenUpdateDataNodeTest() throws InterruptedException {
    TShowClusterResp clusterNodes;
    ...
    try (SyncConfigNodeIServiceClient client =
      ...
      // remove seed config node to change leader
      TConfigNodeLocation seedConfigNodeLocation = clusterNodes.getConfigNodeList().get(0);
      client.removeConfigNode(seedConfigNodeLocation);
    } catch (Exception e) {
      e.printStackTrace();
      fail();
    }
  ...
  public void networkPartitionWhenUpdateDataNodeTest() throws InterruptedException {
    // Shutdown all data nodes
    for (int i = 0; i < testDataNodeNum; i++) {
      EnvFactory.getEnv().shutdownDataNode(i);
    }
    ...
    // shutdown seed config node to make Network Partition
