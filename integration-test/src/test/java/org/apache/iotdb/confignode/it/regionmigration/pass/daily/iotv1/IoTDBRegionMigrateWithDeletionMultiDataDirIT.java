...
public class IoTDBRegionMigrateWithDeletionMultiDataDirIT {
  ...
  public void testRegionMigratePreservesDeletionWithMultiDataDirs() throws Exception {
    try (Connection connection = EnvFactory.getEnv().getConnection();
      ...
      Set<Integer> migratedReplicaDataNodeIds = new HashSet<>();
      migratedReplicaDataNodeIds.add(followerId);
      migratedReplicaDataNodeIds.add(destDataNodeId);
      awaitRegionReplicas(statement, dataRegionIdForTest, migratedReplicaDataNodeIds);
      awaitModsVisibleOnReplicas(
          "root.db", dataRegionIdForTest, Collections.singleton(destDataNodeId));
    ...
    for (int dataNodeId : replicaDataNodeIds) {
      DataNodeWrapper dataNodeWrapper =
          EnvFactory.getEnv()
              ...
              .orElseThrow(() -> new AssertionError("DataNode " + dataNodeId + " not found"));
      Awaitility.await()
