...
def debug_dataset():
    ...
    train_dataset, train_cfg = create_forecasting_dataset('window', data_source=train_ds, input_len=input_len, pred_len=pred_len, time_embed=time_embed)
    val_ds = DataSource(type='sql', session=session, sql=sql['val'])
    val_dataset, val_cfg = create_forecasting_dataset('window', data_source=val_ds, input_len=input_len, pred_len=pred_len, time_embed=time_embed)
    test_ds = DataSource(type='sql', session=session, sql=sql['test'])
    test_dataset, test_cfg = create_forecasting_dataset('window', data_source=test_ds, input_len=input_len, pred_len=pred_len, time_embed=time_embed)
