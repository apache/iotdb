...
public class IoTDBSelectIntoIT {
  ...
  //            .createStatement()) {
  //      statement.execute(
  //          "select s1, s2, s3, s4, s5, s6 into pre_${2}_suf.s1, pre_${2}_suf.s2, pre_${2}_suf.s3,
  // pre_${2}_suf.s4, pre_${2}_suf.s5, pre_${2}_suf.s6 from root.sg.d1");
  //
  //      try (ResultSet resultSet =
  //          statement.executeQuery(
  //              "select pre_d1_suf.s1, pre_d1_suf.s2, pre_d1_suf.s3, pre_d1_suf.s4, pre_d1_suf.s5,
  // pre_d1_suf.s6 from root.sg.d1")) {
  //        assertEquals(1 + 6, resultSet.getMetaData().getColumnCount());
  //
  //        for (int i = 0; i < INSERTION_SQLS.length; ++i) {
  //          assertTrue(resultSet.next());
  ...
  //        DriverManager.getConnection(Config.IOTDB_URL_PREFIX + "127.0.0.1:6667/", "root", "root")
  //            .createStatement()) {
  //      statement.execute("select count(s1) into count_s1 from root.sg.d1 group by ([1,
  // 5),1ms);");
  //
  ...
  //            .createStatement()) {
  //      statement.execute(
  //          "select last_value(s1) into gbf_s1 from root.sg.d1 group by ([1, 10),1ms) fill
  // (float[PREVIOUS]);");
  //
  ...
  //            .createStatement()) {
  //      statement.execute(
  //          "select s1 into fill_s1 from root.sg.d1 where time = 10 fill(float [linear, 1ms,
  // 1ms])");
  //
  ...
  //              .getMessage()
  //              .contains(
  //                  "the number of source paths and the number of target paths should be the
  // same"));
  //    }
  //
  //    try (Statement statement =
  ...
  //              .getMessage()
  //              .contains(
  //                  "the x of ${x} should be greater than 0 and equal to or less than <level> or
  // the length of queried path prefix."));
  //    }
  //
  //    try (Statement statement =
  //        DriverManager.getConnection(Config.IOTDB_URL_PREFIX + "127.0.0.1:6667/", "root", "root")
  ...
  //        DriverManager.getConnection(Config.IOTDB_URL_PREFIX + "127.0.0.1:6667/", "root", "root")
  //            .createStatement()) {
  //      statement.execute("select s1, s2 into ${1}.s1, ${2}.s1 from root.sg.d1 order by time
  // desc");
  //      fail();
  //    } catch (SQLException throwable) {
