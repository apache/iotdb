...
public class MergeTaskTest extends MergeTest {
  ...
  public void testMerge() throws Exception {
    ...
    IndependenceMergeTask mergeTask =
        new IndependenceMergeTask(new MergeResource(seqResources, unseqResources),
            tempSGDir.getPath(),
  ...
  public void mergeWithDeletionTest() throws Exception {
    ...
    TsFileResource[] newResource = new TsFileResource[1];
    IndependenceMergeTask mergeTask =
        new IndependenceMergeTask(new MergeResource(seqResources, unseqResources.subList(0, 1)),
            tempSGDir.getPath(),
            (k, v, l, n) -> {
              try {
    ...
    QueryContext context = new QueryContext();
    Path path = new Path(deviceIds[0], measurementSchemas[1].getMeasurementId());
