...
public class TTLTest {
  ...
  public void testSetMetaTTL() throws IOException, MetadataException {
    ...
    StorageGroupMNode mNode = IoTDB.metaManager.getBelongedStorageGroupMNode(MetaUtils.splitPathByDot(sg1));
    assertEquals(ttl, mNode.getDataTTL());

    // default ttl
    mNode = IoTDB.metaManager.getBelongedStorageGroupMNode(MetaUtils.splitPathByDot(sg2));
  ...
  public void testParseSetTTL() throws QueryProcessException {
    ...
    assertEquals(sg1, MetaUtils.concatDetachedPathByDot(plan.getDetachedStorageGroups()));
    assertEquals(10000, plan.getDataTTL());

    plan = (SetTTLPlan) planner.parseSQLToPhysicalPlan("UNSET TTL TO " + sg2);
    assertEquals(sg2, MetaUtils.concatDetachedPathByDot(plan.getDetachedStorageGroups()));
