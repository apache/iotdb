...
public class LogReplayerTest {
  ...
  public void test() throws IOException, StorageGroupProcessorException {
    ...
    try {
      ...
      for (int i = 0; i < 5; i++) {
        TVList chunk = memTable.query("device" + i, "sensor" + i, TSDataType.INT64,
            Collections.emptyMap(), Long.MIN_VALUE);
        if (i == 0) {
          assertFalse(chunk.hasNextTimeValuePair());
        } else {
          assertTrue(chunk.hasNextTimeValuePair());
          TimeValuePair timeValuePair = chunk.nextTimeValuePair();
