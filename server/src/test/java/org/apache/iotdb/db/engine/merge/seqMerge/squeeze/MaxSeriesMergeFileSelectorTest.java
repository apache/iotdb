...
import static org.apache.iotdb.db.engine.merge.seqMerge.squeeze.selector.SqueezeMaxSeriesMergeFileSelector.MAX_SERIES_NUM;
...
public class MaxSeriesMergeFileSelectorTest extends MergeTest {
  ...
  public void testFullSelection() throws MergeException, IOException {
    IMergeFileSelector mergeFileSelector = new SqueezeMaxSeriesMergeFileSelector(seqResources,
    ...
    mergeFileSelector = new SqueezeMaxSeriesMergeFileSelector(seqResources.subList(0, 1),
        unseqResources, Long.MAX_VALUE);
    ...
    mergeFileSelector = new SqueezeMaxSeriesMergeFileSelector(seqResources,
    ...
    assertEquals(MAX_SERIES_NUM, selectorContext.getConcurrentMergeNum());
    mergeResource.clear();
  }
  ...
  public void testRestrictedSelection() throws MergeException, IOException {
    IMergeFileSelector mergeFileSelector =
        new SqueezeMaxSeriesMergeFileSelector(seqResources, unseqResources, 400000);
    Pair<MergeResource, SelectorContext> selectRes = mergeFileSelector.selectMergedFiles();
    MergeResource mergeResource = selectRes.left;
    SelectorContext selectorContext = selectRes.right;
    List<TsFileResource> seqSelected = mergeResource.getSeqFiles();
  ...
  public void testRestrictedSelection2() throws MergeException, IOException {
    IMergeFileSelector mergeFileSelector = new SqueezeMaxSeriesMergeFileSelector(seqResources,
        unseqResources, 100000);
    Pair<MergeResource, SelectorContext> selectRes = mergeFileSelector.selectMergedFiles();
    MergeResource mergeResource = selectRes.left;
    SelectorContext selectorContext = selectRes.right;
    List<TsFileResource> unseqSelected = mergeResource.getUnseqFiles();
    assertEquals(unseqResources.subList(0, 1), unseqSelected);
    assertEquals(256, selectorContext.getConcurrentMergeNum());
    mergeResource.clear();
  }
}
