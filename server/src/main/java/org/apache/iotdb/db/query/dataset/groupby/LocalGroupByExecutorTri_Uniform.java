...
public class LocalGroupByExecutorTri_Uniform implements GroupByExecutor {
   ...
   *
   * <p>// iterate futureChunkList ListIterator<ChunkSuit4Tri> itr = futureChunkList.listIterator();
   * while (itr.hasNext()) { ChunkSuit4Tri chunkSuit4Tri = itr.next(); ChunkMetadata chunkMetadata =
   * chunkSuit4Tri.chunkMetadata; long chunkMinTime = chunkMetadata.getStartTime(); long
   * chunkMaxTime = chunkMetadata.getEndTime(); if (chunkMaxTime < curStartTime) { // the chunk
   * falls on the left side of the current M4 interval Ii itr.remove(); } else if (chunkMinTime >=
   * curEndTime) { // the chunk falls on the right side of the current M4 interval Ii, // and since
   * futureChunkList is ordered by the startTime of chunkMetadata, // the loop can be terminated
   * early. break; } else if (chunkMaxTime < curEndTime) { // this chunk is not related to buckets
   * later currentChunkList.add(chunkSuit4Tri); itr.remove(); } else { // this chunk is overlapped
   * with the right border of the current bucket currentChunkList.add(chunkSuit4Tri); // still keep
  ...
  public List<AggregateResult> calcResult(
    ...
    for (int b = 0; b < N1; b++) {
      ...
      calculateFirst(currentChunkList, localCurStartTime, localCurEndTime, series);
      */
    }
  ...
  private void calculateFirst(
    ...
    for (ChunkSuit4Tri chunkSuit4Tri : currentChunkList) {
      ...
      if (canUseStatistics(chunkSuit4Tri, curStartTime, curEndTime)) {
        ...
        int i;
        for (i = chunkSuit4Tri.lastReadPos; i < chunkSuit4Tri.globalEndInList; i++) {
          IOMonitor2.DCP_D_getAllSatisfiedPageData_traversedPointNum++;
          long timestamp = timeSeries.data.get(i).getTimestamp();
