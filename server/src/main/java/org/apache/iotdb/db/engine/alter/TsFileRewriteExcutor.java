...
public class TsFileRewriteExcutor {
    ...
    public TsFileRewriteExcutor(TsFileResource tsFileResource, TsFileResource targetTsFileResource, PartialPath fullPath, TSEncoding curEncoding, CompressionType curCompressionType, long timePartition, boolean sequence) {
    ...
    private void rewriteNotAligned(String device, TsFileSequenceReader reader, TsFileIOWriter writer) throws IOException {
        ...
        measurementMap.forEach((measurementId, chunkMetadatas) -> {
            if(chunkMetadatas == null || chunkMetadatas.isEmpty()) {
                logger.warn("[alter timeseries] empty measurement({})", measurementId);
                return;
            }
            // target chunk writer
            ChunkWriterImpl chunkWriter = new ChunkWriterImpl(new MeasurementSchema(
                    measurementId,
    ...
    private void writeTimeAndValueToChunkWriter(ChunkWriterImpl chunkWriter, TimeValuePair timeValuePair) {
