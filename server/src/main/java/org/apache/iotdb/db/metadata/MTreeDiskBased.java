...
public class MTreeDiskBased implements MTreeInterface {
  ...
  public MeasurementMNode createTimeseries(
    ...
    synchronized (this) {
      MNode child = metadataDiskManager.getChild(cur, leafName);
  ...
  public Pair<PartialPath, MeasurementMNode> deleteTimeseriesAndReturnEmptyStorageGroup(
    ...
    if (deletedNode.getAlias() != null) {
      metadataDiskManager.deleteAliasChild(curNode.getParent(), ((MeasurementMNode) curNode).getAlias());
    }
  ...
  public List<Pair<PartialPath, String[]>> getAllMeasurementSchema(
    ...
    curOffset.set(-1);
    count.set(0);
    findPath(metadataDiskManager.getRoot(), nodes, 1, res, offset.get() != 0 || limit.get() != 0, false, null);
    // avoid memory leaks
    limit.remove();
    offset.remove();
  ...
  public Set<String> getChildNodePathInNextLevel(PartialPath path) throws MetadataException {
    ...
    findChildNodePathInNextLevel(metadataDiskManager.getRoot(), nodes, 1, "", childNodePaths, nodes.length + 1);
  ...
  public Set<String> getChildNodeInNextLevel(PartialPath path) throws MetadataException {
    ...
    findChildNodeInNextLevel(metadataDiskManager.getRoot(), nodes, 1, "", childNodes, nodes.length + 1);
    return childNodes;
  }
  ...
  public List<ShowDevicesResult> getDevices(ShowDevicesPlan plan) throws MetadataException {
    ...
    curOffset.set(-1);
    count.set(0);
    findDevices(metadataDiskManager.getRoot(), nodes, 1, devices, offset.get() != 0 || limit.get() != 0);
    // avoid memory leaks
    limit.remove();
    offset.remove();
    curOffset.remove();
