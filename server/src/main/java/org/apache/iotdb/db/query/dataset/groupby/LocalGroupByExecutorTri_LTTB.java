...
public class LocalGroupByExecutorTri_LTTB implements GroupByExecutor {
  ...
  public LocalGroupByExecutorTri_LTTB(
    ...
    try {
      ...
      futureChunkList.addAll(seriesReader.getAllChunkMetadatas4Tri()); // no need sort here
  ...
  public List<AggregateResult> calcResult(
    ...
    //    int idx = (int) Math.floor((curStartTime - startTime) * 1.0 / interval);
  ...
  public boolean canUseStatistics(ChunkSuit4Tri chunkSuit4Tri, long curStartTime, long curEndTime) {
    ...
    return TP_t >= curStartTime && TP_t < curEndTime && BP_t >= curStartTime && BP_t < curEndTime;
  }
