...
public class ExactQuantilePrKLLPostBestPrAggrResult extends AggregateResult {
  ...
  public ExactQuantilePrKLLPostBestPrAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_QUANTILE_PR_KLL_POST_BEST_PR);
  ...
  private void updateStatusFromData(Object data) {
    ...
    else countOfLargerThanCntR++;
  }

  @Override
  public void startIteration() {
    countOfLessThanCntL = countOfLargerThanCntR = countOfCntL = countOfCntR = 0;
    if (iteration == 0) { // first iteration
      ...
      System.out.println(
          "\t[DEBUG pr_kll_Post_bestPr] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + cntL
  ...
  public void finishIteration() {
    ...
    if (iteration == 1) { // first iteration over
      ...
      if (seqN == -1 && mergeUpdate) seqN = n;
    }
    if (iteration == 2 && mergeUpdate) { // mergeUpdate in first pass. Check real N now.
      n = countOfLessThanCntL + countOfCntL + heapKLL.getN() + countOfCntR + countOfLargerThanCntR;
      seqN = n;
      K1 = (int) Math.floor(QUANTILE * (n - 1) + 1);
      K2 = (int) Math.ceil(QUANTILE * (n - 1) + 1);

      System.out.println("\t\t[DEBUG pr_kll_Post_bestPr] Check Real N in 2nd pass:\t" + n);
    }
    ...
    if (cntK1 <= 0 || cntK2 > countOfCntL + countOfCntR + heapKLL.getN()) { // iteration failed.
      ...
      System.out.println(
          "\t[DEBUG pr_kll_Post_bestPr]\tIter Failed."
      ...
      if (detL >= detR) {
        System.out.println(
            "\t[DEBUG pr_kll_Post_bestPr]\tFail but Answer Found.\tdetL,R:"
                + (detL)
                + ","
                + (detR));
    ...
    if (iteration == 1 && mergeUpdate && seqN < n) {
      // some unSeq data are merged/inserted without checking update.
      long IgnoredUnSeqN = n - seqN;
      cntK1 = Math.max(1, cntK1 - IgnoredUnSeqN / 2 - 1);
      cntK2 = Math.min(n, cntK2 + IgnoredUnSeqN / 2 + 1);
      System.out.println(
          "\t\t[DEBUG pr_kll_Post_bestPr] deal with potential update. unSeqN:\t"
              + IgnoredUnSeqN
              + "\t\tnew_K12:\t"
              + cntK1
              + ","
    ...
    if (deterministic_result.length == 3 || deterministic_result[0] >= deterministic_result[1]) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[DEBUG pr_kll_Post_bestPr]\tAnswer Found."
              + " det_result:"
    ...
    double[] iterate_result =
        heapKLL.getFilter(countOfCntL, countOfCntR, cntL, cntR, cntK1, cntK2, bestPr);
    cntL = (iterate_result[0]);
    cntR = (iterate_result[1]);
    detL = (deterministic_result[0]);
    detR = (deterministic_result[1]);
    if (mergeUpdate && iteration == 1) {
      detL = -Double.MAX_VALUE;
      detR = Double.MAX_VALUE;
    }
    ...
    if (cntL == lastCntL && cntR == lastCntR) {
      ...
      setDoubleValue(ans);
      hasFinalResult = true;
      System.out.println("\t[DEBUG pr_kll_Post_bestPr]\tDANGER!! range_not_updated_after_an_iter.");
      return;
    }

    System.out.println(
        "\t[DEBUG pr_kll_Post_bestPr]\tfinish iter"
            + (iteration - 1)
            + "."
            + " cntK1,2:"
            + cntK1
  ...
  public void updateResultFromStatistics(Statistics statistics) {
    ...
    //    System.out.println(
    //        "\t[DEBUG pr_kll_Post_bestPr] update from statistics:\t"
    //            + "min,max:"
    //            + minVal
    //            + ","
    //            + maxVal
    ...
    // out of range
    if (minVal > cntR) {
      countOfLargerThanCntR += statistics.getCount();
      return;
    }
    if (maxVal < cntL) {
  ...
  public void reset() {
    ...
    n = 0;
    iteration = 0;
    countOfLessThanCntL = countOfLargerThanCntR = 0;
  ...
  public void setAttributes(Map<String, String> attrs) {
    ...
    if (attrs.containsKey("merge_update")) {
      String mu = attrs.get("merge_update");
      this.mergeUpdate = Boolean.parseBoolean(mu);
    }
  ...
  public boolean ignoreUpdateInThisPass() {
    return iteration == 0 && mergeBufferRatio > 0 && mergeUpdate;
  }

  @Override
  public void informUnSeq() {
    seqN = n;
    System.out.println("\t\t[DEBUG pr_kll_Post_bestPr]\tinformUnSeq\tN:" + seqN);
  }
}
