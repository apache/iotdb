...
/** Decide the best Pr with N and Mem in the first iteration. */
public class ExactQuantilePrKLLPrioriBestPrAggrResult extends AggregateResult {
  double bestPr = 0.5;
  ...
  public ExactQuantilePrKLLPrioriBestPrAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_QUANTILE_PR_KLL_PRIORI_BEST_PR);
  ...
  public void startIteration() {
    ...
    if (iteration == 0) { // first iteration
      ...
      System.out.println(
          "\t[DEBUG pr_kll_Priori_bestPr] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + cntL
  ...
  public void finishIteration() {
    ...
    if (iteration == 1) { // first iteration over
      ...
      PrioriBestPrHelper prHelper =
          new PrioriBestPrHelper(
              maxMemoryByte / 8, (int) n, heapKLL.getRelatedCompactNum(), mergeBufferRatio, 1);
      double[] tmp = prHelper.findBestPr(5e-4, 5e-4, 5e-1, (int) n);
      bestPr = tmp[0];
      System.out.println(
          "\t\t\t[[DEBUG pr_kll_Priori_bestPr]]\tAfter First Pass. Pr:"
              + tmp[0]
              + "\t\testiPass:"
              + tmp[1]);
    }
    ...
    if (cntK1 <= 0 || cntK2 > countOfCntL + countOfCntR + heapKLL.getN()) { // iteration failed.
      ...
      System.out.println(
          "\t[DEBUG pr_kll_Priori_bestPr]\tIter Failed."
      ...
      if (detL >= detR) {
        System.out.println(
            "\t[DEBUG pr_kll_Priori_bestPr]\tFail but Answer Found.\tdetL,R:"
                + (detL)
                + ","
                + (detR));
    ...
    if (deterministic_result.length == 3 || deterministic_result[0] >= deterministic_result[1]) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[DEBUG pr_kll_Priori_bestPr]\tAnswer Found."
              + " det_result:"
    ...
    if (cntL == lastCntL && cntR == lastCntR) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[DEBUG pr_kll_Priori_bestPr]\tDANGER!! range_not_updated_after_an_iter.");
      return;
    }

    System.out.println(
        "\t[DEBUG pr_kll_Priori_bestPr]\tfinish iter"
            + (iteration - 1)
            + "."
            + " cntK1,2:"
            + cntK1
  ...
  public void updateResultFromStatistics(Statistics statistics) {
    ...
    //    System.out.println(
    //        "\t[DEBUG pr_kll_Priori_bestPr] update from statistics:\t"
    //            + "min,max:"
    //            + minVal
    //            + ","
    //            + maxVal
