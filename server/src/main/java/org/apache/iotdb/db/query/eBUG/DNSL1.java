...
public class DNSL1 {
    ...
    public static List<Point> reducePoints(List<Point> points, int m, boolean debug) {
        int n = points.size();
        int k = m - 1; // 分段数
        int intervalPts = (int) Math.pow((double) n / k, (double) 2 / 3); // divide batch点数
        ...
        for (int start = 0; start < n; start += intervalPts) {
            ...
            List<Point> sampledForBatch = dynamic_programming(batch, k, DP.ERROR.L1, false);
        ...
        List<Point> finalSampledPoints = dynamic_programming(allSampledPoints, k, DP.ERROR.L1, false);
        if (debug) {
...
//        System.out.println(points.size() * m / (Math.pow(points.size() * 1.0 / (m - 1), 2 / 3.0)));
