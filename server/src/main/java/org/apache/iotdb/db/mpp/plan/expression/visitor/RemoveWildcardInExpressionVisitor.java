...
public class RemoveWildcardInExpressionVisitor extends CartesianProductVisitor<ISchemaTree> {
    ...
    public List<Expression> visitFunctionExpression(FunctionExpression functionExpression, ISchemaTree schemaTree) {
        ...
        for (Expression originExpression : functionExpression.getExpressions()) {
            List<Expression> actualExpressions =
                    process(originExpression, schemaTree);
            ...
            if (functionExpression.isBuiltInAggregationFunctionExpression()) {
                ...
                bindTypeForAggregationNonSeriesInputExpressions(
                        ((FunctionExpression) functionExpression).getFunctionName(), children, extendedExpressions);
                break;
            }
        }
    ...
    @Override
    public List<Expression> visitTimeStampOperand(TimestampOperand timestampOperand, ISchemaTree context) {
        return Collections.singletonList(timestampOperand);
    }

    @Override
    public List<Expression> visitConstantOperand(ConstantOperand constantOperand, ISchemaTree context) {
