...
public class SWAB_AD {
    public static List<Point> seg_bottomUp_maxerror_withTimestamps(
        ...
        while (!triangleHeap.isEmpty() && triangleHeap.peek().area < maxError) { // TODO
            ...
            if (tri.prev != null) {
                ...
                double mc = DP.joint_segment_error(points, tri.prev.indices[0], tri.prev.indices[2], DP.ERRORtype.area);
                tri.prev.area = mc;
                if (debug) {
                    System.out.println(
            ...
            if (tri.next != null) {
                ...
                double mc = DP.joint_segment_error(points, tri.next.indices[0], tri.next.indices[2], DP.ERRORtype.area);
                tri.next.area = mc;
                if (debug) {
                    System.out.println(
    ...
    public static int nextSlidingWindowWithTimestamps(
        ...
        int i = 2; // 从第二个点开始
        while (i < points.size() && DP.joint_segment_error(points, 0, i, DP.ERRORtype.area) < maxError) {
            i++; // 窗口扩大
        }
