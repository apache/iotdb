...
public class ExactQuantilePrKLLOptStatAggrResult extends AggregateResult {
  ...
  public ExactQuantilePrKLLOptStatAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_QUANTILE_PR_KLL_OPT_STAT);
  ...
  public void startIteration() {
    ...
    if (iteration == 0) { // first iteration
      ...
      heapKLL = new KLLSketchLazyExact(maxMemoryByte);
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_stat] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + longToResult(cntL)
  ...
  public void finishIteration() {
    ...
    if (deterministic_result.length == 3 || deterministic_result[0] >= deterministic_result[1]) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_stat]\tAnswer Found."
              + " det_result:"
    ...
    if (cntL == lastCntL && cntR == lastCntR) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_stat]\tDANGER!! range_not_updated_after_an_iter.");
      return;
    }

    System.out.println(
        "\t[ExactQuantile DEBUG pr_kll_opt_stat]\tfinish iter"
            + (iteration - 1)
            + "."
            + " cntK1,2:"
            + cntK1
  ...
  public void updateResultFromStatistics(Statistics statistics) {
    ...
    //    System.out.println(
    //        "\t[ExactQuantile DEBUG pr_kll_opt_stat] update from statistics:\t"
    //            + "min,max:"
    //            + minVal
    //            + ","
    //            + maxVal
