...
public abstract class AbstractQueryProcessExecutor implements IQueryProcessExecutor {
    ...
    @Override
    public QueryDataSet processQuery(PhysicalPlan queryPlan, QueryContext context) throws IOException,
            StorageEngineException, PathErrorException, QueryFilterOptimizationException, ProcessorException {
        ...
        } else if (queryPlan instanceof ShowVersionPlan) {
            return processShowVersionQuery((ShowVersionPlan) queryPlan);
        } else {
            throw new ProcessorException(String.format("Unrecognized query plan %s", queryPlan));
        }
    }

    private QueryDataSet processShowVersionQuery(ShowVersionPlan queryPlan) {
        ...
        sg.setBinaryV(new Binary(VERSION));
    ...
    protected abstract QueryDataSet processAuthorQuery(AuthorPlan plan, QueryContext context) throws ProcessorException;

    private QueryDataSet processDataQuery(QueryPlan queryPlan, QueryContext context) throws StorageEngineException,
            QueryFilterOptimizationException, PathErrorException, ProcessorException, IOException {
        ...
        if (queryPlan instanceof GroupByPlan) {
            ...
            return groupBy(deduplicatedPaths, deduplicatedAggregations, groupByPlan.getExpression(),
                    groupByPlan.getUnit(), groupByPlan.getOrigin(), groupByPlan.getIntervals(), context);
        }

        if (queryPlan instanceof AggregationPlan) {
            List<String> deduplicatedAggregations = new ArrayList<>();
            deduplicate(queryPlan.getPaths(), queryPlan.getAggregations(), deduplicatedPaths, deduplicatedAggregations);
            return aggregate(deduplicatedPaths, deduplicatedAggregations, queryPlan.getExpression(), context);
        }

        if (queryPlan instanceof FillQueryPlan) {
            FillQueryPlan fillQueryPlan = (FillQueryPlan) queryPlan;
            deduplicate(queryPlan.getPaths(), deduplicatedPaths);
            return fill(deduplicatedPaths, fillQueryPlan.getQueryTime(), fillQueryPlan.getFillType(), context);
        }
     ...
     */
    private void deduplicate(List<Path> paths, List<String> aggregations, List<Path> deduplicatedPaths,
        ...
        if (paths.size() != aggregations.size()) {
            throw new ProcessorException("The size of the path list does not equal that of the aggregation list.");
        }
     ...
     * Note that the deduplication strategy must be consistent with that of IoTDBQueryResultSet.
     */
    private void deduplicate(List<Path> paths, List<Path> deduplicatedPaths) throws ProcessorException {
