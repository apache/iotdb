...
public class ExactQuantilePrKLLOptSummaryAggrResult extends AggregateResult {
  ...
  public ExactQuantilePrKLLOptSummaryAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_QUANTILE_PR_KLL_OPT_SUMMARY);
  ...
  public void startIteration() {
    ...
    if (iteration == 0) { // first iteration
      ...
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_summary] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + longToResult(cntL)
  ...
  public void finishIteration() {
    ...
    if (cntK1 <= 0 || cntK2 > heapKLL.getN()) { // iteration failed.
      ...
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_summary]\tIter Failed."
      ...
      if (detL >= detR) {
        System.out.println(
            "\t[ExactQuantile DEBUG pr_kll_opt_summary]\tFail but Answer Found.\tdetL,R:"
                + longToResult(detL)
                + ","
                + longToResult(detR));
    ...
    if (deterministic_result.length == 3 || deterministic_result[0] >= deterministic_result[1]) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_summary]\tAnswer Found."
              + " det_result:"
    ...
    if (cntL == lastCntL && cntR == lastCntR) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_opt_summary]\tDANGER!! range_not_updated_after_an_iter.");
      return;
    }

    System.out.println(
        "\t[ExactQuantile DEBUG pr_kll_opt_summary]\tfinish iter"
            + (iteration - 1)
            + "."
            + " cntK1,2:"
            + cntK1
