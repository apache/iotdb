  ...
  private List<ChunkMetaData> loadSatisfiedChunkMetadatas(TsFileResource resource)
    ...
    for (ChunkMetaData data : currentChunkMetaDataList) {
      if (data.getChunkLoader() == null) {
          TsFileSequenceReader tsFileReader = QueryResourceManager.getInstance()
                  .getTsFileReader(resource.getFile().getAbsolutePath());
          data.setChunkLoader(new ChunkLoaderImpl(tsFileReader));
      }
    }
