...
import org.apache.iotdb.db.utils.datastructure.HashMapOpenCacheForQuantile;
...
public class Optimized_5_MedianAggrResult extends AggregateResult {
  ...
  private HashMapOpenCacheForQuantile hashMap;
  ...
  public Optimized_5_MedianAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(TSDataType.DOUBLE, AggregationType.EXACT_MEDIAN_OPT_5);
  ...
  public void startIteration() {
    ...
    if (bitsCounted == 0) { // first iteration
      if (bitsOfDataType == 32) hashMap = new HashMapOpenCacheForQuantile(bitsOfDataType, 31);
      else hashMap = new HashMapOpenCacheForQuantile(bitsOfDataType, 16);
    } else {
  ...
  public void finishIteration() {
    ...
    // hashMapBits:"+hashMap.getRemainingBits());
    if (cnt == 0) {
      hasFinalResult = true;
      return;
    }
    ...
    prefixOfMedian1 |= iterationResult.get(0) << (bitsConcerned - hashMap.getRemainingBits());
    K1 -= iterationResult.get(1);
    prefixOfMedian2 |= iterationResult.get(2) << (bitsConcerned - hashMap.getRemainingBits());
    K2 -= iterationResult.get(3);
    bitsCounted += hashMap.getRemainingBits();
