...
public class TSServiceImpl implements TSIService.Iface, ServerContext {
    ...
    public static TSDataType getSeriesType(String path) throws PathErrorException {
        ...
        try {
            ...
            if (isAllSuccessful) {
                return getTSBatchExecuteStatementResp(
                        getStatus(TSStatusCode.SUCCESS_STATUS, "Execute batch statements successfully"), result);
            } else {
                return getTSBatchExecuteStatementResp(
            ...
            return getTSBatchExecuteStatementResp(getStatus(TSStatusCode.EXECUTE_STATEMENT_ERROR, e.getMessage()),
                    null);
        } finally {
    ...
    private boolean executeStatementInBatch(String statement, StringBuilder batchErrorMessage, List<Integer> result) {
        try {
            ...
            String errMessage = String.format(
                    "Fail to generate physical plan and execute for statement " + "%s because %s", statement,
                    e.getMessage());
        ...
        try {
            ...
            if (execSetConsistencyLevel(statement)) {
                return getTSExecuteStatementResp(
                        getStatus(TSStatusCode.SUCCESS_STATUS, "Execute set consistency level successfully"));
            }
            ...
            return getTSExecuteStatementResp(
                    getStatus(TSStatusCode.SQL_PARSE_ERROR, "Statement format is not right: " + e.getMessage()));
        } catch (Exception e) {
    ...
    private TSExecuteStatementResp executeQueryStatement(String statement, PhysicalPlan plan) {
        ...
        try {
            ...
            if (plan instanceof AuthorPlan) {
                ...
                resp = executeShowTTL();
            } else if (plan instanceof ShowVersionPlan) {
                resp = executeShowVersion();
            } else {
        ...
        try {
            ...
            if (config.isEnableWatermark() && authorizer.isUserUseWaterMark(username.get())) {
                ...
                if (config.getWatermarkMethodName().equals(IoTDBConfig.WATERMARK_GROUPED_LSB)) {
                    ...
                    throw new UnSupportedDataTypeException(String.format("Watermark method is not supported yet: %s",
    ...
    private QueryDataSet createNewDataSet(String statement, TSFetchResultsReq req) throws PathErrorException,
            QueryFilterOptimizationException, StorageEngineException, ProcessorException, IOException {
    ...
    public TSExecuteBatchStatementResp insertBatch(TSBatchInsertionReq req) {
        ...
        try {
            ...
            batchInsertPlan.setColumns(
                    QueryDataSetUtils.readValuesFromBuffer(req.values, req.types, req.measurements.size(), req.size));
