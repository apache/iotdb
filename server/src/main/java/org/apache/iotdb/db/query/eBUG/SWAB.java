...
public class SWAB {
  public static Object[] prefixSum(List<Point> points) {
    ...
    return new Object[] {
      prefixSumX, prefixSumY, prefixSumX2, prefixSumY2, prefixSumXY, baseX, baseY
    };
  }
  ...
  public static double myLA_withTimestamps(List<Point> points, int lx, int rx) {
    ...
    for (int i = lx; i <= rx; i++) {
      double e =
          (k * points.get(i).x + b - points.get(i).y) * (k * points.get(i).x + b - points.get(i).y);
      tmp += e;
    }
    return tmp;
  }
  ...
  //            List<Point> points, int m, Object[] prefixSum, boolean debug) throws IOException {
  //        if (m <= 2) {
  ...
  //
  //                // 1. 处理旧的tri.prev被标记删除的事情（角色替代）
  //                // triangleHeap.remove(tri.prev); // Avoid using heap.remove(x) as it is O(n)
  // complexity!
  //                tri.prev.markDeleted(); // O(1) 这个点标记为废掉，前后关联都砍断，但是不remove因为那太耗时，只要heap
  // poll到它就跳过就可以
  //
  //                Triangle newPre = new Triangle(tri.prev); // deep copy and inherit connection
  ...
  //
  //                // 1. 处理旧的tri.next被标记删除的事情（角色替代）
  //                // triangleHeap.remove(tri.next); // Avoid using heap.remove(x) as it is O(n)
  // complexity
  //                tri.next.markDeleted(); // O(1) 这个点标记为废掉，前后关联都砍断，但是不remove因为那太耗时，只有poll到它就跳过就可以
  //
  //                Triangle newNext = new Triangle(tri.next); // deep copy and inherit connection
