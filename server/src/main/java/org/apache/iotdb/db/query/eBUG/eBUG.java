...
public class eBUG {
    ...
    public static List<Point> buildEffectiveArea(Polyline lineToSimplify, int e, boolean debug, int m) {
        ...
        if (m > 2) {
            System.out.println("online sampling mode, " + "returning " + m + " sampled points sorted by time in ascending order");
        } else {
            System.out.println("offline precomputation mode, " + "returning each point sorted by dominated significance (DS) in ascending order");
        }
        ...
        while (triangleHeap.size() - fakeCnt > remainNonTerminalPoint) {
            ...
            if (tri.prev != null) {
                ...
                if (e > 0) {
                    ...
                    sig = triArea(lineToSimplify.get(tri.prev.indices[0]), lineToSimplify.get(tri.prev.indices[1]), lineToSimplify.get(tri.prev.indices[2]));
                }
                tri.prev.area = sig;

                if (debug) {
                    System.out.println("sig=" + sig);
                    double tmpTri = triArea(lineToSimplify.get(tri.prev.indices[0]), lineToSimplify.get(tri.prev.indices[1]), lineToSimplify.get(tri.prev.indices[2]));
            ...
            if (tri.next != null) {
                ...
                if (e > 0) {
                    ...
                    sig = triArea(lineToSimplify.get(tri.next.indices[0]), lineToSimplify.get(tri.next.indices[1]), lineToSimplify.get(tri.next.indices[2]));
                }
                tri.next.area = sig;

                if (debug) {
                    System.out.println("sig=" + sig);
                    double tmpTri = triArea(lineToSimplify.get(tri.next.indices[0]), lineToSimplify.get(tri.next.indices[1]), lineToSimplify.get(tri.next.indices[2]));
                    System.out.println("\t" + "tri=" + tmpTri + ", " + ((tmpTri > sig) ? "over-estimated" : "equal/less-estimated"));
                }
