    ...
    while (triangleHeap.size() - fakeCnt > remainNonTerminalPoint) {
      ...
      if (tri.prev != null) {
        ...
        List<Point> pointsForSig =
            findEliminated(lineToSimplify, tri.prev.indices[0], tri.prev.indices[2]);
        if (debug) {
          System.out.println(
              "(2) update point on the left " + lineToSimplify.get(tri.prev.indices[1]));
      ...
      if (tri.next != null) {
        ...
        // e parameter
        List<Point> pointsForSig =
            findEliminated(lineToSimplify, tri.next.indices[0], tri.next.indices[2]);
        if (debug) {
          System.out.println(
              "(2) updating point on the right " + lineToSimplify.get(tri.next.indices[1]));
          System.out.println("3<=cnt<=e+3. cnt=" + pointsForSig.size());
          for (Point point : pointsForSig) {
            System.out.println("\t" + point);
          }
