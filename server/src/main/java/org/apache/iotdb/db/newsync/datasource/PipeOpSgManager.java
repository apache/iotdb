...
public class PipeOpSgManager {
  ...
  private ConcurrentSkipListMap<Long, AbstractPipeOpBlock> opBlockMap =
   ...
   */
  public synchronized void addPipeOpBlock(AbstractPipeOpBlock opBlock) {
  ...
  public synchronized Operation getOperation(long beginIndex, long length) throws IOException {
    ...
    Map.Entry<Long, AbstractPipeOpBlock> opBlockEntry = opBlockMap.floorEntry(beginIndex);
  ...
  public synchronized List<Long> commitData(long committedIndex) {
    ...
    Iterator<Map.Entry<Long, AbstractPipeOpBlock>> iter = opBlockMap.entrySet().iterator();

    while (iter.hasNext()) {
      Map.Entry<Long, AbstractPipeOpBlock> entry = iter.next();
      long nextIndex = entry.getKey() + entry.getValue().getDataCount();
      if (nextIndex <= (committedIndex + 1)) {
        AbstractPipeOpBlock dataSrcEntry = entry.getValue();
