...
import org.apache.iotdb.tsfile.utils.MRLSketchLazy;
...
public class ExactMultiQuantilesMRLAggrResult extends AggregateResult {
  ...
  public ExactMultiQuantilesMRLAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_MULTI_QUANTILES_MRL);
  ...
  public void startIteration() {
    if (iteration == 0) { // first iteration
      ...
      cntWorker = new MRLSketchLazy[rest_multi_quantiles];

      minValL = rest_iterate_result[0][0];
      maxValR = rest_iterate_result[0][1];
      for (int i = 0; i < rest_multi_quantiles; i++) {
        cntWorker[i] = new MRLSketchLazy(maxMemoryByte / rest_multi_quantiles);
  ...
  public void finishIteration() {
    ...
    if (iteration
      ...
      if (toEstimateID.size() > 0) {
        ...
        System.out.print("\t\t[DEBUG multi_MRL]:\t iteration:" + iteration + "\tworstSketch:");
  ...
  public void addSketch(MRLSketchLazy sketch, double minV, double maxV) {
    ...
    if (preComputedSketchSize >= maxMemoryByte / mergeBufferRatio) {
      //      this.sketch.mergeWithTempSpace(preComputedSketch, preComputedSketchMinV,
  ...
  private void mergePrecomputedWhenFinish() {
    //    MRLSketchLazy tmpSketch = heapKLL;
    //    heapKLL = new MRLSketchLazy(maxMemoryByte);
    ...
    if (iteration == 0 && mergeBufferRatio > 0) {
      if (statistics.getType() == DOUBLE) {
        ...
        if (stat.getSummaryNum() > 0) {
          //          for (MRLSketchLazy sketch : stat.getKllSketchList()) {
