...
public class SamplingStatSingleAggrResult extends AggregateResult {
  private TSDataType seriesDataType;
  private int iteration;
  private long pageKLLNum, statNum;
  private long cntL, cntR, lastL;
  private long n, K1, heapN;
  private SamplingHeapForStatMerge sampling;
  ...
  public SamplingStatSingleAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.SAMPLING_STAT_SINGLE);
  ...
  public void startIteration() {
    ...
    if (iteration == 0) { // first iteration
      sampling = new SamplingHeapForStatMerge(maxMemoryByte);
      ...
      pageKLLNum = 0;
      System.out.println(
          "\t[SAMPLING STAT SINGLE DEBUG] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + cntL
  ...
  public void finishIteration() {
    ...
    if (pageKLLNum == 0) { // all in heap
      System.out.println("\t[TDIGEST SINGLE DEBUG]" + " calc by heap only. N:" + sampling.totN);
      //      tDigest.show();
      long ans = sampling.quantile(QUANTILE);
      setDoubleValue(longToResult(ans));
      hasFinalResult = true;
      return;
    }
    System.out.println(
        "\t[TDIGEST SINGLE DEBUG] after merge. heapN:" + sampling.totN + "\tn_true:" + n);
    long ans = sampling.quantile(QUANTILE);
    setDoubleValue(longToResult(ans));
    hasFinalResult = true;
  }
  ...
  public void updateResultFromStatistics(Statistics statistics) {
    ...
    if (iteration == 0) {
      ...
      if (statistics.getType() == DOUBLE) {
        ...
        if (stat.getSummaryNum() > 0) {
          ...
          statNum += 1;
          sampling.merge(stat.getSamplingList());
          //          System.out.println(
    ...
    //    System.out.println(
    //        "\t[SAMPLING STAT SINGLE DEBUG] no KLL in stat. update from statistics:\t"
    //            + "min,max:"
    //            + minVal
    //            + ","
    //            + maxVal
  ...
  public void updateResultFromPageData(
    ...
    //    System.out.print("\t[SAMPLING STAT SINGLE DEBUG]\tupdateResultFromPageData:");
    //    int tmp_tot = 0;
  ...
  public void updateResultUsingTimestamps(
    ...
    //    System.out.print("\t[SAMPLING STAT SINGLE DEBUG]\tupdateResultUsingTimestamps:");
    //    int tmp_tot = 0;
