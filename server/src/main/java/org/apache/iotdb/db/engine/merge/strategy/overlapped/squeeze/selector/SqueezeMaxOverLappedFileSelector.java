...
package org.apache.iotdb.db.engine.merge.strategy.overlapped.squeeze.selector;
...
import org.apache.iotdb.db.engine.merge.strategy.overlapped.BaseOverLappedFileSelector;
...
public class SqueezeMaxOverLappedFileSelector extends BaseOverLappedFileSelector {
  ...
  public SqueezeMaxOverLappedFileSelector(Collection<TsFileResource> seqFiles,
    ...
    super(seqFiles, unseqFiles, dataTTL, storageGroupName, storageGroupSysDir);
  }
  ...
  private List<TsFileResource> selectByUnseq(boolean useTightBound) throws IOException {
    ...
    while (unseqIndex < unseqFiles.size()
      ...
      if (seqSelectedNum != seqFiles.size()) {
        for (Entry<String, Integer> deviceStartTimeEntry : unseqFile.getDeviceToIndexMap()
            .entrySet()) {
