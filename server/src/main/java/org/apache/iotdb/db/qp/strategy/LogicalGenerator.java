...
public class LogicalGenerator {
    ...
    public RootOperator getLogicalPlan(AstNode astNode) throws QueryProcessorException, MetadataErrorException {
        analyze(astNode);
     ...
     */
    private void analyze(AstNode astNode) throws QueryProcessorException, MetadataErrorException {
        ...
        case TOK_GROUPBY_DEVICE:
            ...
            return;
        case TOK_SHOW_VERSION:
            initializedOperator = new ShowVersionOperator();
            return;
        default:
    ...
    private void analyzeWhere(AstNode ast, int tokenIntType, FilterOperator filterOp) throws LogicalOperatorException {
        ...
        case OPERATOR_NEQ:
            ...
            BasicFunctionOperator basic = new BasicFunctionOperator(TqlParserConstant.getTSTokenIntType(tokenIntType),
    ...
    private void checkTypeFill(TSDataType dataType, int type) throws LogicalOperatorException {
        ...
        case DOUBLE:
            if (type != TOK_LINEAR && type != TOK_PREVIOUS) {
                throw new LogicalOperatorException(
                        String.format("type %s cannot use %s fill function", dataType, TqlParser.tokenNames[type]));
            }
            return;
        case BOOLEAN:
        case TEXT:
    ...
    /**
     * parse time expression, which is addition and subtraction expression of duration time, now() or DataTimeFormat
     * time.
     * <p>
     * eg. now() + 1d - 2h
     * </p>
    ...
    private Long parseTokenDataExpression(AstNode astNode) throws LogicalOperatorException {
        if (astNode.getType() == TqlParser.PLUS) {
            return parseTokenDataExpression(astNode.getChild(0)) + parseTokenDataExpression(astNode.getChild(1));
     ...
     * 
     * @param astNode
     *            represent duration string like: 12d8m9ns, 1y1mo, etc.
