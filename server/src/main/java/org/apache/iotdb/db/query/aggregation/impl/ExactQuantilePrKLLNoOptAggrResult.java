...
public class ExactQuantilePrKLLNoOptAggrResult extends AggregateResult {
  ...
  public ExactQuantilePrKLLNoOptAggrResult(TSDataType seriesDataType)
      throws UnSupportedDataTypeException {
    super(DOUBLE, AggregationType.EXACT_QUANTILE_PR_KLL_NO_OPT);
  ...
  public void startIteration() {
    ...
    if (iteration == 0) { // first iteration
      ...
      heapKLL = new KLLSketchLazyExact(maxMemoryByte);
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_no_opt] start iteration "
              + iteration
              + " cntL,R:"
              + "["
              + longToResult(cntL)
  ...
  public void finishIteration() {
    ...
    if (deterministic_result.length == 3 || deterministic_result[0] >= deterministic_result[1]) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_no_opt]\tAnswer Found."
              + " det_result:"
    ...
    if (cntL == lastCntL && cntR == lastCntR) {
      ...
      hasFinalResult = true;
      System.out.println(
          "\t[ExactQuantile DEBUG pr_kll_no_opt]\tDANGER!! range_not_updated_after_an_iter.");
      return;
    }

    System.out.println(
        "\t[ExactQuantile DEBUG pr_kll_no_opt]\tfinish iter"
            + (iteration - 1)
            + "."
            + " cntK1,2:"
            + cntK1
  ...
  public void updateResultFromStatistics(Statistics statistics) {
    System.out.println("\t\tERROR NoOpt should NOT use stat.");
