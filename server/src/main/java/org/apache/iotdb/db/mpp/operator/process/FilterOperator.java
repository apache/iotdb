...
public class FilterOperator extends TransformOperator {
  ...
  public TsBlock next() {
    ...
    try {
      while (rowCount < FETCH_SIZE && !timeHeap.isEmpty()) {
        ...
        if (filterPointReader.next()
            && filterPointReader.currentTime() == minTime
            && !filterPointReader.isCurrentNull()
            && filterPointReader.currentBoolean()) {
          collectCurrentRow(timeBuilder, columnBuilders, minTime, outputColumnCount);
          ++rowCount;
        } else {
          dropCurrentRow(minTime, outputColumnCount);
        }
      }
    } catch (Exception e) {
      // TODO: throw here?
  ...
  private void dropCurrentRow(long currentTime, int outputColumnCount)
      throws QueryProcessException, IOException {
    // n = outputColumnCount + 1 -> to include filterPointReader
    for (int i = 0, n = outputColumnCount + 1; i < n; ++i) {
