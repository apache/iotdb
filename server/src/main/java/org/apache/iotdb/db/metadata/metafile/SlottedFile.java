...
public class SlottedFile implements SlottedFileAccess {
    ...
    public void readBytes(long position, ByteBuffer byteBuffer) throws IOException {
        ...
        // if the blockBuffer is empty or the target position is not in current block, read the according block
        if (currentBlockPosition == -1 ||
                position < currentBlockPosition || position >= currentBlockPosition + blockSize) {
        ...
        blockBuffer.limit(Math.min(byteBuffer.limit() - byteBuffer.position(), blockSize - blockBuffer.position()));
