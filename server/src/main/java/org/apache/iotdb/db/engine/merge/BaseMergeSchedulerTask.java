...
public abstract class BaseMergeSchedulerTask {

  private static final Logger logger = LoggerFactory.getLogger(
      BaseMergeSchedulerTask.class);
  ...
  private SizeBasedFileSelectorStrategy mergeSizeSelectorStrategy;
  ...
  protected BaseMergeSchedulerTask(MergeContext context, String taskName, MergeLogger mergeLogger,
    ...
    this.mergeSizeSelectorStrategy = (SizeBasedFileSelectorStrategy) IoTDBDescriptor.getInstance()
        .getConfig().getMergeSmallFileStrategyFactory().getMergeSizeSelectorStrategy();
  ...
  protected void mergePaths(List<PartialPath> pathList) throws IOException {
    ...
    for (int i = 0; i < mergeChunkSubTaskNum; i++) {
      ...
      futures.add(MergeManager.getINSTANCE().submitChunkSubTask(
          new BaseMergeChunkHeapTask(seriesHeaps[finalI], currSeqFiles, currUnseqFiles, finalI)));
    }
    for (int i = 0; i < mergeChunkSubTaskNum; i++) {
      try {
        futures.get(i).get();
  ...
  public abstract List<TsFileResource> mergeSeries() throws IOException;
  ...
  public class BaseMergeChunkHeapTask extends MergeChunkHeapTask {
    ...
    public BaseMergeChunkHeapTask(PriorityQueue<PartialPath> seriesHeaps,
