...
public class SeekCostModel {
  ...
  public static final File EMPIRICAL_SEEK_FILE = new File("./data/approximation/seek_cost.csv");
  ...
  public static boolean readEmpiricalData() {
    ...
    try{
      ...
      while(csvReader.readRecord()) {
        // TODO: 读取每一行的 Record，然后转化为 bytes，存入 empiricalData 中
  ...
  /* TODO: Approximate the seek cost during the queries.
  ...
  */
  public static float approximate(List<QueryRecord> queries, List<String> measurements, List<Long> chunkSize) {
    ...
    for(QueryRecord query: queries) {
      String[] queryMeasurements = query.getSensors();
      String[] sortedQueryMeasurements = sortInSameOrder(queryMeasurements, measurements);

      int k = 0;
      for(int i = 0; i < sortedQueryMeasurements.length; ++i) {
        long curSeekDistance = 0;
        while (true) {
          if (!sortedQueryMeasurements[i].equals(measurements.get(k))) {
   ...
   */
  private static String[] sortInSameOrder(String[] queryMeasurements, List<String> measurements) {
    String[] sortedQueryMeasurements = new String[queryMeasurements.length];
    Set<String> measurementSet = new HashSet<>();
    for(int i = 0; i < queryMeasurements.length; ++i) {
      measurementSet.add(queryMeasurements[i]);
    }
    int k = 0;
    for(int i = 0; i < measurements.size(); ++i) {
      if (measurementSet.contains(measurements.get(i))) {
        sortedQueryMeasurements[k++] = measurements.get(i);
      }
    }
