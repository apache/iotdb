...
public class DP { // Dynamic-Programming
    ...
    // 注意k是分段数，不是采点数
    public static List<Point> dynamic_programming(List<Point> points, int k, ERROR errorType, boolean debug) {
        int N = points.size();
            ...
            for (int j = 0; j < N; j++) {
                ...
                if (debug) {
                    for (int xtmp = 0; xtmp < j + 1; xtmp++) {  // 遍历从 0 到 j 的每个元素
                        if (errorType == ERROR.L_infy) {
                            System.out.printf("  max((k_seg_dist[%d, %d] = %f), (dists[%d, %d] = %f)) --> %f%n",
                                    i - 1, xtmp, kSegDist[i - 1][xtmp],
                                    xtmp, j, dists[xtmp][j],
                                    Math.max(kSegDist[i - 1][xtmp], dists[xtmp][j]));
                        } else {
                            System.out.printf("  (k_seg_dist[%d, %d] = %f) + (dists[%d, %d] = %f) --> %f%n",
                                    i - 1, xtmp, kSegDist[i - 1][xtmp],
                                    xtmp, j, dists[xtmp][j],
...
//    }
    ...
    private static double joint_segment_error(List<Point> points, int lx, int rx, ERROR errorType) {
        ...
        if (errorType != ERROR.area) {
            ...
            return Tool.total_areal_displacement(points.subList(lx, rx + 1), // 注意lx,rx左闭右闭
                    linearInterpolation, false);
        }
    }

    public static double error(List<Point> points, List<Integer> sampledIdx, ERROR errorType) {
        double res = 0;
    ...
    public static void main(String[] args) {
        ...
        String input = "D:\\LabSync\\iotdb\\我的Gitbook基地\\RUI Lei gitbook\\士论\\49. visval改进\\notebook\\raw.csv";
