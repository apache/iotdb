...
public class ${className} extends VariationWindowManager {
  ...
  public ${className}(
    ...
    super(variationWindowParameter, ascending);
  ...
  public TsBlock skipPointsOutOfCurWindow(TsBlock inputTsBlock) {
    ...
    Column controlColumn = inputTsBlock.getColumn(variationWindowParameter.getControlColumnIndex());
    ...
    for (; i < size; i++) {
      ...
      if(!isNull) condition = Math.abs(controlColumn.get${dataType.dataType?cap_first}(i) - previousValue)
          > variationWindowParameter.getDelta();
      </#if>
      if(isIgnoringNull()){
        if (!isNull && condition) {
            break;
