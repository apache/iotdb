...
public abstract class AbstractClient {
    ...
    protected static Options createOptions() {
        ...
        Option port = Option.builder(PORT_ARGS).argName(PORT_NAME).hasArg().desc("Port (optional, default 6667)")
                .build();
        options.addOption(port);

        Option username = Option.builder(USERNAME_ARGS).argName(USERNAME_NAME).hasArg().desc("User name (required)")
                .required().build();
        options.addOption(username);

        Option password = Option.builder(PASSWORD_ARGS).argName(PASSWORD_NAME).hasArg().desc("password (optional)")
                .build();
    ...
    protected static String checkRequiredArg(String arg, String name, CommandLine commandLine, boolean isRequired,
    ...
    protected static OPERATION_RESULT handleInputInputCmd(String cmd, IoTDBConnection connection) {
        ...
        if (specialCmd.equals(HELP)) {
            ...
            System.out.println(String.format("    %s=xxx\t eg. long, default, ISO8601, yyyy-MM-dd HH:mm:ss.",
            ...
            System.out.println(
                    String.format("    %s=xxx\t\t set fetch size when querying data from server.", SET_FETCH_SIZE));
            System.out.println(String.format("    %s\t\t show fetch size", SHOW_FETCH_SIZE));
            System.out.println(String.format("    %s=xxx\t eg. set max lines for cli to ouput, -1 equals to unlimited.",
                    SET_MAX_DISPLAY_NUM));
        ...
        if (specialCmd.startsWith(SET_MAX_DISPLAY_NUM)) {
            String[] values = specialCmd.split("=");
            if (values.length != 2) {
                System.out.println(String.format("Max display number format error, please input like %s = 10000",
                        SET_MAX_DISPLAY_NUM));
                return OPERATION_RESULT.CONTINUE_OPER;
            }
            try {
        ...
        if (specialCmd.startsWith(IMPORT_CMD)) {
            ...
            if (values.length != 2) {
                System.out.println(String.format(
                        "Please input like: import /User/myfile. Noted that your file path cannot contain any space character)"));
                return OPERATION_RESULT.CONTINUE_OPER;
            }
            try {
                ...
                System.out.println(
                        String.format("Failed to import from %s because %s", cmd.split(" ")[1], e.getMessage()));
            } catch (TException e) {
