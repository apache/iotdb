...
public class IoTDBPreparedStatement extends IoTDBStatement implements PreparedStatement {
  ...
  // Parameter type constants for serialization
  private static final byte TYPE_NULL = 0x00;
  private static final byte TYPE_BOOLEAN = 0x01;
  ...
  private static final byte TYPE_BINARY = 0x05;
  ...
  private TSExecuteStatementResp executeInternal() throws SQLException {
    ...
    req.setParameters(serializeParameters());
  ...
  /**
   * Serialize parameters to binary format for transmission. Format: [type:1byte][value:variable]
   */
  private List<ByteBuffer> serializeParameters() {
    List<ByteBuffer> serialized = new ArrayList<>();
    for (int i = 0; i < parameterCount; i++) {
      serialized.add(serializeParameter(i));
    }
    return serialized;
  }

  private ByteBuffer serializeParameter(int index) {
    Object value = parameterValues[index];
    int type = parameterTypes[index];

    if (value == null || type == Types.NULL) {
      return ByteBuffer.wrap(new byte[] {TYPE_NULL});
    }

    switch (type) {
      case Types.BOOLEAN:
        ByteBuffer boolBuf = ByteBuffer.allocate(2);
        boolBuf.put(TYPE_BOOLEAN);
        boolBuf.put((byte) ((Boolean) value ? 1 : 0));
        boolBuf.flip();
        return boolBuf;

      case Types.INTEGER:
      case Types.BIGINT:
        ByteBuffer longBuf = ByteBuffer.allocate(9);
        longBuf.put(TYPE_LONG);
        longBuf.putLong(((Number) value).longValue());
        longBuf.flip();
        return longBuf;

      case Types.FLOAT:
      case Types.DOUBLE:
        ByteBuffer doubleBuf = ByteBuffer.allocate(9);
        doubleBuf.put(TYPE_DOUBLE);
        doubleBuf.putDouble(((Number) value).doubleValue());
        doubleBuf.flip();
        return doubleBuf;
      ...
      case Types.CHAR:
        ...
        ByteBuffer strBuf = ByteBuffer.allocate(5 + strBytes.length);
        strBuf.put(TYPE_STRING);
        strBuf.putInt(strBytes.length);
        strBuf.put(strBytes);
        strBuf.flip();
        return strBuf;
      ...
      case Types.VARBINARY:
        ...
        ByteBuffer binBuf = ByteBuffer.allocate(5 + binBytes.length);
        binBuf.put(TYPE_BINARY);
        binBuf.putInt(binBytes.length);
        binBuf.put(binBytes);
        binBuf.flip();
        return binBuf;
      ...
      default:
        ...
        byte[] defaultBytes = strValue.getBytes(StandardCharsets.UTF_8);
        ByteBuffer defaultBuf = ByteBuffer.allocate(5 + defaultBytes.length);
        defaultBuf.put(TYPE_STRING);
        defaultBuf.putInt(defaultBytes.length);
        defaultBuf.put(defaultBytes);
        defaultBuf.flip();
        return defaultBuf;
    }
  }
  ...
  public void setInt(int parameterIndex, int x) throws SQLException {
    checkParameterIndex(parameterIndex);
    parameterValues[parameterIndex - 1] = (long) x;
  ...
  public void setFloat(int parameterIndex, float x) throws SQLException {
    checkParameterIndex(parameterIndex);
    parameterValues[parameterIndex - 1] = (double) x;
