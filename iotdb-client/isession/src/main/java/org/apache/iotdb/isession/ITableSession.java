...
public interface ITableSession extends AutoCloseable {
  ...
  /**
   * Prepares a SQL statement for later execution. The prepared statement is stored on the server
   ...
   * @return a {@link SessionDataSet} containing the query results (if the statement is a query),
   *     or null (if the statement is a non-query statement).
  ...
  /**
   * Executes a SQL statement immediately with the given parameters. This is equivalent to parsing
   ...
   * @throws IoTDBConnectionException if there is an issue with the IoTDB connection.
   * @throws StatementExecutionException if an error occurs while deallocating the statement (e.g.,
