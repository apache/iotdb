...
from iotdb.utils.IoTDBConstants import TS_BLOCK_VALUE_TYPES, TSDataType
...
@unique
class TsBlockColumnEncoding(IntEnum):
    """
    TsBlock column serialization encoding id on the wire.
    Not the same enum as TSEncoding in IoTDBConstants (series encoding in schema).
    """
...
_TS_BLOCK_VALUE_TYPE_ARRAY = np.array(
    [int(t) for t in TS_BLOCK_VALUE_TYPES], dtype=np.uint8
)
...
def read_column_types(buffer, value_column_count):
    ...
    if not np.all(np.isin(data_types, _TS_BLOCK_VALUE_TYPE_ARRAY)):
...
def read_binary_column(buffer, data_type, position_count):
    if data_type not in (TSDataType.TEXT, TSDataType.OBJECT):
        raise Exception("Invalid data type: " + str(data_type))
    null_indicators, buffer = deserialize_null_indicators(buffer, position_count)
...
def repeat(column, data_type, position_count):
    if data_type in (TSDataType.BOOLEAN, TSDataType.TEXT, TSDataType.OBJECT):
        if column.size == 1:
            return np.full(
                position_count,
                column[0],
...
ENCODING_FUNC_MAP = {
    TsBlockColumnEncoding.PLAIN_BOOLEAN: read_byte_column,
    TsBlockColumnEncoding.PLAIN_INT32: read_int32_column,
    TsBlockColumnEncoding.PLAIN_INT64: read_int64_column,
    TsBlockColumnEncoding.PLAIN_BINARY: read_binary_column,
    TsBlockColumnEncoding.RUN_LENGTH: read_run_length_column,
    TsBlockColumnEncoding.DICTIONARY: read_dictionary_column,
}


def read_column(encoding, buffer, data_type, position_count):
    try:
        enc = TsBlockColumnEncoding(int(encoding))
        func = ENCODING_FUNC_MAP[enc]
