...
TEST_CASE("C API - RowRecord and delete data APIs", "[c_rowDelete]") {
  ...
  const char* tpaths[] = {pb, pi, pf, pd, pt, pdate, pblob};
  ...
  const char* names[] = {"sb", "si", "sf", "sd", "st", "sdate", "sblob"};
  TSDataType_C types[] = {TS_TYPE_BOOLEAN, TS_TYPE_INT32, TS_TYPE_FLOAT, TS_TYPE_DOUBLE,
                          TS_TYPE_TEXT, TS_TYPE_DATE, TS_TYPE_BLOB};
  bool bv = true;
  int32_t iv = 42;
  float fv = 2.5f;
  double dv = 3.25;
  ...
  const void* vals[] = {&bv, &iv, &fv, &dv, tv, &dateVal, blobVal};
  REQUIRE(ts_session_insert_record(g_session, dev, 500LL, 7, names, types, vals) == TS_OK);

  CSessionDataSet* dataSet = nullptr;
  REQUIRE(ts_session_execute_query(
              g_session, "select sb,si,sf,sd,st,sdate,sblob from root.cov_types.d1 where time=500",
              &dataSet) == TS_OK);
  REQUIRE(dataSet != nullptr);
  REQUIRE(ts_dataset_has_next(dataSet));
  CRowRecord* record = ts_dataset_next(dataSet);
  ...
  REQUIRE(ts_row_record_get_field_count(record) == 7);
  ...
  REQUIRE(ts_row_record_get_date_int32(record, 5) == 20250507);
  REQUIRE(std::string(ts_row_record_get_string(record, 6)) == "blobXY");
  REQUIRE(ts_row_record_get_string_byte_length(record, 6) == 6);
  for (int i = 0; i < 7; i++) {
