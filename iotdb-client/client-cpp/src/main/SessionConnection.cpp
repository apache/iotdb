...
int32_t SessionConnection::prepareStatement(const std::string& sql,
  ...
  auto result = callWithRetryAndReconnect<TSPrepareResp>(
      ...
      [](const TSPrepareResp& r) { return r.status; });
...
std::unique_ptr<SessionDataSet> SessionConnection::executePreparedStatement(
  ...
  auto result = callWithRetryAndReconnect<TSExecuteStatementResp>(
      ...
      [](const TSExecuteStatementResp& r) { return r.status; });
  TSExecuteStatementResp resp = result.getResult();
  if (result.getRetryAttempts() == 0) {
    RpcUtils::verifySuccessWithRedirection(resp.status);
  } else {
  ...
  return std::unique_ptr<SessionDataSet>(new SessionDataSet(
      sqlForDisplay, resp.columns, resp.dataTypeList, resp.columnNameIndexMap, resp.queryId,
      statementId, client, sessionId, resp.queryResult, resp.ignoreTimeStamp, timeoutInMs,
