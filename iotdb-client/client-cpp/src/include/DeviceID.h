...
public:
  ...
  bool operator!=(const IDeviceID &other) override { return !(*this == other); }
...
private:
  ...
  split_device_id_string(const std::vector<std::string> &splits) {
    ...
    if (segment_cnt == 1) {
      ...
      final_segments.push_back(splits[0]);
               ...
               static_cast<size_t>(DEFAULT_SEGMENT_NUM_FOR_TABLE_NAME + 1)) {
      // "root.a" -> {"root", "a"}
      // "root.a.b" -> {"root.a", "b"}
      std::string table_name =
          std::accumulate(splits.begin(), splits.end() - 1, std::string(),
      ...
      std::string table_name = std::accumulate(
          splits.begin(), splits.begin() + DEFAULT_SEGMENT_NUM_FOR_TABLE_NAME,
