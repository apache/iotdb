...
static bool isPreparedStatementUnsupportedByServer(const std::exception& e) {
  std::string m(e.what());
  return m.find("prepareStatement") != std::string::npos ||
         m.find("Invalid method name") != std::string::npos;
}
...
TEST_CASE("Test TableSession builder with nodeUrls", "[SessionBuilderInit]") {
  ...
  std::shared_ptr<TableSession> session =
      std::shared_ptr<TableSession>(builder->username(iotdb::integration_test::kUsername)
...
TEST_CASE("TableSession prepared statement all supported parameter types",
          "[tablePreparedStatement][tablePreparedAllTypes]") {
  ...
  session->executeNonQueryStatement("CREATE TABLE t_ps_types ("
                                    "tag1 string tag,"
                                    "b boolean field,"
                                    "i32 int32 field,"
                                    "i64 int64 field,"
  ...
  try {
    ...
    unique_ptr<SessionDataSet> sessionDataSet =
        session->executePreparedStatement(sql, statementName, params, -1);
    int cnt = 0;
    while (sessionDataSet->hasNext()) {
