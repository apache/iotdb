...
Example file name:
`client-cpp-2.0.7-SNAPSHOT-linux-x86_64-glibc228.zip`.

**Linux ABI and glibc:** Two Linux families are published. **`glibc228`** packages
are built on [`manylinux_2_28`](https://github.com/pypa/manylinux) with the
image default toolchain and the **new** libstdc++ ABI (symbols contain
`__cxx11`); they require **glibc ≥ 2.28** on the deployment host. Prefer these
on modern distros when you build your application with the system default
`g++`. **`glibc217`** packages are built on `manylinux2014` with the image
default toolchain (legacy libstdc++ ABI, no `__cxx11` symbols) for hosts that
only have **glibc 2.17** and cannot upgrade to 2.28+. Pick the zip that matches
your deployment glibc; build your app with the same default compiler toolchain as
on that host—no extra ABI macros are required.
...
Linux x86_64 (glibc 2.28 — match a modern distro or `manylinux_2_28`):

```bash
mvn -P with-cpp -pl iotdb-client/client-cpp -am -DskipTests \
  -Dclient.cpp.package.classifier=linux-x86_64-glibc228 package
```

Linux x86_64 (glibc 2.17 — CentOS 7 / manylinux2014):
