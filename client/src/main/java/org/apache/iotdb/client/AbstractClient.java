...
public abstract class AbstractClient {
  ...
  private static final Scanner SCREEN_INPUT = new Scanner(System.in);
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      if (hasResultSet) {
        try (ResultSet resultSet = statement.getResultSet()) {
          ...
          printNextBatchResult(zoneId, resultSet, resultSetMetaData, columnLength, maxSizeList, 0);

          int printCount = 1;
          while (isReachMaxLine && !"q".equals(SCREEN_INPUT.nextLine())) {
            printNextBatchResult(zoneId, resultSet, resultSetMetaData, columnLength, maxSizeList,
                printCount * maxPrintRowCount);
            printCount++;
          }
        }
      }
  ...
  private static void printNextBatchResult(ZoneId zoneId, ResultSet resultSet,
      ResultSetMetaData resultSetMetaData, int columnLength, List<Integer> maxSizeList,
      int preLinesCount)
      throws SQLException {
    maxSizeList.clear();
    ...
    if (isReachMaxLine) {
      println(String.format("Reach maxPrintRowCount = %s lines, enter q to exit, other to continue",
          maxPrintRowCount));
    }
  ...
  private static List<List<String>> cacheResult(ResultSet resultSet, List<Integer> maxSizeList,
    ...
    for (int i = 1; i <= columnCount; i++) {
      List<String> list = new ArrayList<>(maxPrintRowCount);
