...
public class IoTDBQueryResultSet implements ResultSet {
    ...
    private int rowsLimit = 0; // 0 means it is not constrained in sql
    private int rowsOffset = 0; // 0 means it is not constrained in sql, or the offset position has been reached
     ...
     * maxRowsOrRowsLimit = maxRows; 4) When maxRows is NOT constrained and LIMIT is constrained, i.e., maxRows=0 and
    ...
    public IoTDBQueryResultSet(Statement statement, List<String> columnName, TSIService.Iface client,
            TS_SessionHandle sessionHandle, TSOperationHandle operationHandle, String sql, String aggregations,
    ...
    public boolean next() throws SQLException {
        ...
        if (rowsOffset != 0) { // When rowsOffset is constrained and the offset position has NOT been reached
        ...
        if (isNext) {
            /*
             * Note that 'rowsFetched' is not increased in the nextWithoutConstraints(), because 'rowsFetched' should
             * not count the rows fetched before the OFFSET position
             */
            rowsFetched++;
        }
