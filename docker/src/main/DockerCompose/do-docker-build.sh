...
function prepare_buildx(){
    if [[ "$do_publish" == "true" ]]; then
        ...
        find ${current_path}/../ -name 'Dockerfile-1.0.0*' | xargs sed -i 's#FROM openjdk:11-jre-slim#FROM --platform=$TARGETPLATFORM eclipse-temurin:17-jre-focal#g'
    else
        docker_build="docker build" ;
        docker_publish="" ;
        find ${current_path}/../ -name 'Dockerfile-1.0.0*' | xargs sed -i 's#FROM --platform=$TARGETPLATFORM eclipse-temurin:17-jre-focal#FROM openjdk:11-jre-slim#g'
    fi
}
