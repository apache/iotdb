...
| **First Response Latency** | 首包响应耗时：从查询进入 Coordinator 到首次 RPC 结束的服务端耗时。选用首包而非完整执行时间，因为：① IoTDB 采用 volcano 迭代器 + 流式 RPC 分批返回，完整执行时间受结果集大小和客户端消费速度影响，与"规划是否值得缓存"无关；② 首包延迟精确覆盖 Parser → Optimizer → Distribution → 首批数据 的端到端路径，分子（规划成本）与分母（首包延迟）描述同一时间窗口，收益率才有实际意义；③ 使用完整执行时间会系统性低估大结果集查询的缓存收益 |
...
Client → Parser → Analyzer → [Plan Cache] ──→ Logical Planner → Optimizer → Distribution Planner → Executor
                                  ...
                                  ├── clone 缓存的逻辑计划
                                  ├── 注入新字面量
                                  ├── adjustSchema（重新拉设备列表、时间过滤、分区）
                                  └──→ Distribution Planner → Executor
```
...
**为什么只有 MISS 和 MONITOR 指向 `recordExecution`**：

`recordExecution()` 需要真实的 `reusablePlanningCost` 和 `firstResponseLatency` 作为入参，只有走过完整规划路径的查询才能产生这两个值。

| 路径 | 是否走完整规划 | 是否调 recordExecution | 原因 |
|---|---|---|---|
| **HIT** | 否（直接 clone 缓存计划） | 否 | 没有真实 `reusablePlanningCost`；代码中不设置 `planCacheCachedKey`，`QueryExecution` 无法触发反馈。HIT 路径改为通过延迟追踪刷新 `ewmaBenefitRatio`（见 §4.1.3 HIT 路径延迟追踪） |
| **MISS** | 是（缓存未命中，回退到完整规划） | **是** | 有真实的规划耗时和首包延迟，需要反馈给状态机（可能触发 ACTIVE → MONITOR 降级） |
| **MONITOR** | 是（采样期必须走完整规划） | **是** | 这是 EWMA 累积样本的唯一途径，`sampleCount` 在此递增，达到阈值后触发状态转换 |
| **BYPASS** | 是（但模板在冷却期中） | 否 | 状态机处于 BYPASS 冷却中，收集数据不会改变状态，直到 cooldown 到期才回 MONITOR |
